...
function preventVehicleExit()
    Citizen.CreateThread(function()
        ...
        disableControl = true
        disableTurning()
        local finished = exports["nadrp-taskbar"]:taskBar(options[math.random(1,3)],"Taking off Harness",true)
...
function toggleHarness()
    ...
    local finished = 0
    if harness then
        finished = exports["nadrp-taskbar"]:taskBar(5000,"Taking off Harness",true)
    else
        finished = exports["nadrp-taskbar"]:taskBar(5000,"Putting on Harness",true)
    end
    if (finished == 100) then
...
AddEventHandler('NetworkNos', function(plate)
    if GetVehicleNumberPlateText(plate) == plate then
        startNos(GetVehiclePedIsIn(PlayerPedId(),false))
    end
end)
...
AddEventHandler('nos:help', function()
    ...
    TriggerEvent('chat:addMessage', {
        ...
        args = {"You have %" .. math.floor(NosVehicles[currentVehicle]) .. " left"}
    })
end)
...
AddEventHandler('event:control:vehicleMod', function(useID)
    if IsPedInAnyVehicle(PlayerPedId()) and not IsThisModelABike(GetEntityModel(currentVehicle)) then
...
--[[Citizen.CreateThread(function()
    while true do 
        ...
        if DecorExistOn(currentVehicle, "vehicleHarnessDur") then
            print(DecorGetFloat(currentVehicle, "vehicleHarnessDur"))
        else
            print('DECOR DOES NOT EXIST:', currentVehicle)
        end
    end
end)]]

RegisterNetEvent("nadrp-vehdamage:setHarness")
AddEventHandler("nadrp-vehdamage:setHarness",function(result, isNew)
...
RegisterNetEvent("nadrp-vehdamage:useHarnessItem")
AddEventHandler("nadrp-vehdamage:useHarnessItem",function()
    TriggerServerEvent("nadrp-vehdamage:applyHarness", GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), false)), 1.0)
    TriggerEvent("inventory:removeItem",'harness', 1)
end)

RegisterNetEvent("nadrp-vehdamage:resetDowngrade")
AddEventHandler("nadrp-vehdamage:resetDowngrade",function(targetvehicle)
    ...
    while true do
        ...
        if tempVehicle ~= currentVehicle then
            ...
            if currentVehicle == nil or currentVehicle == 0 or currentVehicle == false then
                ...
                TriggerServerEvent("nadrp-vehdamage:getHarness", GetVehicleNumberPlateText(currentVehicle))
            end
                        ...
                        if lastCurrentVehicleSpeed > 30.5 and currentVehicleSpeed < (lastCurrentVehicleSpeed * 0.75) then
                            if not IsThisModelABike(GetEntityModel(currentVehicle)) then
                                ...
                                if harness and harnessDurability > 0.0 then
                                    ...
                                    TriggerServerEvent("nadrp-vehdamage:updateHarness", GetVehicleNumberPlateText(currentVehicle), harnessDurability)
                                    if DecorExistOn(currentVehicle, "vehicleHarnessDur") then
            ...
            if IsControlPressed(1,209) and NosVehicles[currentVehicle] ~= nil then
                if not IsThisModelAHeli(vehModel) and not IsThisModelABoat(vehModel) and not IsThisModelABike(vehModel) and IsVehicleOnAllWheels(currentVehicle) and IsVehicleEngineOn(currentVehicle) then
                    if GetEntitySpeed(currentVehicle) > 12.5 and NosVehicles[currentVehicle] > 0 then
                        if GetEntitySpeed(currentVehicle) > 90.0 then -- 250mph
                            SetVehicleEngineHealth(currentVehicle, 0.0)
                            SetVehicleEngineOn(currentVehicle, false, true, true)
...
RegisterCommand('nos', function(source, args, raw)
    if IsPedInAnyVehicle(PlayerPedId(),false) then 
...
--[[RegisterNetEvent('nadrp-vehdamage:EnteredVehicle')
AddEventHandler('nadrp-vehdamage:EnteredVehicle', function(veh)
    if not DecorExistOn(currentVehicle, "vehicleHarnessDur") then
        TriggerEvent("harness", false, 0.0)
        print('1')
    else
        local dur = DecorGetInt(currentVehicle, "vehicleHarnessDur")
        TriggerEvent("harness", true, dur)
        print('2')
    end
end)]]
