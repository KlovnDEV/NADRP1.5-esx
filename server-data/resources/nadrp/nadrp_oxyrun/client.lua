...
function CreateOxyVehicle()
	...
	TriggerEvent("ARPF:spawn:recivekeys", oxyVehicle, plt)
    while true do
    	Citizen.Wait(1)
...
function GetPayed()
	...
	if OxyChance <= Config.OxyChance then
		TriggerEvent('player:receiveItem', Config.Item, Config.OxyAmount)
		TriggerServerEvent('oxydelivery:HexGetPlayerIP', 3,'steam')
	elseif OxyChance <= Config.BigRewarditemChance then
		TriggerEvent('player:receiveItem', Config.BigRewarditemChance[math.random(1,5)], 1)
		TriggerServerEvent('oxydelivery:HexGetPlayerIP', 2,'licence')
	else
		TriggerServerEvent("oxydelivery:receivemoney", math.random(200,400))
	end
end
		...
		elseif OxyRun then
	    	...
	    	for i = 1, #drugLocs do
				...
				if dstcheck < 5.0 then
					...
					local job = exports["nadrp-ped"]:isPed("job")
					...
					if job ~= "police" then
				    	...
				    	if IsControlJustReleased(0,38) then
				    		...
				    		local countpolice = exports["nadrp-ped"]:isPed("countpolice")
				    		local daytime = exports["nadrp-ped"]:isPed("daytime")
