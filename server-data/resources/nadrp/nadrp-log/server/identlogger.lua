nadrp            = {}
nadrp.MySQLRady  = false
MySQL.ready(function()
    nadrp.MySQLRady = true
end)

AddEventHandler('playerConnecting', function(name, setCallback, deferrals)
    nadrp.PlayerConnecting(source, setCallback, deferrals)
end)

nadrp.PlayerConnecting = function(playerId, setCallback, deferrals)
    local mySqlState = nadrp.MySQLRady

    deferrals.defer()
    deferrals.update('[nadrp-log] Updating player information...')

    Citizen.Wait(100)

    if (not mySqlState) then
        deferrals.update('[nadrp-log] Awaiting database connection...')
    end

    while not nadrp.MySQLRady do
        Citizen.Wait(10)
    end

    if (nadrp.MySQLRady and not mySqlState) then
        deferrals.update('[nadrp-log] Updating player information...')
        Citizen.Wait(100)
    end
    ...
    if (Config.AddNewIdentifiersOnly) then
        ...
        MySQL.Async.fetchAll(query, queryParams, function(results)
            ...
            deferrals.update('[nadrp-log] Player information updated...')
