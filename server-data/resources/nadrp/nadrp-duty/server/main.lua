...
AddEventHandler('duty:onoff', function(job)
    ...
    if job == 'police' or job == 'ambulance' or job == 'mechanic' or job =='doc' or job =='dispatch' then
        xPlayer.setJob('off' ..job, grade)
        TriggerClientEvent('mythic_notify:client:DoLongHudText', source, { type = 'inform', text = 'You are now off-duty', length = 9000})
        TriggerClientEvent('mythic_notify:client:DoLongHudText', source, { type = 'error', text = 'Tracker status = off', length = 9000})
        TriggerEvent("eblips:remove", _source)
    elseif job == 'offpolice' then
        xPlayer.setJob('police', grade)
        TriggerClientEvent('mythic_notify:client:DoLongHudText', source, { type = 'inform', text = 'You are now on-duty', length = 9000})
        TriggerClientEvent('mythic_notify:client:DoLongHudText', source, { type = 'success', text = 'Tracker status = on', length = 9000})
