...
$(function () {
    ...
    function CloseMenu(save) {
        $.post('http://nadrp_clothes/escape', JSON.stringify({save:save}));
    }

    $(document).on('contextmenu', function() {
        $.post('http://nadrp_clothes/togglecursor', JSON.stringify({}));
    })
    ...
    function UpdateInputs(drawables, props, drawtextures, proptextures, skin) {
        for (let i = 0; i < Object.keys(drawables).length; i++) {
        ...
        for (let i = 0; i < Object.keys(drawtextures).length; i++) {
            $("#" + drawtextures[i][0]).find('.input-number').eq(1).val(drawtextures[i][1]);
        }
        for (let i = 0; i < Object.keys(proptextures).length; i++) {
    ...
    $('#skin_string').find('input').keypress(function (e) {
        if (e.which == 13) {
            $.post('http://nadrp_clothes/customskin', JSON.stringify($(this).val()));
        }
    })

    function inputChange(ele,inputType) {
        let inputs = $(ele).parent().parent().find('.input-number');
        let total = 0;
        ...
        if (currentMenu.is($('#clothesmenu')) || $(ele).parents('.panel').hasClass('hair')) {
            ...
            if ($(ele).parents('.panel').attr('id') === "skins") {
                $.post('http://nadrp_clothes/setped', JSON.stringify({
                    "name": $(ele).attr('id'),
                    "value": $(ele).val()
                }))
            }
                ...
                $.post('http://nadrp_clothes/updateclothes', JSON.stringify({
                    "name": nameId,
            ...
            $.post('http://nadrp_clothes/settats', JSON.stringify({tats}))
        }
    }
    ...
    $('.tog_head').on('click', function() {
        toggleCam($(this));
        $.post('http://nadrp_clothes/switchcam', JSON.stringify({name: 'head'}))
    })
    $('.tog_torso').on('click', function() {
        toggleCam($(this));
        $.post('http://nadrp_clothes/switchcam', JSON.stringify({name: 'torso'}))
    })
    $('.tog_leg').on('click', function() {
        toggleCam($(this));
        $.post('http://nadrp_clothes/switchcam', JSON.stringify({name: 'leg'}))
    })
    $('.tog_cam').on('click', function() {
        toggleCam($(this));
        $.post('http://nadrp_clothes/switchcam', JSON.stringify({name: 'cam'}))
    })


    $('.tog_hat').on('click', function() {
        $.post('http://nadrp_clothes/toggleclothes', JSON.stringify({name: "hats"}))
    })
    $('.tog_glasses').on('click', function() {
        $.post('http://nadrp_clothes/toggleclothes', JSON.stringify({name: "glasses"}))
    })
    $('.tog_tops').on('click', function() {
        // dont look at this :)
        $.post('http://nadrp_clothes/toggleclothes', JSON.stringify({name: "jackets"}))
        $.post('http://nadrp_clothes/toggleclothes', JSON.stringify({name: "undershirts"}))
        $.post('http://nadrp_clothes/toggleclothes', JSON.stringify({name: "torsos"}))
        $.post('http://nadrp_clothes/toggleclothes', JSON.stringify({name: "vest"}))
        $.post('http://nadrp_clothes/toggleclothes', JSON.stringify({name: "bags"}))
        $.post('http://nadrp_clothes/toggleclothes', JSON.stringify({name: "neck"}))
        $.post('http://nadrp_clothes/toggleclothes', JSON.stringify({name: "decals"}))
    })
    $('.tog_legs').on('click', function() {
        $.post('http://nadrp_clothes/toggleclothes', JSON.stringify({name: "legs"}))
    })
    $('.tog_mask').on('click', function() {
        $.post('http://nadrp_clothes/toggleclothes', JSON.stringify({name: "masks"}))
    })

    $('#reset').on('click', function() {
        $.post('http://nadrp_clothes/resetped', JSON.stringify({}))
    })
    ...
    window.addEventListener("keydown", throttle(function (ev) {
        ...
        if (_key) {
            $.post('http://nadrp_clothes/rotate', JSON.stringify({key: _key}))
        }
    }, 50))
    ...
    function SaveHeadBlend() {
        ...
        $.post('http://nadrp_clothes/saveheadblend', JSON.stringify(headBlend))
    }

    function SaveFaceShape(ele) {
        $.post('http://nadrp_clothes/savefacefeatures', JSON.stringify({name: ele.attr('data-value'), scale: ele.val()}))
    }
    ...
    function SaveHeadOverlay(ele) {
        ...
        $.post('http://nadrp_clothes/saveheadoverlay', JSON.stringify({
            name: id,
            value: inputs.eq(0).val(),
            opacity: opacity
        }))
    ...
    function AddPalettes() {
        ...
        $('.color_palette').on('click', function() {
            ...
            if ($(this).parents('.panel').hasClass('hair')) {
                $.post('http://nadrp_clothes/savehaircolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value'),
                    secondColour: palettes.eq(1).find('.active').attr('value')
                }));
            }
            else {
                $.post('http://nadrp_clothes/saveheadoverlaycolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value'),
                    secondColour: palettes.eq(1).find('.active').attr('value'),
