...
RegisterServerEvent("nadrp-clothes:insert_character_current")
AddEventHandler("nadrp-clothes:insert_character_current",function(data)
...
RegisterServerEvent("nadrp-clothes:insert_character_skin")
AddEventHandler("nadrp-clothes:insert_character_skin",function(data)
    if not data then return end
    local src = source
    local user = ESX.GetPlayerFromId(source)
    local characterId = user.identifier
...
RegisterServerEvent("nadrp-clothes:insert_character_face")
AddEventHandler("nadrp-clothes:insert_character_face",function(data)
    if not data then return end
    local src = source
    local user = ESX.GetPlayerFromId(source)
    local characterId = user.identifier
...
RegisterServerEvent("nadrp-clothes:get_character_face")
AddEventHandler("nadrp-clothes:get_character_face",function(pSrc)
    ...
    MySQL.Async.fetchAll("SELECT cc.model, cf.hairColor, cf.headBlend, cf.headOverlay, cf.headStructure FROM character_face cf INNER JOIN character_current cc on cc.cid = cf.identifier WHERE cf.identifier = @identifier", {['identifier'] = characterId}, function(result)
        if (result ~= nil and result[1] ~= nil) then
            ...
            if model == 1885233650 or model == -1667301416 then
                TriggerClientEvent("nadrp-clothes:setpedfeatures", src, temp_data)
            end
        else
            TriggerClientEvent("nadrp-clothes:setpedfeatures", src, false)
        end
	end)
end)

RegisterServerEvent("nadrp-clothes:get_character_current")
AddEventHandler("nadrp-clothes:get_character_current",function(pSrc)
    local src = (not pSrc and source or pSrc)
    local user = ESX.GetPlayerFromId(source)
    local characterId = user.identifier
    ...
    MySQL.Async.fetchAll('SELECT model,drawables,props,drawtextures,proptextures FROM character_current WHERE cid = @identifier', {['@identifier'] = characterId}, function(result)
        ...
        TriggerClientEvent("nadrp-clothes:setclothes", src, temp_data,0)
	end)
end)

RegisterServerEvent("nadrp-clothes:retrieve_tats")
AddEventHandler("nadrp-clothes:retrieve_tats", function(pSrc)
    local src = (not pSrc and source or pSrc)
    local user = ESX.GetPlayerFromId(src)
    ...
    MySQL.Async.fetchAll("SELECT * FROM playerstattoos WHERE identifier = @identifier", {['identifier'] = cid}, function(result)
        if(#result == 1) then
			TriggerClientEvent("nadrp-clothes:settattoos", src, json.decode(result[1].tattoos))
        else
			local tattooValue = "{}"
            ...
            TriggerClientEvent("nadrp-clothes:settattoos", src, {})
		end
	end)
end)

RegisterServerEvent("nadrp-clothes:set_tats")
AddEventHandler("nadrp-clothes:set_tats", function(tattoosList)
	local src = source
	local user = ESX.GetPlayerFromId(source)
...
RegisterServerEvent("nadrp-clothes:get_outfit")
AddEventHandler("nadrp-clothes:get_outfit",function(slot)
    ...
    MySQL.Async.fetchAll("SELECT * FROM character_outfits WHERE cid = @cid and slot = @slot", {
        ...
        if result and result[1] then
            ...
            TriggerClientEvent("nadrp-clothes:setclothes", src, data,0)
            local values = {
                ["@cid"] = characterId,
                ["@model"] = data.model,
                ["@drawables"] = json.encode(data.drawables),
...
RegisterServerEvent("nadrp-clothes:set_outfit")
AddEventHandler("nadrp-clothes:set_outfit",function(slot, name, data)
    if not slot then return end
    local src = source
...
RegisterServerEvent("nadrp-clothes:remove_outfit")
AddEventHandler("nadrp-clothes:remove_outfit",function(slot)
...
AddEventHandler("nadrp-clothes:list_outfits",function()
    local src = source
    local user = ESX.GetPlayerFromId(source)
    local cid = user.identifier
    local slot = slot
        ...
        exports.ghmattimysql:scalar("SELECT count(model) FROM character_current WHERE cid = @cid LIMIT 1", {
            ...
            if result == 0 then
                MySQL.Async.fetchAll("select count(cid) assExist from (select cid  from character_current union select cid from users_clothes) a where cid =  @cid", {['@cid'] = cid}, function(clothingCheck)
                    ...
                    TriggerClientEvent('nadrp-clothes:setclothes',src,{},existsClothing)
                end)
                return
            else
                TriggerEvent("nadrp-clothes:get_character_current", src)
            end
            TriggerClientEvent("nadrp-clothes:inService",src,isService)
    	end)
    end)
end)
