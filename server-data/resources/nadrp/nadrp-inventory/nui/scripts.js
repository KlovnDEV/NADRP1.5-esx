...
$(document).ready(function() {
    ...
    window.addEventListener('message', function(event) {
        ...
        if (item.response == "openGui") {
            ...
            if (itemList[item.id]) {
                $.post("http://nadrp-inventory/GiveItem", JSON.stringify([item.id, item.amount, item.generateInformation, true, itemList[item.id].nonStack, item.data, itemList[item.id].weight]));
            } else {
                $.post("http://nadrp-inventory/GiveItem", JSON.stringify([item.id, item.amount, item.generateInformation, false, itemList[item.id].nonStack, item.data]));
            }
        }
    })
...
function UpdateQuality(data, penis) {
    ...
    if (quality == 0) {
        ...
        itemMaxed = "class='quality25'"
    } else if (quality <= 50) {
        itemMaxed = "class='quality50'"
    }
...
function invStack(targetSlot, moveAmount, targetInventory, originSlot, originInventory, purchase, itemCosts, itemidsent, amountmoving, crafting, weapon, amountRemaining) {
    ...
    $.post("http://nadrp-inventory/stack", JSON.stringify(arr));
};
...
function invMove(targetSlot, originSlot, targetInventory, originInventory, purchase, itemCosts, itemidsent, amountmoving, crafting, weapon) {
    ...
    $.post("http://nadrp-inventory/move", JSON.stringify(arr));
...
function invSwap(targetSlot, targetInventory, originSlot, originInventory) {
    ...
    $.post("http://nadrp-inventory/swap", JSON.stringify(arr));
};

function removeCraftItems(itemid, moveAmount) {
    let arr = itemList[itemid].craft
    let amount = moveAmount;
    $.post("http://nadrp-inventory/removeCraftItems", JSON.stringify([arr, amount]));
};
...
function DisplayInventoryMultiple(playerinventory, itemCount, invName, targetinventory, targetitemCount, targetinvName, cash, Owner, serverId) {
    ...
    if (targetinvName.indexOf("Drop") > -1) {
        ...
        slotLimitTarget = 200;
    } else if (targetinvName.indexOf("gun-warehouse") > -1) {
        ...
        slotLimitTarget = 80;
    } else if (targetinvName.indexOf("stash5") > -1) {
        ...
        slotLimitTarget = 5;
    } else if (targetinvName.indexOf("drivethru") > -1) {
...
function BuildDrop(brokenSlots) {
    $.post("http://nadrp-inventory/dropIncorrectItems", JSON.stringify({
        slots: brokenSlots
    }));
}
    ...
    for (i = 0; i < slotLimit + 1; i++) {
        ...
        if (isEmpty(inventory[i])) {
            ...
            if (itemList[itemid] !== undefined) {
                ...
                /*$.post("http://nadrp-inventory/updateMyQuality", JSON.stringify({
                  item_id: itemid,
                  slot: slot,
                  quality: quality
                }));*/
...
function UpdateSetWeights() {
    ...
    $.post("http://nadrp-inventory/Weight", JSON.stringify({
        weight: meh
    }));
}
...
function DragToggle(slot, using) {
    ...
    if (slot) {
        ...
        if (occupiedslot == true && dragging == false) {
            ...
            $.post("http://nadrp-inventory/SlotInuse", JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
...
function CompileStacks(targetSlot, originSlot, inv1, inv2, originAmount, remainingAmount, targetAmount, purchase, itemCosts, itemidsent, moveAmount) {
    ...
    $.post("http://nadrp-inventory/SlotJustUsed", JSON.stringify({
    ...
    if (!isNaN(targetSlot) && !isNaN(originSlot)) {
...
function SwapStacks(targetSlot, originSlot, inv1, inv2) {
    // $.post('http://nadrp-inventory/swapstack', JSON.stringify({
    //   slot1: slot1,
    //   slot2: slot2,
    //   inv1: inv1,
    //   inv2: inv2,
...
function closeInv(pIsItemUsed = false) {
    ...
    $.post("http://nadrp-inventory/ServerCloseInventory", JSON.stringify({
        name: TargetInventoryName
    }));
    TargetInventoryName = "none";
    $.post("http://nadrp-inventory/Close", JSON.stringify({
...
function useitem() {
    if (dragging != false) {
        ...
        if (inventoryUsedName == PlayerInventoryName) {
            ...
            $.post("http://nadrp-inventory/invuse", JSON.stringify(arr));
