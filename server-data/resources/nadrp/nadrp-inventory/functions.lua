...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon)
    ...
    if (itemid == "electronickit") then
        TriggerEvent("nadrp-paletorob:useEleckit")
    end
    ...
    if (itemid == "lsdtab" or itemid == "badlsdtab") then
        ...
        local finished = exports["nadrp-taskbar"]:taskBar(3000,"Taking LSD 🤯",false,false,playerVeh)
        if (finished == 100) then
            if hasEnoughOfItem(itemid, 1,false) then
                TriggerEvent("status:setState",2,1200)
                TriggerEvent("status:setState",24,1200)
                TriggerEvent("nadrp-fx:run", "lsd", 180, nil, (itemid == "badlsdtab" and true or false))
                remove = true
            else
                DumFuk = true
            end
    ...
    if (itemid == "femaleseed") then
        ...
        TriggerEvent("nadrp-weed:startcrop","female")
    end

    if (itemid == "maleseed") then
        TriggerEvent("status:setState",4,1600)
        TriggerEvent("nadrp-weed:startcrop","male")
    end

    if (itemid == "weedoz") then
        local finished = exports["nadrp-taskbar"]:taskBar(5000,"Packing Q Bags",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "weed") then
        ...
        local finished = exports["nadrp-taskbar"]:taskBar(10000,"Packing into Bags",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "weedq") then
        local finished = exports["nadrp-taskbar"]:taskBar(1000,"Rolling Joints",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "lighter") then
        ...
        local finished = exports["nadrp-taskbar"]:taskBar(2000,"Starting Fire",false,false,playerVeh)
        if (finished == 100) then
        end
    end
    ...
    if (itemid == "vodka" or itemid == "beer" or itemid == "guiness" or itemid == "whiskey") or itemid == "peakyb" then
        ...
        TriggerEvent("nadrp-fx:run", "alcohol", 180, alcoholStrength)
    end
    ...
    if (itemid == "advlockpick") then
        if #(GetEntityCoords(PlayerPedId()) - vector3(-104.8828,6472.442,30.727)) < 10.0 then
            TriggerEvent('nadrp-paletorob:useAdvlockpick')
        else
    ...
    if (itemid == "heavyammo") then
        local finished = exports["nadrp-taskbar"]:taskBar(5000,"Reloading",false,false,playerVeh)
        if (finished == 100) then
            if hasEnoughOfItem(itemid, 1, false) then
    ...
    if (itemid == "usbdevice") then
        --local finished = exports["nadrp-taskbar"]:taskBar(15000,"Scanning For Networks",false,false,playerVeh)
        --if (finished == 100) then
    ...
    if (itemid == "armor") or (itemid == "armor2")then
        local finished = exports["nadrp-taskbar"]:taskBar(10000,"Armor",true,false,playerVeh)
        if (finished == 100) then
            if hasEnoughOfItem(itemid, 1, false) then
    ...
    if (itemid == "radio") or (itemid == "radio2") then
        TriggerEvent('nadrp-radio:use', source)
    end
            ...
            Citizen.CreateThread(function()
                while finished ~= 100 and not cancelNos do
                    ...
                    if GetEntitySpeed(GetVehiclePedIsIn(player, false)) > 11 then
                        exports["nadrp-taskbar"]:closeGuiFail()
                        cancelNos = true
                    end
                end
            end)
            finished = exports["nadrp-taskbar"]:taskBar(5000,"Nitrous")
    ...
    if (itemid == "bluesec")  then
        ...
        TriggerServerEvent('nadrp-securityheists:banktstart')
    end
    ...
    if (itemid == "cigar") then
        exports["nadrp-taskbar"]:taskBar(2500,"Lighting Up",false,false,playerVeh)
        Wait(300)
        TriggerEvent('attach:cigar')
    end

    if (itemid == "oxygentank") then
        local finished = exports["nadrp-taskbar"]:taskBar(30000,"Oxygen Tank",true,false,playerVeh)
        if (finished == 100) then
            if hasEnoughOfItem(itemid, 1, false) then
    ...
    if (itemid == "methbag") then
        local finished = exports["nadrp-skillbar"]:taskBar(2500,10)
        if (finished == 100) then
            if hasEnoughOfItem(itemid, 1, false) then
                TriggerEvent("attachItemObjectnoanim","crackpipe01")
                TriggerEvent("status:setState",2,1200)
    ...
    if (itemid == "shitlockpick") then
        ...
        local finished = exports["nadrp-skillbar"]:taskBar(2500,math.random(5,20))
        if (finished == 100) then
            if hasEnoughOfItem(itemid, 1, false) then
    ...
    if (itemid == "harness") then
        ...
        if (PlayerPedId() == driver) then
            TriggerEvent("nadrp-vehdamage:useHarnessItem")
        end
    end
...
function GetItemInfo(checkslot)
    for i,v in pairs(clientInventory) do
        if (tonumber(v.slot) == tonumber(checkslot)) then
            local quality = exports["nadrp-inventory"]:ConvertQuality(v.item_id,v.creationDate)
...
function TaskItem(dictionary,animation,typeAnim,timer,message,func,remove,itemid,playerVeh)
    ...
    if timer > 0 then
        local finished = exports["nadrp-taskbar"]:taskBar(timer,message,true,false,playerVeh)
...
AddEventHandler('inv:lockPick', function(isForced,inventoryName,slot)
    ...
    if targetVehicle == 0 then
        ...
        local finished = exports["nadrp-skillbar"]:taskBar(25000,3)
        if finished ~= 100 then
            lockpicking = false
            return
        end

        local finished = exports["nadrp-skillbar"]:taskBar(2200,10)
        if finished ~= 100 then
            lockpicking = false
            return
        end
        ...
        if targetVehicle ~= 0 and not isForced then
            ...
            local finished = exports["nadrp-skillbar"]:taskBar(math.random(5000,25000),math.random(10,20))
            if finished ~= 100 then
                 lockpicking = false
                return
            end
        ...
        elseif isForced then 
            local hasLockpick = export['nadrp-inventory']:hasEnoughOfItem('lockpick', 1, false)
            if hasLockpick then 
...
--[[RegisterCommand("steal", function(source, args)
    ...
    if closestPlayer ~= -1 and closestDistance <= 1.0 then
        ...
        if IsEntityPlayingAnim(searchPlayerPed, 'missminuteman_1ig_2', 'handsup_enter', 3) or (IsEntityPlayingAnim(searchPlayerPed, "dead", "dead_a", 3)) then
            ...
            local finished = exports["nadrp-taskbar"]:taskBar(3500, "Robbing")
...
AddEventHandler('shipcrate:OnUse', function()
    local hasItem = exports['nadrp-inventory']:hasEnoughOfItem('shipcrate',1)
    if hasItem then
...
function OpenBuyLicenseMenu()
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'nadrp-inventory',{
...
RegisterNetEvent('nadrp-inventory:DisableInventory')
AddEventHandler('nadrp-inventory:DisableInventory', function(toggle)
    isDisabled = toggle
end)