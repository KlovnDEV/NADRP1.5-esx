...
function GroupName(groupid)
  ...
  --local mypasses = exports["nadrp-ped"]:isPed("passes")
  for i=1, #mypasses do
...
AddEventHandler("phone:error", function()
    SendNUIMessage({
      openSection = "error",
      textmessage = "<b>Network Error</b> <br><br> Please contact support if this error persists, thank you for using nadrp Phone Services.",
    })
end)
...
AddEventHandler("gangTasks:updated", function()
  --[[local gang = exports["nadrp-ped"]:isPed("gang")
  local cid = tonumber(exports["nadrp-ped"]:isPed("steamid"))]]
  ...
  local taskObject = {}
    for i = 1, #activeTasks do
        ...
        --local passes = exports["nadrp-ped"]:isPed("passes")
            for z = 1, #passes do
...
function hasPhone()
  local phone = exports["nadrp-inventory"]:hasEnoughOfItem("mobilephone",1,false)
  if phone then
    return true
  else
    return false
...
function hasRadio()
    --if exports["nadrp-inventory"]:hasEnoughOfItem("radio",1,false) and not exports["nadrp-ped"]:isPed("disabled") then
      return true
    --else
     -- return false
    --end
...
AddEventHandler('canPing', function(target)
  ...
  local isCuffed = exports['nadrp-ped']:isPed('handcuffed')
...
function hasDecrypt2()
    if exports["nadrp-inventory"]:hasEnoughOfItem("vpnxj",1,false) and not exports["nadrp-ped"]:isPed("disabled") then
        return true
    else
        return false
    end
...
function GotPhone()
    local pDead = exports["nadrp-death"]:GetDeath()
    if not pDead then
        openGuiNow()
    else
...
AddEventHandler('phone:addToCall', function(voipchannel)
    ...
    exports["nadrp-voice"]:addPlayerToCall(tonumber(voipchannel))
end)
...
AddEventHandler('phone:removefromToko', function(playerRadioChannel)
  exports["nadrp-voice"]:removePlayerFromCall()
end)
