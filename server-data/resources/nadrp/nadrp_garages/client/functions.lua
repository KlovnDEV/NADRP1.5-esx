...
SpawnVehicle = function(data,recuperar)
	...
	ESX.Game.SpawnVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
        ...
        Citizen.Wait(100)
        TriggerServerEvent('nadrp_garages:modifystate', vehicleProps, 0, nil)
...
PutInVehicle = function()
    ...
    if DoesEntityExist(vehicle) then
		...
		ESX.TriggerServerCallback("nadrp_garages:validateVehicle", function(valid)
			if valid then
				AbrirMenuGuardar()
            else
                CloseMenu()
...
SaveInGarage = function(garage)
    ...
    deleteCar(vehicle)
    TriggerServerEvent('nadrp_garages:modifystate', vehicleProps, 1, garage)
    CloseMenu()
end
