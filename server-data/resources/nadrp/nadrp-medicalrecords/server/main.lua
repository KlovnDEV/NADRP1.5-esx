...
RegisterServerEvent("nadrp-medicalrecords:create")
AddEventHandler("nadrp-medicalrecords:create", function(data, name)
    local ts = os.time()
...
RegisterServerEvent("nadrp-medicalrecords:server:search")
AddEventHandler("nadrp-medicalrecords:server:search", function(input)
    local data = false
    local src = source
    ...
    TriggerClientEvent("nadrp-medicalrecords:client:search", src, data)
end)
RegisterServerEvent("nadrp-medicalrecords:server:getName")
AddEventHandler("nadrp-medicalrecords:server:getName", function(identifier)
    local src = source
    local result = MySQL.Sync.fetchAll("SELECT firstname, lastname FROM users WHERE identifier = @identifier", {
        ["@identifier"] = identifier
    })
    TriggerClientEvent("nadrp-medicalrecords:getName", src, result[1])
end)
