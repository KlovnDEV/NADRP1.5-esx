...
RegisterServerEvent('nadrp-weed:createplant')
AddEventHandler('nadrp-weed:createplant', function(pX,pY,pZ,strain,seed)
    local src = source
                    ...
                    MySQL.Async.execute('INSERT INTO weed_plants (identifier, coords, strain, growth, status) VALUES (@ident,@coords,@strain, @growth,@status)', {
                        ...
                        if tonumber(result) == 1 then
                            MySQL.Async.fetchAll('SELECT id,coords,strain,growth,status FROM weed_plants', {
                                ...
                                if result2 ~= nil then
                                    TriggerClientEvent('nadrp-weed:currentcrops', -1, result2)
                                    TriggerClientEvent('nadrp-weed:removeItems', user.source, seed)
                                end
                            end)
                        end
                    end)
...
RegisterServerEvent('nadrp-weed:UpdateWeedGrowth')
AddEventHandler('nadrp-weed:UpdateWeedGrowth', function(id,percent, status)
    local src = source
    local user = ESX.GetPlayerFromId(src)
    local ident = user.identifier
    ...
    if id ~= nil then
        MySQL.Async.execute('UPDATE weed_plants SET growth=@growth WHERE identifier=@ident AND id=@id', {
            ...
            if tonumber(result) == 1 then
                TriggerClientEvent('nadrp-weed:updateplantwithID',user.source, id,percent, "alter")
                for i=1, #crops do 
...
RegisterServerEvent('nadrp-weed:killplant')
AddEventHandler('nadrp-weed:killplant', function(id)
    local src = source
    local user = ESX.GetPlayerFromId(src)
    ...
    if id ~= nil then
        MySQL.Async.execute('DELETE FROM weed_plants WHERE id=@id', {
            ...
            if tonumber(result) == 1 then
                TriggerClientEvent('nadrp-weed:updateplantwithID', -1,id,0,'remove')
                for i=1, #crops do
                    if crops[i]['dbID'] == id then
...
RegisterServerEvent('nadrp-weed:RequestWeedPlants')
AddEventHandler('nadrp-weed:RequestWeedPlants', function()
    local src = source
    local user = ESX.GetPlayerFromId(src)
    local ident = user.identifier
    ...
    MySQL.Async.fetchAll('SELECT id,coords,strain,growth,status FROM weed_plants', {
        ...
        if result ~= nil then
            TriggerClientEvent('nadrp-weed:currentcrops', user.source, result)
        end
    end)
end)

RegisterServerEvent('nadrp-weed:updatePlants')
AddEventHandler('nadrp-weed:updatePlants', function(pCrops)
    local src = source
    local user = ESX.GetPlayerFromId(src)
    local ident = user.identifier
    if pCrops ~= nil then
...
RegisterServerEvent('nadrp-weed:convert')
AddEventHandler('nadrp-weed:convert', function(id)
    local src = source
    local user = ESX.GetPlayerFromId(src)
    ...
    if id ~= nil then
        ...
        TriggerClientEvent('nadrp-weed:updateplantwithID', -1, { id }, 0, 'convert')
    end
end)
    ...
    MySQL.Async.fetchAll('SELECT * FROM weed_plants', {}, function(result)
        if result ~= nil then
            for i=1, #result do
                ...
                TriggerClientEvent('nadrp-weed:updateplantwithID', -1, result[i].id, result[i].growth, 'alter')
            end
        end
    end)
