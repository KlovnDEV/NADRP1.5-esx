...
RegisterCommand("jail", function(source, args, raw)
	...
	local targ = ESX.GetPlayerFromId(tgt)
	local tid = targ.identifier
			...
			if jailTime ~= nil then
				...
				if args[2] ~= nil then
					GetRPName(tgt, function(Firstname, Lastname)
						local str = ' You Jailed [' .. Firstname .. ' ' .. Lastname .. '] for ' .. jailTime .. ' months'
                        TriggerClientEvent('chat:addMessage', src, {
                            template = '<div class="chat-message nonemergency">DOC | {0} </div>',
                            args = {str}
						})
					end)
					TriggerEvent('server-jail-item',tid,true)
				end
			else
...
RegisterServerEvent("tac-qalle-jail:jailPlayer")
AddEventHandler("tac-qalle-jail:jailPlayer", function(t, jT, jR)
	...
	if legit["legit"] == true then
		...
		GetRPName(tgt, function(Firstname, Lastname)
			TriggerClientEvent("tac:showNotification", src, Firstname .. " " .. Lastname .. " has been sentenced to " .. jailTime .. " minutes in jail.")
            local str = ' You Jailed [' .. Firstname .. ' ' .. Lastname .. '] for ' .. jailTime .. ' months'
            TriggerClientEvent('chat:addMessage', src, {
                template = '<div class="chat-message nonemergency">DOC | {0} </div>',
                args = {str}
...
AddEventHandler("tac-qalle-jail:unJailPlayer", function(tI)
	local src = source;
	...
	if xTgt ~= nil then
		...
		if legit["legit"] == true then
            UnJail(tgt)
            local str = ' You have been released from jail'
            TriggerClientEvent('chat:addMessage', src, {
                template = '<div class="chat-message nonemergency">DOC | {0} </div>',
                args = {str}
            })
...
AddEventHandler("tac-qalle-jail:prisonWorkReward", function()
	...
	if sql ~= nil then
		...
		if sql > 0 then
			...
			TriggerClientEvent("tac:showNotification", src, "Thanks, here you have some cash for food!")
		else
			print(
				string.format(
...
AddEventHandler('ffrp-jail:reclaimPossessions', function()
	...
	if ident ~= nil then
		TriggerEvent('server-jail-item', ident, false)
	end
end)

function JailPlayer(jailPlayer, jailTime)
	TriggerClientEvent("tac-qalle-jail:jailPlayer", jailPlayer, jailTime)
...
ESX.RegisterServerCallback("tac-qalle-jail:retrieveJailedPlayers", function(source, cb)
	local jailedPersons = {}
...
ESX.RegisterServerCallback("tac-qalle-jail:retrieveJailTime", function(source, cb)
	local src = source
	local xPlayer = ESX.GetPlayerFromId(src)
	local Identifier = xPlayer.identifier
...
exports('GetJailStatus', function(identifier)
	MySQL.Async.fetchAll("SELECT jail FROM users WHERE identifier = @identifier", { ["@identifier"] = identifier }, function(result)
		local JailTime = tonumber(result[1].jail)
		if JailTime > 0 then
			local inJail = true
