	...
	TriggerEvent('es:getPlayerFromId', source, function(user)
		TriggerEvent('es:canGroupTarget', user.getGroup(), "admin", function(available)
			...
			elseif t == "level" then
				if(GetPlayerName(USER) == nil)then
					TriggerClientEvent('chat:addMessage', Source, {
						template = '<div class="chat-message emergency">[SYSTEM]: Player not found</div>',
						args = { GetPlayerName(tonumber(USER)), tostring(GROUP) }
					})
				else
						...
						TriggerEvent("es:setPlayerData", USER, "permission_level", GROUP, function(response, success)
							if(true)then
								TriggerClientEvent('chat:addMessage', USER, {
									template = '<div class="chat-message emergency">[SYSTEM]: Permission level has been set to : {0}</div>',
									args = { tostring(GROUP) }
								})
							end
						end)
						TriggerClientEvent('chat:addMessage', Source, {
							template = '<div class="chat-message emergency">[SYSTEM]: {0} has been set to {1}</div>',
							args = { GetPlayerName(tonumber(USER)), tostring(GROUP) }
						})
					else
						TriggerClientEvent('chat:addMessage', Source, {
							template = '<div class="chat-message emergency">[SYSTEM]: Invalid Input</div>',
							args = { GetPlayerName(tonumber(USER)), tostring(GROUP) }
						})
					end
				end
		...
		if (player and GetPlayerName(player)) then
			if level then
				TriggerEvent("es:setPlayerData", tonumber(args[1]), "permission_level", tonumber(args[2]), function(response, success)
					...
					TriggerClientEvent('chat:addMessage', source, {
						template = '<div class="chat-message emergency">[SYSTEM]: {0} has been set to {1}</div>',
						args = { GetPlayerName(tonumber(player)), args[2] }
					})
				end)
			else
				...
				if(groups[args[2]])then
					TriggerEvent("es:getPlayerFromId", player, function(user)
						...
						TriggerEvent("es:setPlayerData", player, "group", args[2], function(response, success)
							...
							TriggerClientEvent('chat:addMessage', player, {
								template = '<div class="chat-message emergency">[SYSTEM]: Group has been set to : {0}</div>',
								args = { group }
							})
							TriggerClientEvent('chat:addMessage', source, {
								template = '<div class="chat-message emergency">[SYSTEM]: {0} group has been set to {1}</div>',
								args = { GetPlayerName(tonumber(player)), args[2] }
							})
...
TriggerEvent('es:addCommand', 'report', function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, {
		template = '<div class="chat-message system"> REPORT : {0} </div>',
		args = {" (^2" .. GetPlayerName(source) .. " | " .. source .. "^0) " .. table.concat(args, " ")}
	})
...
TriggerEvent('es:addCommand', 'achat', function(source, args, user)
	local msg = " (^3" .. GetPlayerName(source) .." | "..source.."^0) " .. table.concat(args, " ")
	TriggerClientEvent('chat:addMessage', source, {
		template = '<div class="chat-message success"> [ADMIN] | {0} </div>',
		args = {msg}
	})
	TriggerEvent("es:getPlayers", function(pl)
		for k,v in pairs(pl) do
...
TriggerEvent("es:addGroupCommand", 'dm', "mod", function(source, args, user)
	...
	if targ ~= nil then
		if targ > 0 then
			...
			if GetPlayerName(targ) ~= nil then
				TriggerClientEvent('chat:addMessage', targ, {
					template = '<div class="chat-message system"> [^1ADMIN^0] | {0} </div>',
					args = {msg}
				});
				...
				TriggerEvent("es:getPlayers", function(pl)
					for k,v in pairs(pl) do
						TriggerEvent("es:getPlayerFromId", k, function(user)
							if(user.getPermissions() >= 5 --[[and k ~= source]]) then
								local msg = "["..GetPlayerName(source).."] | (^2" .. GetPlayerName(targ) .." | "..targ.."^0) " .. msg
								TriggerClientEvent('chat:addMessage', k, {
...
TriggerEvent('es:addGroupCommand', 'announce', "admin", function(source, args, user)
	...
	TriggerClientEvent('chat:addMessages', -1, {
		template = '<div class="chat-message server">ANNOUNCEMENT: {0}</div>',
		args = {msg}
	})
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = {"^1SYSTEM", "Insufficienct permissions!"} })
