...
AddEventHandler('tac:removeInventoryItem', function(type, itemName, itemCount)
	...
	if type == 'item_standard' then
		...
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > xItem.count or xItem.count < 1) then
				...
				TriggerClientEvent('tac:showNotification', _source, _U('threw_standard', itemCount, xItem.label))
			end
		end
	...
	elseif type == 'item_money' then
		...
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > playerCash or playerCash < 1) then
				...
				local pickupLabel = ('~y~%s~s~ [~r~%s~s~]'):format(_U('cash'), _U('locale_currency', ESX.Math.GroupDigits(itemCount)))
				ESX.CreatePickup('item_money', 'money', itemCount, pickupLabel, _source)
				TriggerClientEvent('tac:showNotification', _source, _U('threw_money', ESX.Math.GroupDigits(itemCount)))
			end
		end
	...
	elseif type == 'item_account' then
		...
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > account.money or account.money < 1) then
				...
				local pickupLabel = ('~y~%s~s~ [~r~%s~s~]'):format(account.label, _U('locale_currency', ESX.Math.GroupDigits(itemCount)))
				ESX.CreatePickup('item_account', itemName, itemCount, pickupLabel, _source)
				TriggerClientEvent('tac:showNotification', _source, _U('threw_account', ESX.Math.GroupDigits(itemCount), string.lower(account.label)))
			end
		end
	...
	elseif type == 'item_weapon' then
		...
		if xPlayer.hasWeapon(itemName) then
			...
			if itemCount > 0 then
				TriggerClientEvent('tac:pickupWeapon', _source, weaponPickup, itemName, itemCount)
				TriggerClientEvent('tac:showNotification', _source, _U('threw_weapon_ammo', weaponLabel, itemCount))
			else
				-- workaround for CreateAmbientPickup() giving 30 rounds of ammo when you drop the weapon with 0 ammo
				TriggerClientEvent('tac:pickupWeapon', _source, weaponPickup, itemName, 1)
				TriggerClientEvent('tac:showNotification', _source, _U('threw_weapon', weaponLabel))
			end
		end
...
AddEventHandler('tac:useItem', function(itemName)
	...
	if count > 0 then
		...
		TriggerClientEvent('tac:showNotification', xPlayer.source, _U('act_imp'))
	end
end)
...
AddEventHandler('tac:onPickup', function(id)
	...
	if pickup.type == 'item_standard' then
		...
		if remaining > 0 then
			TriggerClientEvent('tac:showNotification', _source, _U('cannot_pickup_room', item.label))
...
ESX.RegisterServerCallback('tac:getPlayerData', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('tac:getOtherPlayerData', function(source, cb, target)
