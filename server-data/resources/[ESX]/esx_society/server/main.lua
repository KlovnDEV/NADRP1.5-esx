	...
	if xPlayer.job.name == society.name then
		TriggerEvent('tac_addonaccount:getSharedAccount', society.account, function(account)
			if amount > 0 and account.money >= amount then
				...
				TriggerClientEvent('tac:showNotification', _source, _U('have_withdrawn', ESX.Math.GroupDigits(amount)))
			else
...
AddEventHandler('tac_society:depositMoney', function(society, amount)
	...
	if xPlayer.job.name == society.name then
		if amount > 0 and xPlayer.getMoney() >= amount then
			...
			TriggerClientEvent('tac:showNotification', _source, _U('have_deposited', ESX.Math.GroupDigits(amount)))
		else
			TriggerClientEvent('tac:showNotification', _source, _U('invalid_amount'))
		end
	else
...
ESX.RegisterServerCallback('tac_society:getSocietyMoney', function(source, cb, societyName)
	local society = GetSociety(societyName)
...
ESX.RegisterServerCallback('tac_society:getEmployees', function(source, cb, society)
...
ESX.RegisterServerCallback('tac_society:getJob', function(source, cb, society)
...
ESX.RegisterServerCallback('tac_society:setJob', function(source, cb, identifier, job, grade, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if isBoss then
		...
		if xTarget then
			...
			if type == 'hire' then
				TriggerClientEvent('tac:showNotification', _source, _U('you_have_been_hired', job))
			elseif type == 'promote' then
				TriggerClientEvent('tac:showNotification', _source, _U('you_have_been_promoted'))
			elseif type == 'fire' then
				TriggerClientEvent('tac:showNotification', _source, _U('you_have_been_fired', xTarget.getJob().label))
			end
...
ESX.RegisterServerCallback('tac_society:setJobSalary', function(source, cb, job, grade, salary)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('tac_society:getOnlinePlayers', function(source, cb)
	local xPlayers = ESX.GetPlayers()
	local players  = {}
...
ESX.RegisterServerCallback('tac_society:getVehiclesInGarage', function(source, cb, societyName)
	local society = GetSociety(societyName)
...
ESX.RegisterServerCallback('tac_society:isBoss', function(source, cb, job)
...
function WashMoneyCRON(d, h, m)
	MySQL.Async.fetchAll('SELECT * FROM society_moneywash', {}, function(result)
		for i=1, #result, 1 do
			...
			if xPlayer then
				TriggerClientEvent('tac:showNotification', _source, _U('you_have_laundered', ESX.Math.GroupDigits(result[i].amount)))
			end
