...
RegisterCommand('breach', function(source, args, raw)
        ...
        for id,v in pairs(robbableHouses) do
            if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 2.5 then
                ...
                if (finished == 100) then
                    isLockpicking = false
                    pedSpawned = false
                    newHouseCoords = {x = robbableHouses[id].x, y = robbableHouses[id].y, z = robbableHouses[id].z}
                    if math.random(1,4) == 1 then
                        TriggerEvent('tac-dispatch:houserobbery', newHouseCoords)
                    end
                    ...
                    TriggerEvent('status:setState', 26, 600)
                    TriggerEvent('houseRobberies:createHouse', id)   
                end
            end
        end
...
AddEventHandler('houseRobberies:createHouse', function(id)
    ...
    while isRobbing do
        ...
        if GetDistanceBetweenCoords(playerCoords, house.x+3.6, house.y-15, house.z-50, true) < 2.5 then
            showMessage('Press ~r~E~w~ To Exit House')
        ...
        if isRobbing then
            for i=1,#myRobbableItems do
                if (GetDistanceBetweenCoords(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], GetEntityCoords(GetPlayerPed(-1))) < 1.4) and not myRobbableItems[i]['isSearched'] then
                    DrawText3Ds(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], '~w~Press ~r~H~s~ To Search ' .. myRobbableItems[i]["name"])
                    local searchCoords = { x = generator.x + myRobbableItems[i]["x"], y = generator.y + myRobbableItems[i]["y"], z = generator.z + myRobbableItems[i]["z"]}
            ...
            if disturbance > 85 then
                if not calledin then
                    ...
                    if fuckup == 2 and GetEntitySpeed(GetPlayerPed(-1)) > 0.8 then
                        calledin = true
                        if not isAgro and isRobbing then
                            agroNPC()
                            TriggerEvent('tac-dispatch:houserobbery', newHouseCoords)
                        end
                    end
...
RegisterNetEvent('houseRobberies:searchAnim')
AddEventHandler('houseRobberies:searchAnim', function()
