...
function CheckDamage(ped, bone, weapon)
    ...
    if Config.Bones[bone] ~= nil then
        if not BodyParts[Config.Bones[bone]].isDamaged then
            ...
            TriggerServerEvent('mythic_hospital2:server:SyncInjuries', {
                limbs = BodyParts,
...
RegisterNetEvent('mythic_hospital2:client:SyncBleed')
AddEventHandler('mythic_hospital2:client:SyncBleed', function(bleedStatus)
    isBleeding = tonumber(bleedStatus)
    DoBleedAlert()
end)

RegisterNetEvent('mythic_hospital2:client:FieldTreatLimbs')
AddEventHandler('mythic_hospital2:client:FieldTreatLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 1
    end
...
RegisterNetEvent('mythic_hospital2:client:FieldTreatBleed')
AddEventHandler('mythic_hospital2:client:FieldTreatBleed', function()
    if isBleeding > 0 then
...
RegisterNetEvent('mythic_hospital2:client:ReduceBleed')
AddEventHandler('mythic_hospital2:client:ReduceBleed', function()
    if isBleeding > 0 then
        isBleeding = tonumber(isBleeding) - 1
...
RegisterNetEvent('mythic_hospital2:client:ResetLimbs')
AddEventHandler('mythic_hospital2:client:ResetLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 0
    end
...
RegisterNetEvent('mythic_hospital2:client:RemoveBleed')
AddEventHandler('mythic_hospital2:client:RemoveBleed', function()
    isBleeding = 0
...
RegisterNetEvent('mythic_hospital2:client:UsePainKiller')
AddEventHandler('mythic_hospital2:client:UsePainKiller', function(tier)
    if tier < 4 then
    ...
    while healing do
        ...
        if health >= 200 or timer == 20 then
            healing = false
            return
        end
    end
...
RegisterNetEvent('mythic_hospital2:client:ifak')
AddEventHandler('mythic_hospital2:client:ifak', function()
    local tTimer = 0
    healing = true
    while healing do
        tTimer = tTimer + 1
        ...
        if health >= 200 or tTimer == 20 then
            healing = false
            return
        end
    end
...
RegisterNetEvent('mythic_hospital2:client:UseAdrenaline')
AddEventHandler('mythic_hospital2:client:UseAdrenaline', function(tier)
    if tier < 4 then
        onDrugs = 180 * tier
    end
