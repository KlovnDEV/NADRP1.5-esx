	...
	ESX.TriggerServerCallback("esx_atmRobbery:isRobbingPossible", function(cooldownATM)
		if not cooldownATM and not IsPedInAnyVehicle(PlayerPedId(), false) then
			ESX.TriggerServerCallback('esx_atmRobbery:getOnlinePoliceCount', function(policeCount)
				if policeCount then
					local lol = 1
					local hasItem = exports['nadrp-inventory']:hasEnoughOfItem("usbdevice", 1, false)
					if hasItem then
						...
						if luck ~= 3 then
							...
							exports["nadrp-taskbar"]:taskBar(20000,"Connecting Device",false,false)
...
function hackingEvent(success)
	...
	if success then
		local finished = exports["nadrp-skillbar"]:taskBar(math.random(1000,1500),math.random(15,20))
