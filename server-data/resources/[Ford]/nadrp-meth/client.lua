...
AddEventHandler('esx_methcar:stop', function()
	...
	exports['mythic_notify']:DoHudText('error', 'Production Stopped')
...
AddEventHandler('esx_methcar:startprod', function()
	--DisplayHelpText("~r~Starting production")
	exports['mythic_notify']:DoHudText('success', 'Starting production')
	started = true
	FreezeEntityPosition(CurrentVehicle,true)
	displayed = false
	print('Started Meth production')
	exports['mythic_notify']:DoHudText('success', 'Meth production started!')
	-- Add Cop here  TriggerServerEvent('Trackables:Notify','Suspicious smoking vehicle reported near Sandy Shores.',GetEntityCoords(GetPlayerPed(-1)),'police')
	TriggerEvent('civilian:alertPolice', 100.0, 'meth', 0,0,0)
...
AddEventHandler('esx_methcar:smoke', function(posx, posy, posz, bool)
	if bool == 'a' then
		...
		SetPtfxAssetNextCall("core")
		local smoke = StartParticleFxLoopedAtCoord("exp_grd_flare", posx, posy, posz + 1.7, 0.0, 0.0, 0.0, 1.0, false, false, false, false)
				...
				if GetPedInVehicleSeat(car, -1) == playerPed then
					if started == false then
						if displayed == false then
							...
							exports['mythic_notify']:DoHudText('inform', 'Press G to start making drugs!')
							displayed = true
						end
					end
					if IsControlJustReleased(0, Keys['G']) and exports['nadrp-inventory']:hasEnoughOfItem('carbattery', 2) and exports['nadrp-inventory']:hasEnoughOfItem('acetone' , 5)  then
						if pos.y >= 3500 then
							if IsVehicleSeatFree(CurrentVehicle, 3) then
								TriggerEvent('esx_methcar:startprod')
								TriggerServerEvent('esx_methcar:make')
								...
								exports['mythic_notify']:DoHudText('error', 'The vehicle is already occupied')
							end
						else
							--ESX.ShowNotification('~r~You are too close to the city, head further up north to begin meth production')
							exports['mythic_notify']:DoHudText('inform', 'You are too close to the city, head further up north to begin meth production')
						end
					end
				end
			end
		...
		if started == true then
			if progress < 96 then
				...
				if not pause and IsPedInAnyVehicle(playerPed) then
					progress = progress +  1
					--exports['mythic_notify']:DoHudText('error', 'Meth production '.. progress .. '%')
					exports['mythic_notify']:DoHudText('error', 'Meth production '.. progress .. '%')
					Citizen.Wait(6000)
				end
				...
				if progress > 22 and progress < 24 then
					...
					if selection == 0 then
						...
						ESX.ShowNotification('~o~S. Leave it be ')
						ESX.ShowNotification('~o~D. Replace it')
						ESX.ShowNotification('~c~Select which option you want to do')
					end
					if selection == 1 then
						print("Slected 1")
				...
				if progress > 30 and progress < 32 then
					...
					if selection == 0 then
						...
						ESX.ShowNotification('~o~S. Leave it be')
						ESX.ShowNotification('~o~D. Put on a mask with airfilter')
						ESX.ShowNotification('~c~Select which option you want to do')
					end
					if selection == 1 then
						print("Slected 1")
				...
				if progress > 38 and progress < 40 then
					...
					if selection == 0 then
						...
						ESX.ShowNotification('~o~S. Raise the temperature')
						ESX.ShowNotification('~o~D. Lower the pressure')
						ESX.ShowNotification('~c~Select which option you want to do')
					end
					if selection == 1 then
						print("Slected 1")
				...
				if progress > 41 and progress < 43 then
					...
					if selection == 0 then
						...
						ESX.ShowNotification('~o~S. Try to sucking it out using syringe')
						ESX.ShowNotification('~o~D. Add more lithium to balance it out')
						ESX.ShowNotification('~c~Select which option you want to do')
					end
					if selection == 1 then
						print("Slected 1")
				...
				if progress > 46 and progress < 49 then
					...
					if selection == 0 then
						...
						ESX.ShowNotification('~o~S. Put it away')
						ESX.ShowNotification('~o~D. Drink it')
						ESX.ShowNotification('~c~Select which option you want to do')
					end
					if selection == 1 then
						print("Slected 1")
				...
				if progress > 55 and progress < 58 then
					...
					if selection == 0 then
						...
						ESX.ShowNotification('~o~S. Replace the filter')
						ESX.ShowNotification('~o~D. Clean it using a tooth brush')
						ESX.ShowNotification('~c~Select which option you want to do')
					end
					if selection == 1 then
						print("Slected 1")
				...
				if progress > 82 and progress < 84 then
					...
					if selection == 0 then
						...
						ESX.ShowNotification('~o~S. Go outside and take a shit')
						ESX.ShowNotification('~o~D. Shit inside')
						ESX.ShowNotification('~c~Select which option you want to do')
					end
					if selection == 1 then
						print("Slected 1")
				...
				if progress > 88 and progress < 90 then
					...
					if selection == 0 then
						...
						ESX.ShowNotification('~o~S. No')
						ESX.ShowNotification('~o~D. What if I add meth to glass instead?')
						ESX.ShowNotification('~c~Select which option you want to do')
					end
					if selection == 1 then
						print("Slected 1")
				...
				progress = 100
				exports['mythic_notify']:DoHudText('error', 'Meth production '.. progress .. '%')
				ESX.ShowNotification('~r~~h~Production finished')
	...
	while true do
		if pause == true then
			...
			if IsControlJustPressed(1, 34) then
				selection = 1
				ESX.ShowNotification('~r~Selected option number 1')
			end
			if IsControlJustPressed(1, 8) then
				selection = 2
				ESX.ShowNotification('~r~Selected option number 2')
			end
			if IsControlJustPressed(1, 9) then
				selection = 3
				ESX.ShowNotification('~r~Selected option number 3')
			end
		else
			Citizen.Wait(1000)
		end
