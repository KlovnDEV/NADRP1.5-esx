...
function UpdateBlip()
    Citizen.CreateThread(function()
        ...
        SetBlipSprite(HotdogBlip, 495)
        SetBlipDisplay(HotdogBlip, 4)
        SetBlipScale(HotdogBlip, 0.8)
            ...
            if distance < 10 then
                ...
                if not IsWorking then
                    if distance < OffsetData.Distance then
                        DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "[E] Start work")
                        if IsControlJustPressed(0, Keys["E"]) then
                            StartWorking()
                        end
                    elseif distance < 3 then
                        DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "Start work")
                    end
                else
                    if distance < OffsetData.Distance then
                        DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "[E] Stop work")
                        if IsControlJustPressed(0, Keys["E"]) then
                            StopWorking()
                        end
                    elseif distance < 3 then
                        DrawText3Ds(v.coords.x, v.coords.y, v.coords.z, "Stop work")
                    end
                end
            end
        end
                ...
                if ClosestObject ~= nil and ClosestObject == StandObject then
                    ...
                    if ObjectDistance < 1.0 then
                        if not IsPushing then
                            DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '[E] Grab a hotdog stand')
                            if IsControlJustPressed(0, Keys["E"]) then
                                TakeHotdogStand()
                            end
                        else
                            DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '[E] Release the stand')
                            if IsControlJustPressed(0, Keys["E"]) then
                                LetKraamLose()
                            end
                        end
                    elseif ObjectDistance < 3.0 then
                        DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, 'Hotdog stand')
                    end
                end
            else
                break
                ...
                if ClosestObject ~= nil and ClosestObject == StandObject then
                    ...
                    if ObjectDistance < 1.0 then
                        if SellingData.Enabled then
                            DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '~y~[H] ~w~Prepare a hotdog [Selling: ~r~ON~w~]')
                        else
                            DrawText3Ds(ObjectOffset.x, ObjectOffset.y, ObjectOffset.z, '~y~[H] ~w~Prepare a hotdog [Selling: ~r~OFF~w~]')
                        end
...
RegisterCommand('sellhotdogs', function()
        
        TriggerEvent('qb-hotdogjob:client:ToggleSell')
        ...
        if PlayerDist < 7.5 then
            if SellingData.Hotdog then
                if HotdogsForSale == 0 and SellingPrice == 0 then
                    ...
                    if SellingData.Hotdog and not PreparingFood then
