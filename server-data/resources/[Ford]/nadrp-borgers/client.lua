...
function Fries()
    local itemCount = exports["nadrp-inventory"]:getQuantity("frozenfries")
    if exports["nadrp-inventory"]:hasEnoughOfItem("frozenfries",1,false) and isBsWhitelist() then
        isCooking = true
        local player = PlayerPedId()
        ...
        if finished ~= 100 then
            ...
            local finished2 = exports["nadrp-skillbar"]:taskBar(4750,math.random(5,15))
            if finished2 ~= 100 then
                isCooking = false
                ClearPedTasksImmediately(player)
                exports['mythic_notify']:SendAlert('error', 'Be a better Chef 4Head')
            else
                local finished3 = exports["nadrp-skillbar"]:taskBar(4500,math.random(5,15))
...
function MeatBurger()
        ...
        local itemCount = exports["nadrp-inventory"]:getQuantity("patty")
        if exports["nadrp-inventory"]:hasEnoughOfItem("patty",1,false) then
            isCooking = true
            local player = PlayerPedId()
            TaskStartScenarioInPlace(player, 'PROP_HUMAN_BBQ', 0, true)
            local finished = exports["nadrp-skillbar"]:taskBar(5000,math.random(5,15))
            if finished ~= 100 then
                ...
                if finished2 ~= 100 then
                    ...
                    -- local finished = exports['nadrp-taskbar']:taskBar(10000, 'Cooking')
                    -- if finished ==  100 then
...
-- function Package()
--     local itemCount = exports["nadrp-inventory"]:getQuantity("frozenfries")
--     if exports["nadrp-inventory"]:hasEnoughOfItem("frozenfries",1,false) then
--         TriggerEvent('inventory:removeItem',"frozenfries", 1) 
--         TriggerEvent("player:receiveItem","fries",1)
--         ClearPedTasksImmediately(player)
--     end
