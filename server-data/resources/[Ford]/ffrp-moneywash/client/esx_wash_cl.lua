...
function CleanCash()
	for i=1, #Cleanables do
		if exports["ffrp-inventory"]:hasEnoughOfItem(Cleanables[i].item,1,false) then
			local itemCount = exports["ffrp-inventory"]:getQuantity(Cleanables[i].item)
			if itemCount > 0 and not cleaning then
				...
				exports['t0sic_loadingbar']:StartDelayedFunction('Washing money', 18000, function()
					if exports["ffrp-inventory"]:hasEnoughOfItem(Cleanables[i].item,itemCount,false) then
