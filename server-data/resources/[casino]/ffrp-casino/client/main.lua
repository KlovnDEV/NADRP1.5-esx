...
mainDesk:onPointInOut(PolyZone.getPlayerPosition, function(isPointInside, point)
    if isPointInside then
        NearDesk = true
        KeyPressThread()
        exports["ffrp-notify"]:ShowPersistAlert('[E] - Buy/Sell Chips', 'info', 'show')
    else
        NearDesk = false
        exports["ffrp-notify"]:ShowPersistAlert('[E] - Buy/Sell Chips', 'info', 'hide')
    end
end)
...
RegisterNUICallback('buyChips', function(data, cb)
    TriggerServerEvent('ffrp-casino:buyChips', tonumber(data.amount))
    cb('ok')
end)

RegisterNUICallback('sellChips', function(data, cb)
    TriggerServerEvent('ffrp-casino:sellChips', tonumber(data.amount))
    cb('ok')
end)
