...
function CreatePeds()
	...
	for i,v in pairs(customTables) do
		...
		SetObjectTextureVariant(tableObj, v.color or 0)
	...
	for i,v in pairs(tables) do
		...
		local dealer = CreatePed(4, model,v.pedCoords.x, v.pedCoords.y, v.pedCoords.z, v.pedCoords.w, false, true)
...
AddEventHandler("BLACKJACK:RequestBets", function(index, _timeLeft)
	...
	Citizen.CreateThread(function()
		...
		EndScaleformMovieMethod()
		selectedBet = tables[scrollerIndex].minBet
		while true do
			Wait(0)
			local tableLimit = (tables[scrollerIndex].maxBet)
			local tableMin = (tables[scrollerIndex].minBet)
			if IsControlJustPressed(1, 205) then -- Q / Y
				selectedBet = tables[scrollerIndex].maxBet
			...
			--print(selectedBet, tableLimit, bettingNums[selectedBet])
			...
			if IsControlJustPressed(1, 201) then -- ENTER / A
				...
				if canBet then
					...
					exports['ffrp-notify']:ShowAlert('You dont have enough chips', 'error', 2500)
				end
			end
		end
		...
		while true do Wait(0)	
			...
			if IsControlJustPressed(1, 192) and #hand == 2 and #splitHand == 0 then
				...
				if canBet then
					...
					exports['mythic_notify']:DoHudText('inform', 'You don\'t have enough money to double down')
				end
			end
			if IsControlJustPressed(1, 209) and CanSplitHand(hand) == true then
				...
				if canBet then
					...
					exports['mythic_notify']:DoHudText('inform', 'You don\'t have enough money to split')
	...
	while true do
		...
		if not IsEntityDead(playerPed) then
			for i,v in pairs(tables) do
				...
				if #(vector3(cord.x, cord.y, cord.z) - GetEntityCoords(playerPed)) < 2.0 then
					local tableCoords = GetEntityCoords(tableObj)
					
					if tableCoords == vector3(0.0, 0.0, 0.0) then
					...
					if tableCoords ~= vector3(0.0, 0.0, 0.0) then
						...
						dist = GetDistanceBetweenCoords(coords, GetEntityCoords(playerPed), true)
						...
						if #(coords - GetEntityCoords(playerPed)) < 1.5 then
							found = true
							if IsControlJustPressed(1, 51) then
								if not IsSeatOccupied(coords, 0.5) and canSit then
									playingBlackjack = true
									if satDownCallback ~= nil then
										satDownCallback()
									end

									if showingAlert then 
										ToggleAlert(false)
									end
...
function ToggleAlert(toggle)
	local tableMin,tableMax = GetTableLimit()
	if nearTable ~= false then
		if toggle == true then
			local min = bettingNums[tableMin]
			local max = bettingNums[tableMax]
			showingAlert = true
			if min >= 5000 then
				exports["ffrp-notify"]:ShowPersistAlert('[E] - $'..min..' - $'..max..'', 'error', 'show')
			else
				exports["ffrp-notify"]:ShowPersistAlert('[E] - $'..min..' - $'..max..'', 'success', 'show')
			end
		elseif toggle == false then
			showingAlert = false
			exports["ffrp-notify"]:ShowPersistAlert('', '', 'hide')
		end
	end
end
...
function GetTableLimit()
	...
	for i=1, #tables do
		if #(vector3(coords.x,coords.y, coords.z) - vector3(tables[i].coords[1],tables[i].coords[2],tables[i].coords[3])) < 3.0 then 
			local minBet = tables[i].minBet
			local maxBet = tables[i].maxBet
...
function CheckProps()
	for k,v in pairs(tables) do
		...
		local tableObj = GetClosestObjectOfType(v.coords[1],v.coords[2],v.coords[3], 2.0, `vw_prop_casino_blckjack_01`, false, false, false)
		local tableCoords = GetEntityCoords(tableObj)
		if tableCoords == vector3(0.0, 0.0, 0.0) then
			tableObj = GetClosestObjectOfType(v.coords[1],v.coords[2],v.coords[3], 2.0, `vw_prop_casino_blckjack_01b`, false, false, false)
			tableCoords = GetEntityCoords(tableObj)
		end
		SetObjectTextureVariant(tableObj, v.color)

		-- Remove Three Card Tables
		local prop = "vw_prop_casino_3cardpoker_01"
		local ThreeCardObj = GetClosestObjectOfType(v.coords[1],v.coords[2],v.coords[3], 1.0, `vw_prop_casino_3cardpoker_01`, false, false, false)
		if ThreeCardObj == 0 then
			prop = "vw_prop_casino_3cardpoker_01b"
			ThreeCardObj = GetClosestObjectOfType(v.coords[1],v.coords[2],v.coords[3], 1.0, `vw_prop_casino_3cardpoker_01b`, false, false, false)
			if ThreeCardObj ~= 0 then
				SwapModel(ThreeCardObj, prop, v.color)
			end
		end
		if ThreeCardObj ~= 0 then
			SwapModel(ThreeCardObj, prop, v.color)
...
function SwapModel(passedObj, passedProp, color)
	print(passedObj, passedProp, color)
	local defualtProp = GetHashKey(passedProp)
	if passedProp == "vw_prop_casino_3cardpoker_01b" then
		swapProp = GetHashKey("vw_prop_casino_blckjack_01b")
	elseif passedProp == "vw_prop_casino_3cardpoker_01" then 
		swapProp = GetHashKey("vw_prop_casino_blckjack_01")
	end

	if not HasModelLoaded(swapProp) then
		RequestModel(swapProp)
		while (not HasModelLoaded(swapProp)) do
			Wait(1)
		end
	end

	local obj = CreateModelSwap(GetEntityCoords(passedObj), 1.0, defualtProp, swapProp, false)
	SetObjectTextureVariant(passedObj, color)
end
