ESX = nil
local closestDoor, closestV, closestDistance, playerPed, playerCoords, doorCount
...
local lastDoorstate, isDrawing
function ShowDoorAlert(doorState, update)
	if lastDoorstate == doorState then
		return
	end
	local isAuthorized = IsAuthorized(closestV)
				...
				if not closestV.doors then
					...
					if not closestV.locked then
						if Config.ShowUnlockedText then
							ShowDoorAlert("unlocked", true)
						else
							if isDrawing then
								ShowDoorAlert("hide", true)
								isDrawing = false 
							end
						end
					else
						ShowDoorAlert("locked", true)
					end
				else
					local door = {}
...
function IsAuthorized(doorID)
	...
	for _,authJob in pairs(doorID.authorizedJobs) do
		if job == authJob then
			return true
		end
	end
...
AddEventHandler('nui_doorlock:newDoorSetup', function(args)
	if not args[1] then 
		print('/newdoor [doortype] [locked] [jobs]\nDoortypes: door, sliding, garage, double, doublesliding\nLocked: true or false\nJobs: Up to four can be added with the command') 
		return 
	end
