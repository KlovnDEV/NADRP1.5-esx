...
AddEventHandler('esx:playerLoaded', function()
    Wait(1000)
	ESX.TriggerServerCallback("ak4y-dailyTasks:playTimeCheck", function(result)
        if result then 
...
RegisterNUICallback('selectTask', function(data, cb)
	ESX.TriggerServerCallback("ak4y-dailyTasks:selectTask", function(result)
        if result then 
...
RegisterNUICallback('cancelTask', function(data, cb)
	ESX.TriggerServerCallback("ak4y-dailyTasks:cancelTask", function(result)
        if result then 
            mySelectedTask = nil
            done = false
            playTimeMin = nil
...
RegisterNUICallback('getReward', function(data, cb)
	ESX.TriggerServerCallback("ak4y-dailyTasks:getReward", function(result)
        if result then 
            mySelectedTask = nil
            done = false
            playTimeMin = nil
