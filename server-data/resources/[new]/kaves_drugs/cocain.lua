...
AddEventHandler("kaves_drugs:gatheringCoca", function(coord, heading, rotx, roty, rotz)
	...
	while gathering do
		...
		if dst < 1.5 then
			if IsControlJustPressed(0, 38) and gathering then
                gathering = false
                TriggerEvent("player:receiveItem","coca", Config.Coca.gatheringZone.count)
			end
		end
		Citizen.Wait(1)
	end
    ...
    while true do
        ...
        if dst < 5 and not packaged then
            ...
            if dst < 2 then
                ...
                if IsControlJustPressed(0, 38) then
                    --if exports["shrp-inventory"]:hasEnoughOfItem("coca", Config.Coca.gatheringZone.count,true) then
                    --   packaged = true
                        TriggerEvent("inventory:removeItem", "coca", Config.Coca.gatheringZone.count)
                        TriggerEvent("kaves_drugs:packageCoca")
                    --else
                    --    ESX.ShowNotification("You don't have a coca")
                    --end
                --end
            end
        end
...
AddEventHandler("kaves_drugs:packageCoca", function()
    ...
    while packaged do
        ...
        if dst < 1.5 then
            if IsControlJustPressed(0, 38) then
                TriggerEvent("player:receiveItem","packagedcoca", Config.Coca.packageZone.count)
                packaged = false
            end
        end
        Citizen.Wait(1)
