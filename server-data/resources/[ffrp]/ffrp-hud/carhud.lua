        ...
        if vehModel ~= nil and vehModel ~= 0 then
            if DoesEntityExist(currVeh) then
                if IsThisModelABoat(vehModel) or IsThisModelAJetski(vehModel) or IsThisModelAnAmphibiousCar(vehModel) or IsThisModelAnAmphibiousQuadbike(vehModel) then
                	local finished = exports["ffrp-taskbar"]:taskBar(2000,"Toggling Anchor")
...
function attachProp2(attachModelSent,boneNumberSent,x,y,z,xR,yR,zR)
	...
	exports["ffrp-ped"]:GlobalObject(attachedProp2)
...
function attachProp(attachModelSent,boneNumberSent,x,y,z,xR,yR,zR)
	...
	exports["ffrp-ped"]:GlobalObject(attachedProp)
...
RegisterNetEvent("ffrp-admin:currentDevmode")
AddEventHandler("ffrp-admin:currentDevmode", function(devmode)
    currentValues["dev"] = devmode
end)

RegisterNetEvent("ffrp-admin:currentDebug")
AddEventHandler("ffrp-admin:currentDebug", function(debugToggle)
...
AddEventHandler('client:useCig', function()
	...
	exports["ffrp-taskbar"]:taskBar(15000, "Smoking Cigarette")
...
AddEventHandler("fire:damageUser", function(Reqeuester)
	...
	if IsPedShooting(Attackerped) then
        ...
        if name == `WEAPON_FIREEXTINGUISHER` and not exports["ffrp-ped"]:isPed("dead") then
		...
		if not IsPedSwimmingUnderWater( PlayerPedId() ) and currentValues["oxy"] < 25.0 then
			if GetGameTimer() - lastDamageTrigger > 3000 then
				...
				if currentValues["oxy"] <= 0 then
					...
					if exports["ffrp-ped"]:isPed("dead") then
...
RegisterNetEvent('ffrp-voice:radioActive')
AddEventHandler('ffrp-voice:radioActive', function(radioStatus)
	currentValues["radio"] = radioStatus
end)

AddEventHandler('ffrp-hud:UpdateVoice', function(voiceMode)
...
AddEventHandler('client:useJoint', function()
	...
	local finished = exports["ffrp-taskbar"]:taskBar(7500, "Smoking Joint")
	if finished == 100 then
