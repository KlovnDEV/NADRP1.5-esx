...
function hasKey(id)
	local myhousekeys = exports["ffrp-ped"]:isPed("myhousekeys")
	...
	if DoesBikeExist then
		if vehicle ~= nil then
			...
			if plate == rentalPlate then 
				HasRental = false
				TriggerServerEvent('ffrp-garages:returnRental')
...
function AttemptRent(bike)
	if not HasRental then
		TriggerServerEvent('ffrp-garages:attemptRent', bike)
		CloseMenu()
	else
...
RegisterNetEvent('ffrp-garages:SpawnRental')
AddEventHandler('ffrp-garages:SpawnRental', function(bike)
	if bike ~= nil then
...
AddEventHandler("onResourceStart", function(res)
	if res == GetCurrentResourceName() then
		TriggerServerEvent("garages:CheckGarageForVeh")
		--TriggerServerEvent('ffrp_garages:resetOutVehicles')
	end
end)
		...
		if DoesEntityExist(caisseo) then
			...
			if state == "Out" and coordlocation == nil then
				...
				ESX.Game.SpawnVehicle(vehProps.model, spawnPos, spawnPos.h, function(veh)
					...
					Citizen.Wait(100)
					TriggerServerEvent('ffrp_garages:modifystate', vehProps, 0, nil)
				end)
...
AddEventHandler('garages:StoreVehicle', function(plates)
	...
	Citizen.CreateThread(function()
		...
		if DoesEntityExist(caissei) then
			...
			TriggerServerEvent('ffrp_garages:modifystate', vehProps, 1, current_used_garage)
		else
...
function deleteVeh(ent)
	...
	local finished = exports["ffrp-taskbar"]:taskBar(1000,"Impounding")
...
AddEventHandler('impoundVehicle', function()
   ...
   -- targetVehicle = getVehicleInDirection(coordA, coordB)
	...
	if not IsThisModelATrain(GetEntityModel(targetVehicle)) then
		ESX.TriggerServerCallback('ffrp_garages:isPlayerOwned', function(owned)
			if owned then
				TriggerServerEvent("ffrp_garages:modifystate",props,"Standard Impound", "Impound Lot")
...
AddEventHandler('fullimpoundVehicle', function()
	...
	if not IsThisModelATrain(GetEntityModel(targetVehicle)) then
		ESX.TriggerServerCallback('ffrp_garages:isPlayerOwned', function(owned)
			if owned then
				TriggerServerEvent("ffrp_garages:modifystate",props,"Police Impound","Impound Lot")
				deleteVeh(targetVehicle)
				print(owned)
			else
				deleteVeh(targetVehicle)
