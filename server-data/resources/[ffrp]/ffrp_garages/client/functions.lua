...
SpawnVehicle = function(data,recuperar)
	...
	ESX.Game.SpawnVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
        ...
        TriggerEvent("ARPF:spawn:recivekeys", yourVehicle, plate)
        Citizen.Wait(100)
        TriggerServerEvent('ffrp_garages:modifystate', vehicleProps, 0, nil)
        HandleCamera(cachedData["currentGarage"])
    end)
    if recuperar then
        exports['mythic_notify']:SendAlert('error', 'You paid $200 to recover your vehicle!')
        TriggerServerEvent('ffrp_garages:pay')
    end
    ClearMenu()
end
...
PutInVehicle = function()
    ...
    if DoesEntityExist(vehicle) then
		...
		ESX.TriggerServerCallback("ffrp_garages:validateVehicle", function(valid)
			if valid then
				AbrirMenuGuardar()
            else
                CloseMenu()
...
SaveInGarage = function(garage)
    ...
    deleteCar(vehicle)
    TriggerServerEvent('ffrp_garages:modifystate', vehicleProps, 1, garage)
    CloseMenu()
end
...
SetVehicleProperties = function(vehicle, vehicleProps)
    ...
    SetVehicleXenonLightsColour(vehicle, vehicleProps["xenoncolour"])
    ...
    DecorSetInt(vehicle, "CurrentFuel", math.floor(tonumber(vehicleProps["fuelLevel"])))
end
...
GetVehicleProperties = function(vehicle)
    if DoesEntityExist(vehicle) then
        ...
        vehicleProps["fuelLevel"] = DecorGetInt(vehicle, "CurrentFuel")
        vehicleProps["xenoncolour"] = GetVehicleXenonLightsColour(vehicle)
        return vehicleProps
    end
end
...
DrawScriptMarker = function(markerData)
	...
	ESX.ShowHelpNotification("~b~E ~w~or ~b~ENTER ~w~Accepts ~b~Arrows ~w~Move ~b~Backspace ~w~Exit")
end
