...
RegisterNetEvent('ffrp-paletorob:useEleckit')
AddEventHandler('ffrp-paletorob:useEleckit', function()
    local ped = PlayerPedId()
            ...
            if IsNearStation then
                for i=1, #powerStations do
                    if #(vector3(powerStations[i].x, powerStations[i].y,powerStations[i].z) - GetEntityCoords(ped)) < 1.5 then
                        if not powerStations[i].disabled then
                            if exports["ffrp-inventory"]:hasEnoughOfItem("electronickit",1,false) then
...
RegisterNetEvent('ffrp-paletorob:useAdvlockpick')
AddEventHandler('ffrp-paletorob:useAdvlockpick', function()
    local ped = PlayerPedId()
    local IsPowerDisabled = IsPowerDisabled()
    ESX.TriggerServerCallback('GetPoliceOnline', function(CopsConnected)
        if CopsConnected >= CopsNeeded then
            for i=1, #doorCoords do
                if #(vector3(doorCoords[i].x, doorCoords[i].y,doorCoords[i].z) - GetEntityCoords(ped)) < 1.5 then
                    if not doorCoords[i].disabled then
                        if IsPowerDisabled then
                            if exports["ffrp-inventory"]:hasEnoughOfItem("advlockpick",1,false) and exports["ffrp-inventory"]:hasEnoughOfItem("heavycutters",1,false) then
                                TriggerEvent('tac-dispatch:PaletoBank')
                                local finished = exports['ffrp-skillbar']:taskBar(3000, math.random(5,15))
                                if finished ~= 100 then
                                    exports['mythic_notify']:SendAlert('error', 'failed')
                                else
                                    local finished2 = exports['ffrp-skillbar']:taskBar(2000, math.random(5,15))
                                    if finished2 ~= 100 then
                                        exports['mythic_notify']:SendAlert('error', 'failed')
                                    else
                                        local finished3 = exports['ffrp-skillbar']:taskBar(1000, math.random(5,15))
                                        if finished3 ~= 100 then
                                            exports['mythic_notify']:SendAlert('error', 'failed')
                                        else
                                            local finished = exports["ffrp-taskbar"]:taskBar(300000, "Unlocking door please wait 🔓")
                                            TriggerEvent('tac-dispatch:PaletoBank')
	                                        if finished == 100 then
                                            exports['mythic_notify']:SendAlert('success', 'Door Unlocked!')
                                            TriggerServerEvent('ffrp-paletorob:updateStates', "doors", i, true)
                                            TriggerServerEvent('ffrp-doors:alterlockstate', doorCoords[i].doorNum, 0)
                                            end
                                        end
                                    end
                                end
...
RegisterNetEvent('ffrp-paletorob:updateStates')
...
RegisterNetEvent('ffrp-paletorob:resetBank')
AddEventHandler('ffrp-paletorob:resetBank', function()
    for i=1, # powerStations do
...
function PowerGridCallback(success)
	...
	if success then
        ...
        TriggerServerEvent("ffrp-paletorob:updateStates", "station", station, true)
	else
...
function LootSpotCallback(success)
	...
	if success then
        ...
        TriggerServerEvent("ffrp-paletorob:updateStates", "loot", lootId, true)
