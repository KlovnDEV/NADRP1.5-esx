...
RegisterNUICallback('joinRadio', function(data, cb)
  ...
  local getPlayerRadioChannel = exports['ffrp-voice']:GetRadioChannel()
  ...
  if tonumber(data.channel) ~= tonumber(getPlayerRadioChannel) then
      if tonumber(data.channel) <= Config.RestrictedChannels then
        if(PlayerData.job.name == 'police' or PlayerData.job.name == 'ambulance' or PlayerData.job.name == 'fire'  or PlayerData.job.name == 'mechanic') then
          ...
          exports['ffrp-voice']:addPlayerToRadio(data.channel)
...
RegisterNUICallback('volumeup', function(data, cb)
  ...
  TriggerEvent('ffrp-voice:SetRadioVolume', radioVolume)
  cb('ok')
end)
...
RegisterNetEvent('ffrp-radio:use')
AddEventHandler('ffrp-radio:use', function()
  enableRadio(true)
end)

RegisterNetEvent('ffrp-radio:onRadioDrop')
AddEventHandler('ffrp-radio:onRadioDrop', function(source)
  local playerName = GetPlayerName(PlayerId())
  local getPlayerRadioChannel = exports['ffrp-voice']:GetRadioChannel()
...
RegisterCommand('radio', function()
  if IsControlEnabled(0, 245) then
    local item = exports["ffrp-inventory"]:hasEnoughOfItem('radio', 1, false)
    local item2 = exports["ffrp-inventory"]:hasEnoughOfItem('radio2', 1, false)
