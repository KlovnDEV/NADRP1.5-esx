    ...
    while true do
        if #(vector3(Config.WashingX, Config.WashingY, Config.WashingZ) - GetEntityCoords(PlayerPedId()))  < 25 and washingActive == false then
            ...
            if #(vector3(Config.WashingX, Config.WashingY, Config.WashingZ) - GetEntityCoords(PlayerPedId())) < 1 then
                ...
                if IsControlJustReleased(1, 51) and exports['ffrp-inventory']:hasEnoughOfItem('stone', 10) and not recent then
                    recent = true
                    local finished = exports['ffrp-taskbar']:taskBar(10000, 'Washing stone')
    ...
    while true do
        ...
        if #(vector3(Config.RemeltingX, Config.RemeltingY, Config.RemeltingZ) - GetEntityCoords(PlayerPedId())) < 25 and remeltingActive == false then
            ...
            if #(vector3(Config.RemeltingX, Config.RemeltingY, Config.RemeltingZ) - GetEntityCoords(PlayerPedId())) < 1 then
                if IsControlJustReleased(1, 51) and exports['ffrp-inventory']:hasEnoughOfItem('washedstone', 10) then
                    local finished = exports['ffrp-taskbar']:taskBar(10000, 'Smelting')
                    if finished == 100 then
                        local cid = exports['ffrp-ped']:isPed('steamid')
    ...
    while true do
		...
		if dist <= 2.5 then
			if IsControlJustPressed(1, 51) then
                local itemCount = exports["ffrp-inventory"]:getQuantity("diamond")
                local cid = exports['ffrp-ped']:isPed('steamid')
...
Citizen.CreateThread(function()
    ...
    while true do
		...
		if dist <= 2.5 then
			if IsControlJustPressed(1, 51) then
                local itemCount = exports["ffrp-inventory"]:getQuantity("gold")
                local cid = exports['ffrp-ped']:isPed('steamid')
                if itemCount > 0 and not trading then
                    trading = true
...
Citizen.CreateThread(function()
    ...
    while true do
		...
		if dist <= 2.5 then
            if IsControlJustPressed(1, 51) then
                local cid = exports['ffrp-ped']:isPed('steamid')
                local itemCount = exports["ffrp-inventory"]:getQuantity("copper")
                if itemCount > 0 and not trading then
                    trading = true
...
Citizen.CreateThread(function()
    ...
    while true do
		...
		if dist <= 2.5 then
			if IsControlJustPressed(1, 51) then
                local itemCount = exports["ffrp-inventory"]:getQuantity("iron")
                local cid = exports['ffrp-ped']:isPed('steamid')
                if itemCount > 0 and not trading then
                    trading = true
