...
RegisterServerEvent('ffrp-paycheck:hasPaycheck')
AddEventHandler('ffrp-paycheck:hasPaycheck', function()
...
RegisterServerEvent('ffrp-paycheck:addPaycheck')
AddEventHandler('ffrp-paycheck:addPaycheck', function(amount)
    local user = ESX.GetPlayerFromId(source)
    local identifier = user.identifier
    MySQL.Async.fetchAll('SELECT payslip FROM users WHERE identifier=@identifier', {
        ["@identifier"] = identifier
