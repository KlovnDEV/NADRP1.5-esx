...
RegisterNetEvent('mdt:refreshCharges')
AddEventHandler('mdt:refreshCharges', function(reports, warrants, officer, job)
    if #warrants == 0 then warrants = false end
    if #reports == 0 then reports = false end
    SendNUIMessage({
        type = "recentReportsAndWarrantsLoaded",
...
RegisterCommand('+mdt', function()
    if job == "police" then
        ...
        elseif IsControlJustPressed(0, 178) or IsDisabledControlJustPressed(0, 178) and not IsPedInAnyPoliceVehicle(playerPed) then
            TriggerServerEvent("mdt:hotKeyOpen")
        end
...
RegisterCommand('-mdt', function() end)
RegisterKeyMapping('+mdt', '(Police): Toggle MDT', 'keyboard', 'DELETE')

Citizen.CreateThread(function()
    TriggerEvent('chat:removeSuggestion', "+mdt")
    TriggerEvent('chat:removeSuggestion', "-mdt")
end)