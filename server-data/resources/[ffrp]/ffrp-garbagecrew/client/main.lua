...
AddEventHandler('tac:playerLoaded', function(xPlayer)
	PlayerData = xPlayer
	TriggerServerEvent('tac_garbagecrew:setconfig')
end)

RegisterNetEvent('tac:setJob')
AddEventHandler('tac:setJob', function(job)
	PlayerData.job = job
	TriggerEvent('tac_garbagecrew:checkjob')
end)

RegisterNetEvent('tac_garbagecrew:movetruckcount')
AddEventHandler('tac_garbagecrew:movetruckcount', function(count)
	Config.TruckPlateNumb = count
end)

RegisterNetEvent('tac_garbagecrew:updatejobs')
AddEventHandler('tac_garbagecrew:updatejobs', function(newjobtable)
	CollectionJobs = newjobtable
end)


RegisterNetEvent('tac_garbagecrew:selectnextjob')
AddEventHandler('tac_garbagecrew:selectnextjob', function()
...
RegisterNetEvent('tac_garbagecrew:enteredarea')
AddEventHandler('tac_garbagecrew:enteredarea', function(zone)
...
RegisterNetEvent('tac_garbagecrew:leftarea')
AddEventHandler('tac_garbagecrew:leftarea', function()
...
AddEventHandler('tac_garbagecrew:checkjob', function()
	if PlayerData.job.name ~= 'garbage' then
...
Citizen.CreateThread( function()
	while true do 
		...
		if IsInArea and not HasAlreadyEnteredArea then
			...
			sleep = 0
			TriggerEvent('tac_garbagecrew:enteredarea', currentZone)
		end

		if not IsInArea and HasAlreadyEnteredArea then
			HasAlreadyEnteredArea = false
			sleep = 1000
			TriggerEvent('tac_garbagecrew:leftarea', currentZone)
		end
...
function CollectBagFromBin(currentZone)
	...
	if DoesEntityExist(worktruck) and GetDistanceBetweenCoords(GetEntityCoords(worktruck), GetEntityCoords(GetPlayerPed(-1)), true) < 25.0 then
		...
		TriggerServerEvent('tac_garbagecrew:bagremoval', currentZone.pos, currentZone.trucknumber) 
		...
		TriggerServerEvent('tac_garbagecrew:unknownlocation', currentZone.pos, currentZone.trucknumber)
	end
end
...
function PlaceBagInTruck(thiszone)
	...
	depositlist = nil
	truckpos = nil
	TriggerServerEvent('tac_garbagecrew:bagdumped', binpos, truckplate)
	HasAlreadyEnteredArea = false
end
...
function SelectBinAndCrew(location)
	...
	if bin ~= 0 then
		...
		TriggerServerEvent('tac_garbagecrew:setworkers', GetEntityCoords(bin), truckplate, truckid )
		truckpos = nil
		albetogetbags = true
		SetBlipRoute(Blips['delivery'], false)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
			...
			if data.current.value == 'citizen_wear' then
				...
				if Config.UseWorkClothing then
					ESX.TriggerServerCallback('tac_skin:getPlayerSkin', function(skin, jobSkin)
						local model = nil
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehiclespawner', {
			...
			ESX.Game.SpawnVehicle(data.current.value, Config.VehicleSpawn.pos, 270.0, function(vehicle)
				...
				if trucknumber <=9 then
					...
					TriggerEvent("ARPF:spawn:recivekeys", vehicle, worktruckplate)
				...
				TriggerServerEvent('tac_garbagecrew:movetruckcount')   
