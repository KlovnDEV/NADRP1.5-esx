...
AddEventHandler('escort:person', function()
    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
    local handcuff = exports['ffrp-ped']:isPed('handcuffed')
    if closestPlayer ~= -1 and closestDistance <= 2.0 then
        if not handcuffed then 
            TriggerServerEvent('ffrp-policejob:drag', GetPlayerServerId(closestPlayer))
        else
...
AddEventHandler('seat:person', function()
    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
    if closestPlayer ~= -1 and closestDistance <= 2.0 then
        TriggerServerEvent('ffrp-policejob:putInVehicle', GetPlayerServerId(closestPlayer))
    else
        exports['mythic_notify']:SendAlert('error', 'No Player Found')
    end
end)
...
AddEventHandler('unseat:person', function()
    --local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
    --if closestPlayer ~= -1 and closestDistance <= 2.0 then
    --    TriggerServerEvent('ffrp-policejob:OutVehicle', GetPlayerServerId(closestPlayer))
    --else
    --    exports['mythic_notify']:SendAlert('error', 'No Player Found')
    --end
...
AddEventHandler('police:cuff', function()
    ...
    if sonic < 5.0 then
        if closestPlayer ~= -1 and closestDistance <= 2.0 then
            ...
            TriggerServerEvent('ffrp-policejob:requesthard', target_id, playerheading, playerCoords, playerlocation)
            Citizen.Wait(1500)
...
RegisterNetEvent('ffrp-policejob:uncuff')
AddEventHandler('ffrp-policejob:uncuff', function()
    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
    local playerheading = GetEntityHeading(GetPlayerPed(-1))
    local playerlocation = GetEntityForwardVector(PlayerPedId())
    local playerCoords = GetEntityCoords(GetPlayerPed(-1))
    ...
    if closestPlayer ~= -1 and closestDistance <= 2.0 then
        ...
        TriggerServerEvent('ffrp-policejob:requestrelease', target_id, playerheading, playerCoords, playerlocation)
        Citizen.Wait(1200)
...
RegisterNetEvent('ffrp-policejob:softcuff')
AddEventHandler('ffrp-policejob:softcuff', function()
    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
    local playerheading = GetEntityHeading(GetPlayerPed(-1))
    local playerlocation = GetEntityForwardVector(PlayerPedId())
    local playerCoords = GetEntityCoords(GetPlayerPed(-1))
    ...
    if sonic < 5.0 then
        if closestPlayer ~= -1 and closestDistance <= 2.0 then
            ...
            TriggerServerEvent('ffrp-policejob:requestarrest', false,target_id,playerheading, playerCoords, playerlocation)
            Citizen.Wait(1500)
            TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 3, 'handcuff', 0.4)
        else
            exports['mythic_notify']:SendAlert('error', 'No Player Found')
...
RegisterNetEvent('ffrp-ems:revive')
AddEventHandler('ffrp-ems:revive', function()
    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
...
RegisterNetEvent('ffrp-ems:heal')
AddEventHandler('ffrp-ems:heal', function()
    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
    ...
    if closestPlayer ~= -1 and closestDistance <= 2.0 then
        ...
        if not IsEntityPlayingAnim(GetPlayerPed(-1), "weapons@first_person@aim_rng@generic@projectile@thermal_charge@", "plant_floor", 3) then
            ...
            TriggerServerEvent('ffrp-death:healplayer', GetPlayerServerId(closestPlayer), 'small')
            Citizen.Wait(2000)
...
RegisterNetEvent('ffrp-ems:bigheal')
AddEventHandler('ffrp-ems:bigheal', function()
    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
    ...
    if closestPlayer ~= -1 and closestDistance <= 2.0 then
        ...
        if not IsEntityPlayingAnim(GetPlayerPed(-1), "weapons@first_person@aim_rng@generic@projectile@thermal_charge@", "plant_floor", 3) then
            TaskPlayAnim( GetPlayerPed(-1), "weapons@first_person@aim_rng@generic@projectile@thermal_charge@", "plant_floor", 8.0, 1.0, -1, 49, 0, 0, 0, 0 )
            TriggerServerEvent('ffrp-death:healplayer', GetPlayerFromServerId(closestPlayer), 'big')
            Citizen.Wait(2000)
            ClearPedTasksImmediately(GetPlayerPed(-1))
        end
    else
...
local function SwitchAnimSet(animation)
    ...
    TriggerServerEvent("ffrp:setMetaData", {animSet = AnimSet, armour = armour})
end
...
RegisterNetEvent('ffrp:LoadMetaData')
AddEventHandler('ffrp:LoadMetaData', function(data)
...
AddEventHandler('FlipVehicle', function()
    ...
    if DoesEntityExist(vehicle) then
        local finished = exports['ffrp-taskbar']:taskBar(10000, 'Flipping vehicle')
