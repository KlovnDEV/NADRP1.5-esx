	...
	if bait == "turtle" then
		if rnd >= 78 then
			if rnd >= 94 then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'It was huge and it broke your fishing rod!', length = 5000})
				TriggerClientEvent('fishing:break', _source)
				TriggerClientEvent('inventory:removeItem',xPlayer.source,"fishingrod", 1)
			else
				TriggerClientEvent('fishing:setbait', _source, "none")
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'You caught a turtle, These are endangered species and are illegal to posses', length = 5000})
				TriggerClientEvent("player:receiveItem",xPlayer.source,"turtle",1)
			end
		else
			if rnd >= 75 then
				weight = math.random(4,9)
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'You caught a fish:' .. weight .. 'kg', length = 5000})
		...
		if bait == "none" then
			if rnd >= 70 then
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'You are currently fishing without any equipped bait', length = 5000})
				...
				TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Well done you caught plastic :)', length = 5000})
			end
		end
		if bait == "shark" then
			if rnd >= 82 then
				if rnd >= 91 then
					...
					TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'You caught a shark! These are endangered species and are illegal to posses', length = 5000})
...
AddEventHandler('Ford:TradeFish', function(itemCount)
    ...
    if itemCount and itemCount > 0 then
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Traded ' .. itemCount .. ' Fish For $' .. payment })
		TriggerClientEvent('inventory:removeItem',_source ,"packagedfish",  itemCount) 
        xPlayer.addMoney(payment)
    else
        Citizen.Wait(5000)
        TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'You dont have enough Fish'})
    end
end)
...
AddEventHandler('Ford:TradeTurtle', function(itemCount)
    ...
    if itemCount and itemCount > 0 then
        TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Traded ' .. itemCount .. ' Turtle For $' .. payment })
        xPlayer.addMoney(payment)
    else
        Citizen.Wait(5000)
        TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'You dont have enough Fish'})
...
AddEventHandler('Ford:TradeShark', function(itemCount)
    ...
    if itemCount and itemCount > 0 then
        TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'inform', text = 'Traded ' .. itemCount .. ' Shark For $' .. payment })
        xPlayer.addMoney(payment)
    else
        Citizen.Wait(5000)
        TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'You dont have enough Fish'})
	...
	local xPlayer  = ESX.GetPlayerFromId(_source)
			if item == "fish" then
					local FishQuantity = xPlayer.getInventoryItem('fish').count
					    ...
					    TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'You dont have enough fish', length = 5000})
			...
			if item == "turtle" then
				...
				if FishQuantity <= 0 then
					TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'You dont have enough turtles', length = 5000})
			...
			if item == "shark" then
				...
				if FishQuantity <= 0 then
					TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'You dont have enough sharks', length = 5000})
