    ...
    while true do
		...
		if dist <= 3.0 then
			...
			if IsControlJustPressed(0, Keys['E']) then
				local itemCount = exports["ffrp-inventory"]:getQuantity("packagedfish")
				if not selling then
					if exports["ffrp-inventory"]:hasEnoughOfItem("packagedfish",1,false) then
						selling = true
						exports['t0sic_loadingbar']:StartDelayedFunction('Trading Packaged Fish', (math.random(40, 60) * 1000), function()
							...
							if #(GetEntityCoords(PlayerPedId()) - vector3(SellFish.x, SellFish.y, SellFish.z)) < 5.0 then
								if exports["ffrp-inventory"]:hasEnoughOfItem("packagedfish",itemCount,false) then
...
Citizen.CreateThread(function()
    while true do
		...
		if dist <= 5.0 then
			...
			if IsControlJustPressed(0, Keys['E']) then 
				local itemCount = exports["ffrp-inventory"]:getQuantity("turtle")
				if not selling then
					if exports["ffrp-inventory"]:hasEnoughOfItem("turtle",1,false) then
						selling = true
						exports['t0sic_loadingbar']:StartDelayedFunction('Trading Turtle', (math.random(40, 60) * 1000), function()
							...
							if #(GetEntityCoords(PlayerPedId()) - vector3(SellTurtle.x, SellTurtle.y, SellTurtle.z)) < 5.0 then
								if exports["ffrp-inventory"]:hasEnoughOfItem("turtle",itemCount,false) then 
...
Citizen.CreateThread(function()
    while true do
		...
		local itemCount = exports["ffrp-inventory"]:getQuantity("shark")
		...
		if dist <= 2.0 then
			...
			if IsControlJustPressed(0, Keys['E']) then
				if not selling then
					if exports["ffrp-inventory"]:hasEnoughOfItem("shark",1,false) then
						selling = true
						exports['t0sic_loadingbar']:StartDelayedFunction('Trading Shark', (math.random(40, 60) * 1000), function()
							...
							if #(GetEntityCoords(PlayerPedId()) - vector3(SellShark.x, SellShark.y, SellShark.z)) < 5.0 then
								if exports["ffrp-inventory"]:hasEnoughOfItem("shark",itemCount,false) then
...
Citizen.CreateThread(function()
	while true do
		...
		if dist <= 0.5 then
			if IsControlJustPressed(0, Keys['E']) then
				if exports["ffrp-inventory"]:hasEnoughOfItem("fish",10,false) then
					PortionFish()
				else
...
function PortionFish()
	...
	local finished = exports["ffrp-taskbar"]:taskBar(10000,"Chopping fish into fillets",false,false,playerVeh)
	if position == 1 then
