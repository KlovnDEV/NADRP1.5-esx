	...
	while true do
		...
		if dist <= 0.5 then
			if IsControlJustPressed(0, Keys['E']) then
				if exports["ffrp-inventory"]:hasEnoughOfItem("alivechicken",1,false) then
					PortionChicken()
				else
		...
		if distP <= 0.5 then
			sleep = 0
			if IsControlJustPressed(0, Keys['E']) and exports["ffrp-inventory"]:hasEnoughOfItem("slaughteredchicken",1,false) then 
				PackChicken(1)
		...
		if distP2 <= 0.5 then
			sleep = 0
			if IsControlJustPressed(0, Keys['E']) and exports["ffrp-inventory"]:hasEnoughOfItem("slaughteredchicken",1,false) then -- "E"
				PackChicken(2)
			elseif IsControlJustPressed(0, Keys['G']) then
				StopPacking(2)
			end
...
function PackChicken(position)
	...
	local finished = exports["ffrp-taskbar"]:taskBar(10000,"Packaging Chicken",false,false,playerVeh)
	FreezeEntityPosition(GetPlayerPed(-1), true)
	--Citizen.Wait(6500)
	if (finished == 100) then
		local hasItem = exports["ffrp-inventory"]:hasEnoughOfItem("slaughteredchicken", 2, false)
		if hasItem then
...
function PortionChicken()
	...
	local finished = exports["ffrp-taskbar"]:taskBar(10000,"Chopping up chicken",false,false,playerVeh)
	if position == 1 then
	...
	ClearPedTasks(GetPlayerPed(-1))
	if (finished == 100) then
		local hasItem = exports["ffrp-inventory"]:hasEnoughOfItem("alivechicken", 1, false)
		if hasItem then
...
Citizen.CreateThread(function()
    while true do
		...
		if dist <= 2.0 then
			...
			if IsControlJustPressed(0, Keys['E']) then
				local itemCount = exports["ffrp-inventory"]:getQuantity("packagedchicken")
				if not selling then
					if exports["ffrp-inventory"]:hasEnoughOfItem("packagedchicken",1,false) then
						selling = true
						exports['t0sic_loadingbar']:StartDelayedFunction('Trading Chicken', (math.random(40, 60) * 1000), function()
							...
							if #(GetEntityCoords(PlayerPedId()) - vector3(sellX, sellY, sellZ)) < 5.0 then
								if exports["ffrp-inventory"]:hasEnoughOfItem("packagedchicken",itemCount,false) then
