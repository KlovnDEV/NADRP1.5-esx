...
function EndSelling()
	...
	sellingcrack = false
	alerto = false
	TriggerEvent('ffrp-density:SetPedDensity', 0.4)
end
...
RegisterNetEvent('ffrp-selldrugs:start')
AddEventHandler('ffrp-selldrugs:start', function()
	if cooldown then
	...
	print(MyStreetName)
    if (MyStreetName == "Forum Dr" or MyStreetName == "Brouge Ave" or MyStreetName == "Grove St" or MyStreetName == "Macdonald St" or MyStreetName == "Carson Ave") and exports["ffrp-inventory"]:hasEnoughOfItem("weedq",1,false) or exports["ffrp-inventory"]:hasEnoughOfItem("weedq2",1,false) and dst < 500.0 then
    ...
    TriggerEvent('ffrp-density:SetPedDensity', 0.6)
    while sellingcocaine or sellingcrack or sellingweed or sellingmeth do
		...
		TriggerEvent("ffrp-selldrugs:AllowSale", RandomNPC, saleprice, sellAmount)
    end
...
RegisterNetEvent('ffrp-selldrugs:MovePed')
AddEventHandler("ffrp-selldrugs:MovePed",function(p)
    local usingped = p
...
RegisterNetEvent('ffrp-selldrugs:AllowSale')
AddEventHandler('ffrp-selldrugs:AllowSale', function(NPC,saleprice,amount)
	local timer = 0
	local plyCoords = GetEntityCoords(PlayerPedId())
	local startdst = #(GetEntityCoords(NPC) - plyCoords)
	TriggerEvent("ffrp-selldrugs:MovePed",NPC)
	while true do
...
function SellDrugs(NPC,saleprice,amount)
	...
	local crack = exports["ffrp-inventory"]:hasEnoughOfItem("1gcrack",amount,false)
	if crack and sellingcrack then
		TriggerEvent("inventory:removeItem", "1gcrack", amount)
	end

	local weedbaggies = exports["ffrp-inventory"]:hasEnoughOfItem("weedq",amount,false)
	if weedbaggies and sellingweed then
		TriggerEvent("inventory:removeItem", "weedq", amount)
	end

	local methbag = exports["ffrp-inventory"]:hasEnoughOfItem("methbag",amount,false)
	if methbag and sellingmeth then
		TriggerEvent("inventory:removeItem", "methbag", amount)
	end

	local cocaine = exports["ffrp-inventory"]:hasEnoughOfItem("1gcocaine",amount,false)
...
RegisterNetEvent('ffrp-gangs:corner:amount')
AddEventHandler('ffrp-gangs:corner:amount', function(newAmount)
			...
			if CopsConnected >= 2 then
				if amount ~= nil then
					if not IsPedInAnyVehicle(PlayerPedId(), false) then
						TriggerEvent('ffrp-selldrugs:start', CopsConnected)
						TriggerEvent('ffrp-gangs:corner:amount', amount)
					else
