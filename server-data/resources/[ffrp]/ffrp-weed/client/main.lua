...
RegisterNetEvent("ffrp-weed:currentcrops")
AddEventHandler("ffrp-weed:currentcrops", function(result)
	local newcrops = {}
	for i = 1, #result do
...
function InsertPlant(seed, seedType)
	...
	TriggerServerEvent("ffrp-weed:createplant",x,y,z,strain, seedType)
end
...
function convertFemales()
	...
	TriggerServerEvent("ffrp-weed:convert",convertedIds,convertedTable)
end

RegisterNetEvent("ffrp-weed:startcrop")
AddEventHandler("ffrp-weed:startcrop", function(seedType)
	local plyCoords = GetEntityCoords(PlayerPedId())
	...
	if not exports["ffrp-inventory"]:hasEnoughOfItem("plastic",3,true) then
		return
	end
...
RegisterNetEvent("ffrp-weed:destroyplant")
AddEventHandler("ffrp-weed:destroyplant", function()
	local plyCoords = GetEntityCoords(PlayerPedId())
	local close = 0
	local dst = 1000.0
	for i = 1, #crops do
	...
	local finished = exports["ffrp-taskbar"]:taskBar(40000,"Destroy")
	if finished == 100 then
		TriggerServerEvent("ffrp-weed:killplant",crops[close]["dbID"])
	end
end)

RegisterNetEvent("ffrp-weed:updateplantwithID")
AddEventHandler("ffrp-weed:updateplantwithID", function(ids,newPercent,status)
...
RegisterNetEvent("ffrp-weed:giveitems")
AddEventHandler("ffrp-weed:giveitems", function(strain)
...
RegisterNetEvent("ffrp-weed:updateCrops")
AddEventHandler("ffrp-weed:updateCrops", function(newCrops)
	crops = newCrops
end)
...
RegisterNetEvent('ffrp-weed:removeItems')
AddEventHandler('ffrp-weed:removeItems', function(seedType)
				...
				if #(vector3(crops[close]["x"],crops[close]["y"],crops[close]["z"]-0.3) - plyCoords) < 10.0 then
					...
					if IsControlJustReleased(2, 38) and #(vector3(crops[close]["x"],crops[close]["y"],crops[close]["z"]-0.3) - plyCoords) < 2.0 and counter == 0 then
						if crops[close]["growth"] >= 100 then
							local finished = exports["ffrp-taskbar"]:taskBar(1000,"Picking")
							TriggerEvent("Evidence:StateSet",4,1600)
							TriggerServerEvent("ffrp-weed:killplant",crops[close]["dbID"])
							TriggerEvent("ffrp-weed:giveitems",crops[close]["strain"])
						else
							if crops[close]["status"] == 1 then
								...
								if crops[close]["strain"] == "Seeded" then
									if exports["ffrp-inventory"]:hasEnoughOfItem("fertilizer",1,false) then
										...
										TriggerServerEvent("ffrp-weed:UpdateWeedGrowth",crops[close]["dbID"],new, crops[close]['status'])
									else
										exports['mythic_notify']:SendAlert('error','You need Fertilizer for this!')
									end
								else
									if exports["ffrp-inventory"]:hasEnoughOfItem("water",1,false) then
										TriggerEvent("status:setState",4,1600)
...
AddEventHandler('tac:playerLoaded', function()
	TriggerServerEvent('ffrp-weed:RequestWeedPlants')
end)
...
RegisterCommand('destroyplant', function(source, args, raw) 
	...
	if job == 'police' then
		TriggerEvent('ffrp-weed:destroyplant')
	end
end)
