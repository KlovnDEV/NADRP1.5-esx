...
local office = false
local curHouseCoords = { ['x'] = 0.0,['y'] = 0.0,['z'] = 0.0,['h'] = 0.0, ['info'] = ' default ' }
local entryType = 1
local shithouse = false
local shopshit = false
local Controlkey = {["generalUse"] = {38,"E"},["housingMain"] = {74,"H"},["housingSecondary"] = {47,"G"}} 
local scannedPOIs = {}
...
modifyingTable = { 
	...
	["wardrobe"] = { ["x"] = 0.0, ["y"] = 0.0, ["z"] = 0.0, ["h"] = 0.0  },
...
function resetModifyingTable()
	...
	modifyingTable["wardrobe"] = { ["x"] = 0.0, ["y"] = 0.0, ["z"] = 0.0, ["h"] = 0.0  }
...
robberycoordsMansions = {
	...
	[265] = { ["x"] = -1536.987, ["y"] = 130.603, ["z"] = 57.371, ["h"] = 317.794,  ["info"] = 'Fords Mansion', ['office'] = 2 },
	[266] = { ["x"] = -3093.728, ["y"] = 349.583, ["z"] = 7.544, ["h"] = 73.315,  ["info"] = ' Ineseno Road 6', ['office'] = 2 },
}
...
Citizen.CreateThread(function()
	...
	local updatetime = 0
 	while true do
		if modifying then
			found = true
			if modifyingTable["garages"]["x"] ~= 0.0 then
				DrawText3Ds( modifyingTable["garages"]["x"],modifyingTable["garages"]["y"],modifyingTable["garages"]["z"] , 'Garage Spawn Point' )
			end
			...
			if modifyingTable["wardrobe"]["x"] ~= 0.0 then
				DrawText3Ds( modifyingTable["wardrobe"]["x"],modifyingTable["wardrobe"]["y"],modifyingTable["wardrobe"]["z"] , 'Clothing Point' )
			end
		...
		if myhouseid == 0 then
			...
			if entryType == 1 then
				...
				if myhouseid ~= 0 then
					if mansion then
						if (#(vector3( curHouseCoords["x"] -5.776 , curHouseCoords["y"] + 9.3, curHouseCoords["z"] - 23.0) - playerCoords) < 1.2 ) then
							found = true
							DrawText3Ds( curHouseCoords["x"] -5.776 , curHouseCoords["y"] + 9.3, curHouseCoords["z"] - 23.0, '~b~'..Controlkey["housingMain"][2]..'~s~ front door' )
						...
						if robberyExitCoordsMansions[myhouseid] then
							if robberyExitCoordsMansions[myhouseid]["x"] ~= 0.0 then
								if (#(vector3(curHouseCoords["x"]+14.1793,curHouseCoords["y"]+1.900,curHouseCoords["z"]-32.42) - playerCoords) < 1.2 ) then
									found = true
									DrawText3Ds( curHouseCoords["x"]+14.1793,curHouseCoords["y"]+1.900,curHouseCoords["z"]-32.82, '~b~'..Controlkey["housingMain"][2]..'~s~ back door' )
									if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
					...
					elseif mansion2 then
						if (#(vector3(curHouseCoords["x"]+13.5793,curHouseCoords["y"]+12.400,curHouseCoords["z"]-26.42) - playerCoords) < 1.2 ) then
							found = true
							DrawText3Ds(curHouseCoords["x"]+13.5793,curHouseCoords["y"]+12.400,curHouseCoords["z"]-26.42, '~b~'..Controlkey["housingMain"][2]..'~s~ front door' )
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								TriggerServerEvent("houserobberies:exit",myhouseid,mansion,false)
								Citizen.Wait(5000)
							end
					...
					elseif shopshit then
						if (#(vector3((curHouseCoords["x"]+3),(curHouseCoords["y"]-5),(curHouseCoords["z"]-34.0)) - playerCoords) < 1.2 ) then
							found = true
							DrawText3Ds((curHouseCoords["x"]+3),(curHouseCoords["y"]-5),(curHouseCoords["z"]-35.0), '~b~'..Controlkey["housingMain"][2]..'~s~ to leave' )
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								ExitHouse()
								Citizen.Wait(5000)
							end
					...
					elseif trailer then
						if (#(vector3((curHouseCoords["x"]-1.443),(curHouseCoords["y"]-1.892),(curHouseCoords["z"]-74.0)) - playerCoords) < 100.2 ) then
							found = true
							DrawText3Ds( (curHouseCoords["x"]-1.443),(curHouseCoords["y"]-1.892),(curHouseCoords["z"]-74.0), '~b~'..Controlkey["housingMain"][2]..'~s~ to leave' )
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								ExitHouse()
								Citizen.Wait(5000)
							end
						...
						if (#(vector3(curHouseCoords["x"] + 4.3,curHouseCoords["y"] - 15.95,curHouseCoords["z"]-21.42) - playerCoords) < 1.2 ) then
							found = true
							DrawText3Ds( (curHouseCoords["x"] + 4.3),(curHouseCoords["y"] - 15.95),(curHouseCoords["z"]-21.42), '~b~'..Controlkey["housingMain"][2]..'~s~ front door' )
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								TriggerServerEvent("houserobberies:exit",myhouseid,mansion,false)
								Citizen.Wait(5000)
							end
						...
						if robberyExitCoords[myhouseid] then
							if robberyExitCoords[myhouseid]["x"] ~= 0.0 then
								if (#(vector3(curHouseCoords["x"] -3.8,curHouseCoords["y"] + 5.25,curHouseCoords["z"]-21.42) - playerCoords) < 1.2 ) then
									found = true
									DrawText3Ds( (curHouseCoords["x"] - 3.8),(curHouseCoords["y"] + 5.25),(curHouseCoords["z"]-21.42), '~b~'..Controlkey["housingMain"][2]..'~s~ back door' )
									if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
										TriggerServerEvent("houserobberies:exit",myhouseid,mansion,true)
										Citizen.Wait(5000)
									end
				...
				if mansion then
					if (#(vector3(curHouseCoords["x"] -5.776 , curHouseCoords["y"] + 9.3, curHouseCoords["z"] - 24.0) - playerCoords) < 1.2 ) then
						found = true
						DrawText3Ds(curHouseCoords["x"] -5.776 , curHouseCoords["y"] + 9.3, curHouseCoords["z"] - 24.0, '~b~'..Controlkey["housingMain"][2]..'~s~ to leave' )
						if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
							ExitHouse()
							Citizen.Wait(5000)
						end
				...
				elseif rentedShop then
					if (#(vector3(curHouseCoords["x"]-1.4,curHouseCoords["y"]-4.47,curHouseCoords["z"]-32.2) - playerCoords) < 2.2 ) then
						found = true
						DrawText3Ds( curHouseCoords["x"]-1.4,curHouseCoords["y"]-4.47,curHouseCoords["z"]-32.2, '~b~'..Controlkey["housingMain"][2]..'~s~ to leave' )
						if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
							ExitHouse()
							Citizen.Wait(5000)
						end
				...
				elseif rentedOffice then
					if (#(vector3(curHouseCoords["x"]+9,curHouseCoords["y"]+0.52,curHouseCoords["z"]-33.99) - playerCoords) < 3.2 ) then
						found = true
						DrawText3Ds( curHouseCoords["x"]+9,curHouseCoords["y"]+0.52,curHouseCoords["z"]-33.99, '~b~'..Controlkey["housingMain"][2]..'~s~ to leave' )
						if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
							ExitHouse()
							Citizen.Wait(5000)
						end
				...
				elseif mansion2 then
					if (#(vector3(curHouseCoords["x"]+13.5793,curHouseCoords["y"]+12.400,curHouseCoords["z"]-26.42) - playerCoords) < 1.2 ) then
						found = true
						DrawText3Ds( curHouseCoords["x"]+14.0793,curHouseCoords["y"]+12.000,curHouseCoords["z"]-26.42, '~b~'..Controlkey["housingMain"][2]..'~s~ to leave' )
						if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
							ExitHouse()
							Citizen.Wait(5000)
						end
				...
				elseif office then
					if (#(vector3(curHouseCoords["x"]-3.5793,curHouseCoords["y"]+3.100,curHouseCoords["z"]-22.42) - playerCoords) < 1.2 ) then
						found = true
						DrawText3Ds( curHouseCoords["x"]-3.5793,curHouseCoords["y"]+3.100,curHouseCoords["z"]-22.82, '~b~'..Controlkey["housingMain"][2]..'~s~ to leave' )
						if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
							ExitHouse()
							Citizen.Wait(5000)
						end
				...
				elseif trailer then
					if (#(vector3((curHouseCoords["x"]-1.343),(curHouseCoords["y"]-2.292),(curHouseCoords["z"]-71.75)) - playerCoords) < 1.2 ) then
						found = true
						DrawText3Ds( (curHouseCoords["x"]-1.343),(curHouseCoords["y"]-2.292),(curHouseCoords["z"]-71.75), '~b~'..Controlkey["housingMain"][2]..'~s~ to leave' )
						if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
							ExitHouse()
							Citizen.Wait(5000)
						end
					...
					if apartment then
						if (#(vector3(curHouseCoords["x"] - 1.15,curHouseCoords["y"] - 3.7,curHouseCoords["z"]-25.50) - playerCoords) < 1.2 ) then
							found = true
							DrawText3Ds(curHouseCoords["x"] - 1.15,curHouseCoords["y"] - 3.7,curHouseCoords["z"]-25.50, '~b~'..Controlkey["housingMain"][2]..'~s~ to leave')
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								ExitHouse()
								Citizen.Wait(5000)
							end
					...
					elseif shithouse then
						if (#(vector3(curHouseCoords["x"]+5.3,curHouseCoords["y"]-5.5,curHouseCoords["z"]-24.2) - playerCoords) < 1.2 ) then
							found = true
							DrawText3Ds( curHouseCoords["x"]+5.3,curHouseCoords["y"]-5.5,curHouseCoords["z"]-24.2, '~b~'..Controlkey["housingMain"][2]..'~s~ to leave' )
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								ExitHouse()
								Citizen.Wait(5000)
							end
					...
					elseif beachhouse then
						if (#(vector3(curHouseCoords["x"], curHouseCoords["y"]-4, curHouseCoords["z"]-29.5) - playerCoords) < 1.2 ) then
							found = true
							DrawText3Ds(curHouseCoords["x"], curHouseCoords["y"]-4,curHouseCoords["z"]-29.5, '~b~'..Controlkey["housingMain"][2]..'~s~ to leave' )
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								ExitHouse()
								Citizen.Wait(5000)
							end
					...
					elseif shopshit then
						if (#(vector3((curHouseCoords["x"]+3),(curHouseCoords["y"]-5),(curHouseCoords["z"]-34.0)) - playerCoords) < 5555.2 ) then
							found = true
							DrawText3Ds( (curHouseCoords["x"]+3),(curHouseCoords["y"]-5),(curHouseCoords["z"]-34.0), '~b~'..Controlkey["housingMain"][2]..'~s~ to leave' )
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								ExitHouse()
								Citizen.Wait(5000)
							end
						...
						if (#(vector3(curHouseCoords["x"] + 5.7,curHouseCoords["y"] - 16.95,curHouseCoords["z"]-24.42) - playerCoords) < 1.2 ) then
							found = true
							DrawText3Ds( (curHouseCoords["x"] + 5.7),(curHouseCoords["y"] - 16.95),(curHouseCoords["z"]-24.42), '~b~'..Controlkey["housingMain"][2]..'~s~ to leave' )
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								ExitHouse()
								Citizen.Wait(5000)
							end
...
RegisterCommand('raid', function(source, args, raw)
	local job = exports["ffrp-ped"]:isPed("myJob")
	if job == "police" then
		TriggerEvent('housing:raid')
	end
end)
...
AddEventHandler('housing:raid', function()
	if housenameraid ~= "none" and myhouseid ~= 0 then
		...
		local log = '['..name..':'..ident..']: Raided '..housenameraid
		TriggerEvent("server-inventory-open", "1", "house-"..housenameraid)
		TriggerServerEvent('ffrp-log:AddHousingLog',name,ident,'raid',log,housenameraid)
	end
end)
...
AddEventHandler('housing:findsalecid', function(checkcid,price,house_id,house_model,name,x,y,z)
	...
	while buying > 0 do
		if not sentChatmsg then
			if name ~= nil then
				local msg = "Offered property " .. housename .. " with down payment of $3000 then $3000 per week, Type /confirm to confirm."
				TriggerEvent('chat:addMessage', {
					template = '<div class="chat-message nonemergency"><b>Contract</b> | <br> {0} </div>',
					args = {msg}
				});
			else
				local msg = [[Your Agreement is a lease-purchase with the Real Estate Company.
				25 percent of total purchase price as down payment must be made upon closing with 5 percent of remaining balance paid per week for a total of 15 weeks.
				If the client causes any harm to any employee of the Real Estate Company or does not make proper payment for 14 days,
				this agreement is revoked, and property surrendered. This clause will be in perpetuity of the leaseholder’s occupation of the property.
	...
	if buy then
		...
		if name ~= nil then
			...
			TriggerServerEvent("house:purchasehouse",house_id,house_model,price,housename,false) 
		end
	end
	sentChatmsg = false
end)
...
RegisterCommand('confirm', function(source, args, raw)
...
AddEventHandler('GarageData', function(garages)
	...
	for i=1, #garages do
		...
		if house_model < 3 or house_model == 5 then
			...
			mygarages["garage"..table_id] = { ["home_owner"] = garages[i]["home_owner"], ["house_name"] = house_name, ["house_id"] = house_id, ["house_model"] = house_model, ["house_poi"] = garages[i]["house_poi"],  ["x"] = x,  ["y"] = y,  ["z"] = z,  ["table_id"] = table_id }
		end
	end
...
AddEventHandler('housing:info:realtor', function(args)
	...
	if args == "unlock" then
		if modifying then
			TriggerServerEvent("housing:unlock",houseinfo[1],houseinfo[2], true)
		else
			TriggerServerEvent("housing:unlock",houseinfo[1],houseinfo[2], false)
		end
...
function GetHouseInformation()
	...
	for i=1,#robberycoords do
		if (#(vector3(robberycoords[i]["x"],robberycoords[i]["y"],robberycoords[i]["z"]) - GetEntityCoords(PlayerPedId())) < 1.2 ) then
			...
			if robberycoords[i]["trailer"] ~= nil then
				house_model = 5
			else
				house_model = 1
			end
...
function GetHouseCoords(type,house_id,house_model)
	if type == 'robberycoords' then
		local coords = robberycoords[house_id]
		local data = {x = coords.x, y = coords.y, z = coords.z}
		return data
	elseif type == 'robberycoordsMansions' then 
		local coords = robberycoordsMansions[house_id]
		local data = {x = coords.x, y = coords.y, z = coords.z}
		return data
	end
end
...
AddEventHandler("openFurniture", function()
	if entryType == 2 then
		if mansion or mansion2 or office then
			TriggerServerEvent("house:GetFurniture",myhouseid,2)
		elseif trailer

		then
			TriggerServerEvent("house:GetFurniture",myhouseid,5)
		else
			TriggerServerEvent("house:GetFurniture",myhouseid,1)
		end
	end
end)
...
function buildHouse()
	...
	SetEntityCoords(PlayerPedId(), generator.x+ 5.7, generator.y- 16.95, generator.z+ 25.42)

	local building = CreateObject(`playerhouse_tier1`,generator.x+2.29760700,generator.y-1.33191200,generator.z+1.26253700,false,false,false)
	FreezeEntityPosition(building,true)
	Citizen.Wait(100)

	Citizen.Wait(1500)
	SetEntityCoords(PlayerPedId(), generator.x+5.7, generator.y-16.95, generator.z+2.9)
	SetEntityHeading(PlayerPedId(),curHouseCoords["h"])
...
function buildApartment(heading)
	...
	local building = CreateObject(`ffrp_hotel`,generator.x,generator.y,generator.z,false,false,false)

	FreezeEntityPosition(building,true)
	Citizen.Wait(100)

	local a = CreateObject(`v_49_motelmp_winframe`,generator.x+1.34,generator.y-3.93,generator.z+2.31,false,false,false)
	local b = CreateObject(`v_49_motelmp_glass`,generator.x+1.34,generator.y-3.93,generator.z+2.33,false,false,false)
	local c = CreateObject(`v_49_motelmp_curtains`,generator.x+1.34,generator.y-3.82,generator.z+2.2,false,false,false)
	FreezeEntityPosition(a,true)
	FreezeEntityPosition(b,true)
	FreezeEntityPosition(c,true)
	Citizen.Wait(500)
	SetEntityCoords(PlayerPedId(), curHouseCoords["x"] -1.0, curHouseCoords["y"] - 3.5, curHouseCoords["z"] - 27.0)
end

function buildMansion()
	SetEntityCoords(PlayerPedId(),-801.5,178.69,72.84)
	Citizen.Wait(1000)
	...
	local generator = { x = curHouseCoords["x"] , y = curHouseCoords["y"], z = curHouseCoords["z"] - 34}
	SetEntityCoords(PlayerPedId(), curHouseCoords["x"] -5.776 , curHouseCoords["y"] + 9.3, curHouseCoords["z"] - 25.0)

	local building = CreateObject(`shell_michael`,generator.x+3.57921200,generator.y+3.70079500,generator.z,false,false,false)
	FreezeEntityPosition(building,true)
...
function buildMansionHighEnd()
	...
	local generator = { x = curHouseCoords["x"] , y = curHouseCoords["y"], z = curHouseCoords["z"] - 33}
	SetEntityCoords(PlayerPedId(), curHouseCoords["x"]+14.0793,curHouseCoords["y"]+12.000,curHouseCoords["z"]-26.42)

	local building = CreateObject(`frankelientje`,generator.x+3.57921200,generator.y+3.70079500,generator.z+0.045,false,false,false)
	FreezeEntityPosition(building,true)
...
function buildTrevor()
	...
	local generator = { x = curHouseCoords["x"] , y = curHouseCoords["y"], z = curHouseCoords["z"] - 33.0}
	SetEntityCoords(PlayerPedId(), curHouseCoords["x"], curHouseCoords["y"]-4, curHouseCoords["z"]-29.5)

	local building = CreateObject(`shell_trevor`,generator.x+0.42222900,generator.y-0.13131700,generator.z+1.18855100,false,false,false)
	FreezeEntityPosition(building,true)

	SetEntityHeading(PlayerPedId(),351.0)
	SetGameplayCamRelativeHeading(0.0)
...
AddEventHandler('GPSLocations', function()
	...
	TriggerEvent("openGPS",robberycoordsMansions,robberycoords)
end)

exports('GetHouseCoords', GetHouseCoords)