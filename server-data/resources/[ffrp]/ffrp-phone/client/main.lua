...
AddEventHandler("phone:listunpaid", function(outstandingArray)
  local data = outstandingArray
  if outstandingArray ~= false then
      SendNUIMessage({
        openSection = "showOutstandingPayments",
        outstandingPayments = data,
        type = "cardealer"
      })
  else
    SendNUIMessage({
...
function GroupName(groupid)
  ...
  --local mypasses = exports["ffrp-ped"]:isPed("passes")
  for i=1, #mypasses do
...
RegisterNUICallback('bankGroup', function(data)
    ...
    if rank >= 1 then
      ...
      local log = name..' deposited $'..cashamount
      TriggerServerEvent("server:bankGroup", gangid,cashamount)
      exports['ffrp-log']:AddLog('business', name, steamid, 'Deposit', log, job)
      print('bankGroup')
    else
...
RegisterNUICallback('payGroup', function(data)
    ...
    if rank >= 4 then
      local steamid = exports['ffrp-ped']:isPed('steamid')
      local name = GetPlayerName(PlayerId())
      local log = name..' Payed CID:'..cid..' $'..cashamount
      TriggerServerEvent("server:givepayGroup", job,cashamount,cid)
      exports['ffrp-log']:AddLog('business', name, steamid, 'Payed', log, job)
      print('payGroup', log)
    else
      exports['mythic_notify']:SendAlert('inform', 'You dont have permissions to use this')
    end
...
RegisterNUICallback('promoteGroup', function(data)
    ...
    if rank >=  4 then
      local steamid = exports['ffrp-ped']:isPed('steamid')
      local name = GetPlayerName(PlayerId())
      local log = name..' Promoted CID:'..cid..' To rank '..newrank
      TriggerServerEvent("server:promoteGroup", gangid,newrank,cid)
      exports['ffrp-log']:AddLog('business', name, steamid, 'Promoted', log, job)
      print('promoteGroup')
    else
      exports['mythic_notify']:SendAlert('inform', 'You dont have permissions to use this')
    end
...
RegisterNUICallback('manageGroup', function(data)
  for i=1, #groups do
    if job == groups[i] then
      TriggerServerEvent("group:pullinformation", job)
    end
  end
end)
...
AddEventHandler("phone:error", function()
    SendNUIMessage({
      openSection = "error",
      textmessage = "<b>Network Error</b> <br><br> Please contact support if this error persists, thank you for using FFRP Phone Services.",
    })
end)

RegisterNetEvent("group:fullList")
AddEventHandler("group:fullList", function(result,bank,groupid, groupname)
    SendNUIMessage({
...
--[[RegisterNUICallback('groupFire', function(data, cb)
  print('Rank: '..json.encode(data.rank))
  print('ID: '..json.encode(data.id))
  --TriggerServerEvent('group:fireEmployee')
end)]]

RegisterNetEvent('group:refresh')
AddEventHandler('group:refresh', function()
    if job ~= nil then
      TriggerServerEvent('group:pullinformation', job)
    end
end)
...
AddEventHandler("gangTasks:updated", function()
  --[[local gang = exports["ffrp-ped"]:isPed("gang")
  local cid = tonumber(exports["ffrp-ped"]:isPed("steamid"))]]
  ...
  local taskObject = {}
    for i = 1, #activeTasks do
        ...
        --local passes = exports["ffrp-ped"]:isPed("passes")
            for z = 1, #passes do
...
function AddCarBlip(vehCoords)
  if not recentBlip then
    recentBlip = true
    local carBlip = AddBlipForCoord(vehCoords.x, vehCoords.y, vehCoords.z)
    SetBlipSprite(carBlip, 225)
    SetBlipColour(carBlip, 2)
    SetBlipAsShortRange(carBlip, false)
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentString("Vehicle Tracker")
...
AddEventHandler("phone:Garage", function(vehs)
    ...
    for ind, value in pairs(vehs) do
        ...
        local lastpayment = value.lastpayment
        ...
        if financetimer ~= nil then
            local somethingfinal = (financetimer / 1440)
...
function hasPhone()
  local phone = exports["ffrp-inventory"]:hasEnoughOfItem("mobilephone",1,false)
  if phone then
    return true
  else
    return false
...
function hasRadio()
    --if exports["ffrp-inventory"]:hasEnoughOfItem("radio",1,false) and not exports["ffrp-ped"]:isPed("disabled") then
      return true
    --else
     -- return false
    --end
...
RegisterCommand('ping', function(source, args, raw) 
  local targID = tonumber(args[1])
  if targID ~= nil and targID > 0 then
    TriggerEvent('canPing', targID)
  end
end)

RegisterCommand('pingaccept', function(source, args, raw)
  TriggerEvent('acceptPing')
end)

RegisterNetEvent('canPing')
AddEventHandler('canPing', function(target)
  local isDead = exports['ffrp-death']:GetDeath()
  local isCuffed = exports['ffrp-ped']:isPed('handcuffed')
...
function hasDecrypt2()
    if exports["ffrp-inventory"]:hasEnoughOfItem("vpnxj",1,false) and not exports["ffrp-ped"]:isPed("disabled") then
        return true
    else
        return false
    end
...
RegisterNetEvent('phone:heHasPhone')
AddEventHandler('phone:heHasPhone', function()
    GotPhone()
end)

function GotPhone()
    local pDead = exports["ffrp-death"]:GetDeath()
    if not pDead then
        openGuiNow()
    else
...
function SetCustomNuiFocus(hasKeyboard, hasMouse)
  ...
  TriggerEvent("ffrp-actionbar:disable", HasNuiFocus)
end

RegisterCommand('phone', function(source, args, raw) 
  if hasPhone() then
    if guiEnabled then
      closeGui()
    else
...
RegisterNUICallback('deleteYP', function()
    TriggerServerEvent('phone:RemoveYP')
end)
...
AddEventHandler("yellowPages:retrieveLawyersOnline", function(command)
    ...
    if not isFound then
        TriggerEvent('chat:addMessage', {
            template = '<div class="chat-message system"> Service : {0} </div>',
            args = {"There are no lawyers available right now."}
        })
    end
...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    if guiEnabled or creatingMap then 
      Citizen.Wait(0)
    else
      Citizen.Wait(500)
    end
...
AddEventHandler('phone:addToCall', function(voipchannel)
    ...
    exports["ffrp-voice"]:addPlayerToCall(tonumber(voipchannel))
end)
...
AddEventHandler('phone:receiveCall', function(phoneNumber, srcID, calledNumber, isPayphone)
...
function receivingCall(callFrom, isPayphone)
    callTimer = 8
...
AddEventHandler('phone:removefromToko', function(playerRadioChannel)
  exports["ffrp-voice"]:removePlayerFromCall()
end)
...
AddEventHandler('phone:triggerPager', function()
    --local job = exports["ffrp-ped"]:isPed("myjob")
    if job == "doctor" then
...
AddEventHandler('phone:sendSMS', function(number, message)
    if(number ~= nil and message ~= nil) then
        ...
        TriggerEvent('chat:addMessage', {
            template = '<div class="chat-message system"> Service : {0} </div>',
            args = {"You must fill in a number and message!"}
        })
    end
end)
...
AddEventHandler('phone:reply', function(message)
    if lastnumber ~= 0 then
        ...
        TriggerEvent('chat:addMessage', {
            template = '<div class="chat-message system"> Service : {0} </div>',
            args = {"No user has recently SMS'd you."}
        })
    end
end)
...
AddEventHandler('phone:addContact', function(name, number)
    if(name ~= nil and number ~= nil) then
        ...
        TriggerEvent('chat:addMessage', {
            template = '<div class="chat-message system"> Service : {0} </div>',
            args = {"You must fill in a name and number!"}
        })
    end
end)
...
function LoadMapBlips(id, reverseTrack, laps)
  ...
  if(customMaps[id].checkpoints ~= nil) then
    local checkpoints = json.decode(customMaps[id].checkpoints)
...
function StartEvent(map,laps,counter,reverseTrack,raceName, startTime, mapCreator, mapDistance, mapDescription)
  ...
  if map == 0 then
    ShowText("Please pick a map.")
    return
  end
  print(map)
  local mapCheckpoints = json.decode(customMaps[map]["checkpoints"])
  ...
  local s1, s2 = GetStreetNameAtCoord(mapCheckpoints.x, mapCheckpoints.y, mapCheckpoints.z)
  local street1 = GetStreetNameFromHashKey(s1)
  zone = tostring(GetNameOfZone(mapCheckpoints.x, mapCheckpoints.y, mapCheckpoints.z))
...
function RunRace(identifier)
  ...
  local myLap = 0
  local mapCheckpoints = json.decode(customMaps[map]["checkpoints"])
  local checkpoints = #mapCheckpoints
  ...
  while myLap < laps+1 and racing do
    ...
    if (Vdist(mapCheckpoints[mycheckpoint]["x"],mapCheckpoints[mycheckpoint]["y"],mapCheckpoints[mycheckpoint]["z"], plyCoords.x,plyCoords.y,plyCoords.z) ) < mapCheckpoints[mycheckpoint]["dist"] then
      ...
      SetNewWaypoint(mapCheckpoints[mycheckpoint]["x"],mapCheckpoints[mycheckpoint]["y"])
...
RegisterNUICallback('racing:event:join', function(data)
  ...
  local mapCheckpoints = json.decode(customMaps[currentRaces[id]["map"]]["checkpoints"])
  local checkPointIndex = 1
  ...
  if Vdist(mapCheckpoints[checkPointIndex]["x"], mapCheckpoints[checkPointIndex]["y"], mapCheckpoints[checkPointIndex]["z"],plyCoords.x,plyCoords.y,plyCoords.z) < 40 then
    ...
    --print(currentRaces[id]["open"], JoinedRaces[id], racing)
...
RegisterNUICallback('racing:event:start', function(data)
  StartEvent(data.raceMap + 1, data.raceLaps, data.raceCountdown, data.reverseTrack, data.raceName, data.raceStartTime, data.mapCreator, data.mapDistance, data.mapDescription)
  Wait(500)
  SendNUIMessage({
    openSection = "racing:events:list",
    races = currentRaces
...
AddEventHandler('ffrp:playerBecameJob', function(jobpassed)
  job = jobpassed.name
  jobGrade = jobpassed.grade
end)

RegisterKeyMapping('phone', 'Opens Phone', 'keyboard', 'F1')