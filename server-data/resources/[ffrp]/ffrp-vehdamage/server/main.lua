...
RegisterServerEvent('ffrp-vehdamage:applyHarness')
AddEventHandler('ffrp-vehdamage:applyHarness', function(plt,dur)
    local src = source
    ...
    if isOwned[1] ~= nil and isOwned[1] ~= "[]" then
        MySQL.Async.execute('UPDATE owned_vehicles SET harnessDurability=@duribility WHERE plate=@plate', {
            ...
            TriggerClientEvent('ffrp-vehdamage:setHarness', src, 1.0, true)
        end)
    else
        TriggerClientEvent('ffrp-vehdamage:setHarness', src, false, true)
    end
end)

RegisterServerEvent('ffrp-vehdamage:getHarness')
AddEventHandler('ffrp-vehdamage:getHarness', function(plate)
    local src = source
    ...
    MySQL.Async.fetchAll('SELECT harnessDurability FROM owned_vehicles WHERE plate=@plate', {
        ...
        if result[1] ~= nil and result[1] ~= "[]" then
            ...
            TriggerClientEvent('ffrp-vehdamage:setHarness', src, harDur + 0.0, false)
        else
            TriggerClientEvent('ffrp-vehdamage:setHarness', src, false, false)
        end
    end)
end)

RegisterServerEvent('ffrp-vehdamage:updateHarness')
AddEventHandler('ffrp-vehdamage:updateHarness', function(plt, harnessDurability)
    local src = source
    local plate = string.gsub(plt, "%s+", "")
...
AddEventHandler('baseevents:enteredVehicle', function(veh)
    local user = ESX.GetPlayerFromId(source)
    TriggerClientEvent('ffrp-vehdamage:EnteredVehicle', user.source)
end)