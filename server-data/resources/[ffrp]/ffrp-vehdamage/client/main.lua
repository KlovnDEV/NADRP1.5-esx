...
function preventVehicleExit()
    Citizen.CreateThread(function()
        ...
        disableControl = true
        disableTurning()
        local finished = exports["ffrp-taskbar"]:taskBar(options[math.random(1,3)],"Taking off Harness",true)
...
function toggleHarness()
    ...
    local finished = 0
    if harness then
        finished = exports["ffrp-taskbar"]:taskBar(5000,"Taking off Harness",true)
    else
        finished = exports["ffrp-taskbar"]:taskBar(5000,"Putting on Harness",true)
    end
    if (finished == 100) then
...
RegisterNetEvent("ffrp-vehdamage:setHarness")
AddEventHandler("ffrp-vehdamage:setHarness",function(result, isNew)
...
RegisterNetEvent("ffrp-vehdamage:useHarnessItem")
AddEventHandler("ffrp-vehdamage:useHarnessItem",function()
    TriggerServerEvent("ffrp-vehdamage:applyHarness", GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), false)), 1.0)
    TriggerEvent("inventory:removeItem",'harness', 1)
end)

RegisterNetEvent("ffrp-vehdamage:resetDowngrade")
AddEventHandler("ffrp-vehdamage:resetDowngrade",function(targetvehicle)
    ...
    while true do
        ...
        if tempVehicle ~= currentVehicle then
            ...
            if currentVehicle == nil or currentVehicle == 0 or currentVehicle == false then
                ...
                TriggerServerEvent("ffrp-vehdamage:getHarness", GetVehicleNumberPlateText(currentVehicle))
            end
                        ...
                        if lastCurrentVehicleSpeed > 30.5 and currentVehicleSpeed < (lastCurrentVehicleSpeed * 0.75) then
                            if not IsThisModelABike(GetEntityModel(currentVehicle)) then
                                ...
                                if harness and harnessDurability > 0.0 then
                                    ...
                                    TriggerServerEvent("ffrp-vehdamage:updateHarness", GetVehicleNumberPlateText(currentVehicle), harnessDurability)
                                    if DecorExistOn(currentVehicle, "vehicleHarnessDur") then
...
--[[RegisterNetEvent('ffrp-vehdamage:EnteredVehicle')
AddEventHandler('ffrp-vehdamage:EnteredVehicle', function(veh)
