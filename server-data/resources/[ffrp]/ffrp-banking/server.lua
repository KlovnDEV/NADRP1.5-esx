...
RegisterServerEvent('ffrp-banking:updateClosedBank')
AddEventHandler('ffrp-banking:updateClosedBank', function(bankid)
    closedBanks[bankid]["closed"] = true
    TriggerClientEvent('ffrp-banking:updateClosedBank', -1, closedBanks)
end)

RegisterServerEvent('ffrp-banking:resetClosedBank')
AddEventHandler('ffrp-banking:resetClosedBank', function(bankid)
...
AddEventHandler('es:playerLoaded', function(source, user)
    ...
    TriggerClientEvent('ffrp-banking:updateBalance', source, user.getBank())
end)
...
RegisterCommand('cash', function(source, args, raw)
    ...
    TriggerClientEvent('ffrp-banking:viewCash', source, xPlayer.getMoney())
end)
...
function deposit(player, amount)
    ...
    TriggerClientEvent("ffrp-banking:updateBalance", source, new_balance)
...
AddEventHandler('ffrp-bank:addNotAllowed', function(pl)
...
RegisterServerEvent('ffrp-bank:deposit')
    ...
    TriggerEvent('es:getPlayerFromId', source, function(user)
        ...
        if(string.len(rounded) >= 9) then
            ...
            if(rounded <= user.getMoney()) then
                TriggerClientEvent("ffrp-banking:updateBalance", source, (user.getBank() + rounded))
                TriggerClientEvent("ffrp-banking:addBalance", source, rounded)
...
RegisterServerEvent('ffrp-bank:withdraw')
    ...
    TriggerEvent('es:getPlayerFromId', source, function(user)
        ...
        if(string.len(rounded) >= 9) then
            ...
            if(tonumber(rounded) <= tonumber(bankbalance)) then
                TriggerClientEvent("ffrp-banking:updateBalance", source, (user.getBank() - rounded))
                TriggerClientEvent("ffrp-banking:removeBalance", source, rounded)
...
RegisterServerEvent('ffrp-bank:transfer')
AddEventHandler('ffrp-bank:transfer', function(to, amountt)
	...
	if(xTarget == nil or xTarget == -1) then
		...
		if tonumber(_source) == tonumber(to) then
			...
			if balance <= 0 or balance < tonumber(amount) or tonumber(amount) <= 0 then
                ...
                TriggerClientEvent('ffrp-banking:viewBalance', to)
			end
		end
	end
end)
...
RegisterServerEvent('ffrp-bank:givecash')
AddEventHandler('ffrp-bank:givecash', function(toPlayer, amount)
    ...
    if tonumber(source) ~= tonumber(toPlayer) then
        if (tonumber(user.getMoney()) >= tonumber(amount)) then
            ...
            --TriggerClientEvent('ffrp-banking:updateCash', user.getMoney(), true)
...
AddEventHandler('es:playerLoaded', function(source)
    TriggerEvent('es:getPlayerFromId', source, function(user)
        local bankbalance = user.getBank()
        TriggerClientEvent("ffrp-banking:updateBalance", source, bankbalance)
