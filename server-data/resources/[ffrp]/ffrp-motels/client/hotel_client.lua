...
RegisterNetEvent('ffrp-motels:forceOut')
AddEventHandler('ffrp-motels:forceOut', function(roomNumber,roomtype)
	isForced = false
...
RegisterNetEvent('ffrp-motels:AttemptUpgrade')
AddEventHandler('ffrp-motels:AttemptUpgrade', function()
	--if #(vector3(260.72366333008,-375.27133178711,-44.137680053711) - GetEntityCoords(PlayerPedId())) < 3.0 then
		TriggerServerEvent('ffrp-motels:upgradeApartment', myRoomType)
		TriggerEvent("ffrp-motels:myroomtype",myRoomType)
	--end
end)

RegisterNetEvent('ffrp-motels:SetID')
AddEventHandler('ffrp-motels:SetID', function(hidX)
	hid = hidX
end)

RegisterNetEvent('ffrp-motels:SetID2')
AddEventHandler('ffrp-motels:SetID2', function(hidX)
	hid = hidX
	forcedID = hidX
end)

RegisterNetEvent("ffrp-motels:forceEnter")
AddEventHandler("ffrp-motels:forceEnter", function(roomNumber,roomtype)
...
RegisterNetEvent('ffrp-motels:CommandEnter')
AddEventHandler('ffrp-motels:CommandEnter', function(roomNumber,roomtype)
...
RegisterNetEvent('ffrp-motels:updateLockStatus')
AddEventHandler('ffrp-motels:updateLockStatus', function(newStatus)
	myRoomLock = newStatus
end)

RegisterNetEvent('ffrp-motels:createRoomFirst')
AddEventHandler('ffrp-motels:createRoomFirst', function(numMultiplier,roomType)
	myRoomNumber = numMultiplier
	myRoomType = roomType
	TriggerEvent("ffrp-motels:myroomtype",myRoomType)
end)

RegisterNetEvent('ffrp-motels:teleportRoom')
AddEventHandler('ffrp-motels:teleportRoom', function(numMultiplier,roomType)
...
RegisterNetEvent('ffrp-motels:attemptringbell')
AddEventHandler("ffrp-motels:attemptringbell",function(apnm)
...
RegisterNetEvent('ffrp-motels:buzzbuzz')
AddEventHandler("ffrp-motels:buzzbuzz",function(apartmentnumber)
...
RegisterNetEvent('ffrp-motels:interactState')
AddEventHandler('ffrp-motels:interactState', function(state)
	canInteract = state
end)

RegisterNetEvent('ffrp-motels:newRoomType')
AddEventHandler('ffrp-motels:newRoomType', function(newRoomType)
...
--[[AddEventHandler('onResourceStop', function(resource)
	if resource == GetCurrentResourceName() then
		print('RESET MOTEL')
		if roomType == 1 then
			TriggerServerEvent('ffrp-motels:resetMotel', myRoomNumber)
		end
	end
end)]]
 	...
 	while true do
		...
		if insideApartment or comparedst < 100 then
			...
			if entryUpgradesDst < 1.0 then
				...
				--	TriggerEvent("ffrp-motels:AttemptUpgrade")
				--	Citizen.Wait(2500)
				--end
			end
			...
			if (entry4rd < 5 and myRoomType == 3) or (entry3rd < 5 and myRoomType == 3) or (entry1st < 35.0 and myRoomType == 1) or (entry5rd < 65.0 and myRoomType == 1) or (entry2nd < 5 and myRoomType == 2) or (entry6th < 81.0 and myRoomType == 1) or (entry7th < 15.0 and myRoomType == 1) then
				...
				if IsControlJustReleased(1,Controlkey["housingSecondary"][1]) then
					if #(vector3(apartments1[myRoomNumber]["x"],apartments1[myRoomNumber]["y"],apartments1[myRoomNumber]["z"]) - plyCoords) < 5 and myRoomType == 1 then	
						...
						TriggerServerEvent("ffrp-motels:updatelocks", myRoomLock, myRoomNumber)
						Citizen.Wait(500)
				...
				if IsControlJustReleased(1,Controlkey["housingMain"][1]) then
					...
					if #(vector3(apartments1[myRoomNumber]["x"],apartments1[myRoomNumber]["y"],apartments1[myRoomNumber]["z"]) - plyCoords) < 5 and myRoomType == 1 then	
						...
						TriggerServerEvent("ffrp-motels:getInfo")
						Citizen.Wait(500)
				...
				if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
					if inRoom() then
						...
						if not isForced then
							TriggerServerEvent('ffrp-motels:getID')
						end
						if hid == 0 then
							hid = exports['ffrp-ped']:isPed('cid')
						end
		...
		if 	(#(vector3(curRoom.x - 1.6, curRoom.y + 1.20, curRoom.z + 1.00) - plyCoords) < 2.0 and curRoomType == 1) or 
			...
			if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
				...
				if LEO then
					...
					TriggerServerEvent('ffrp-motels:getID')
					--TriggerServerEvent('ffrp-motels:GetInteract',maxRoomWeight,forcedID)
...
function buildRoom(numMultiplier,roomType)
	...
	print('[ffrp-motels]: Requested building')
	FreezeEntityPosition(building,true)
	Citizen.Wait(100)
	FloatTilSafe(numMultiplier,roomType,building)
	print('[ffrp-motels]: Begin building objects')
	--[[CreateObject(`v_49_motelmp_stuff`,generator.x,generator.y,generator.z,false,false,false)
		...
		print('[ffrp-motels]: Finished building objects')
	...
	print('[ffrp-motels]: Set Object states')
	if not isForced then
		TriggerServerEvent('ffrp-motels:getID')
	end
	curRoomType = 1
	CleanUpPeds()
	print('[ffrp-motels]: Done building room')
...
function FloatTilSafe(numMultiplier,roomType,buildingsent)
	...
	TriggerEvent("ffrp-death:revive")
end
...
RegisterCommand('enterroom', function(source, args, raw)
	...
	if roomNumber ~= nil then 
		...
		if #(vector3(apartments1[roomNumber]["x"], apartments1[roomNumber]["y"], apartments1[roomNumber]["z"]) - coords) < 3.0 then
			TriggerServerEvent('ffrp-motels:RequestEntry', tonumber(roomNumber))
		else
...
RegisterNetEvent('ffrp-motels:createRoom')
AddEventHandler('ffrp-motels:createRoom', function(numMultiplier,roomType,mykeys,illness,isImprisoned,isClothesSpawn)
	...
	for i = 1, #mykeys do
		...
		local keyinsert = exports['ffrp-property']:GetHouseCoords('robberycoords', house_id, house_model)

		if house_model == 2 then
			keyinsert = exports['ffrp-property']:GetHouseCoords('robberycoordsMansions', house_id, house_model)
	...
	if isClothesSpawn then
		...
		TriggerServerEvent('ffrp:GetMetaData')
	else
...
function prisionSpawn()
	...
	TriggerServerEvent("server-request-update",exports["ffrp-ped"]:isPed("steamid"))
	TriggerEvent('ffrp-jail:PlayerLoaded')
