...
on("onResourceStart", async (resource) => {
    if (resource == GetCurrentResourceName()){
		cid = exports["ffrp-ped"].isPed("steamid")
		if (cid = "") {
...
on('playerSpawned', (broughtData) => {
	let cid = exports["ffrp-ped"].isPed("steamid")
		...
		if (GetVehicleDoorLockStatus(vehicleFound) == 2 && distanceRear < 1.5) {
			...
			if (distanceRear > 1.5) {
				...
				if(licensePlate != null){
					...
					let foodtruck = false
					if(exports["ffrp-ped"].isPed("myjob") == "taco") {
						foodtruck = true
					}
...
on('ffrp-weapons:SetAmmo', (sentammoTable) => {
	if (sentammoTable) {
...
on('inventory-bind', (slot) => {
	if(exports["ffrp-death"].GetDeath()){return;}
	let cid = exports["ffrp-ped"].isPed("steamid")
...
function OpenGui() {
	...
	weaponsLicence = exports["ffrp-ped"].isPed("weaponsLicence")
	cash = exports["ffrp-ped"].isPed("mycash")

	let brought = hadBrought[cid]
	let cop = false
	let job = exports["ffrp-ped"].isPed("myjob")
	if(job == "police") {
		cop = true
	}
	setTimeout(()=>{
