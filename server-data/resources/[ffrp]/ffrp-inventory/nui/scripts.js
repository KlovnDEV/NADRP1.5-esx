...
$(document).ready(function() {
    ...
    window.addEventListener('message', function(event) {
        ...
        if (item.response == "openGui") {
            ...
            if (itemList[item.id]) {
                $.post("http://ffrp-inventory/GiveItem", JSON.stringify([item.id, item.amount, item.generateInformation, true, itemList[item.id].nonStack, item.data, itemList[item.id].weight]));
            } else {
                $.post("http://ffrp-inventory/GiveItem", JSON.stringify([item.id, item.amount, item.generateInformation, false, itemList[item.id].nonStack, item.data]));
            }
        }
    })
...
function invStack(targetSlot, moveAmount, targetInventory, originSlot, originInventory, purchase, itemCosts, itemidsent, amountmoving, crafting, weapon, amountRemaining) {
    ...
    $.post("http://ffrp-inventory/stack", JSON.stringify(arr));
};
...
function invMove(targetSlot, originSlot, targetInventory, originInventory, purchase, itemCosts, itemidsent, amountmoving, crafting, weapon) {
    ...
    $.post("http://ffrp-inventory/move", JSON.stringify(arr));
...
function invSwap(targetSlot, targetInventory, originSlot, originInventory) {
    ...
    $.post("http://ffrp-inventory/swap", JSON.stringify(arr));
};

function removeCraftItems(itemid, moveAmount) {
    let arr = itemList[itemid].craft
    let amount = moveAmount;
    $.post("http://ffrp-inventory/removeCraftItems", JSON.stringify([arr, amount]));
};
...
function BuildDrop(brokenSlots) {
    $.post("http://ffrp-inventory/dropIncorrectItems", JSON.stringify({
        slots: brokenSlots
    }));
}
    ...
    for (i = 0; i < slotLimit + 1; i++) {
        ...
        if (isEmpty(inventory[i])) {
            ...
            if (itemList[itemid] !== undefined) {
                ...
                /*$.post("http://ffrp-inventory/updateMyQuality", JSON.stringify({
                  item_id: itemid,
                  slot: slot,
                  quality: quality
                }));*/
...
function UpdateSetWeights() {
    ...
    $.post("http://ffrp-inventory/Weight", JSON.stringify({
        weight: meh
    }));
}
...
function DragToggle(slot, using) {
    ...
    if (slot) {
        ...
        if (occupiedslot == true && dragging == false) {
            ...
            $.post("http://ffrp-inventory/SlotInuse", JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
...
function CompileStacks(targetSlot, originSlot, inv1, inv2, originAmount, remainingAmount, targetAmount, purchase, itemCosts, itemidsent, moveAmount) {
    ...
    $.post("http://ffrp-inventory/SlotJustUsed", JSON.stringify({
...
function SwapStacks(targetSlot, originSlot, inv1, inv2) {
    // $.post('http://ffrp-inventory/swapstack', JSON.stringify({
    //   slot1: slot1,
    //   slot2: slot2,
    //   inv1: inv1,
    //   inv2: inv2,
...
function closeInv(pIsItemUsed = false) {
    ...
    $.post("http://ffrp-inventory/ServerCloseInventory", JSON.stringify({
        name: TargetInventoryName
    }));
    TargetInventoryName = "none";
    $.post("http://ffrp-inventory/Close", JSON.stringify({
...
function useitem() {
    if (dragging != false) {
        ...
        if (inventoryUsedName == PlayerInventoryName) {
            ...
            $.post("http://ffrp-inventory/invuse", JSON.stringify(arr));
