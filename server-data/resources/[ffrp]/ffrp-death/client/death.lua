...
function InVeh()
  ...
  --local intrunk = exports["ffrp-ped"]:isPed("intrunk")
...
AddEventHandler('disableAllActions', function()
    if not disablingloop then
        ...
        Citizen.Wait(100)
        TriggerEvent("resurrect:relationships")
        TriggerEvent("ffrp-death:doAnim")
...
RegisterNetEvent('ffrp-death:doAnim')
AddEventHandler('ffrp-death:doAnim', function()
...
RegisterNetEvent('ffrp-death:cpr')
AddEventHandler('ffrp-death:cpr', function()
    ...
    if curDist < 3.0 then
        if tonumber(closestPlayer) ~= tonumber(PlayerPedId()) then
            if closestDistance < 3.0 then
                local finished = exports['ffrp-skillbar']:taskBar(4000,math.random(5,15))
                if finished ~= 100 then
                    exports["mythic_notify"]:SendAlert('error', 'Failed...')
                else
                    local finished2 = exports['ffrp-skillbar']:taskBar(3000,math.random(5,15))
                    if finished2 ~= 100 then
                        ...
                        if finished3 ~= 100 then
                            ...
                            if finished4 ~= 100 then
                                ...
                                TriggerServerEvent("ffrp-death:cpr", GetPlayerServerId(closestPlayer))
                            end
                        end
                    end
                end
...
RegisterNetEvent('ffrp-death:revive')
AddEventHandler('ffrp-death:revive', function()
    attemptRevive()
end)
...
function releaseBody()
    ...
    TriggerServerEvent('ffrp-death:RemoveItems', 1)
...
RegisterNetEvent("ffrp-death:heal")
AddEventHandler("ffrp-death:heal", function()
