...
AddEventHandler("ffrp:playerBecameJob", function(jobpassed, name, notify)
	job = jobpassed.name
    if not job then
        job = "None"
    end
...
RegisterNetEvent('ffrp-weapons:SetAmmo')
AddEventHandler('ffrp-weapons:SetAmmo', function(sentammoTable)
	if sentammoTable ~= nil then
		ammoTable = sentammoTable
		for k,v in pairs(ammoTable) do
			if v.type ~= nil then
				if ammoTable[""..v.type..""] == nil then 
					ammoTable[""..v.type..""] = {}
					ammoTable[""..v.type..""]["type"] = v.type
					ammoTable[""..v.type..""]["ammo"] = v.ammo
				end
			end
		end
		GiveAmmoNow()
...
function GiveAmmoNow()
	for i,v in pairs(ammoTable) do
		if v.type ~= nil and v.ammo ~= nil then
	...
	while true do
		...
		if IsPedShooting(ped) then
			...
			if throwableWeapons[weapon] then
				if exports["ffrp-inventory"]:hasEnoughOfItem(weapon,1,false) then
		...
		if IsControlJustReleased(0,157) or IsDisabledControlJustPressed(0, 157) and not Disabled then
			TriggerEvent("inventory-bind",1)
		end

		if IsControlJustReleased(0,158) or IsDisabledControlJustPressed(0, 158) and not Disabled then
			TriggerEvent("inventory-bind",2)
		end

		if IsControlJustReleased(0,160) or IsDisabledControlJustPressed(0, 160) and not Disabled then
			TriggerEvent("inventory-bind",3)
		end

		if IsControlJustReleased(0,164) or IsDisabledControlJustPressed(0, 164) and not Disabled then
			TriggerEvent("inventory-bind",4)
		end

		if IsControlJustPressed(0,Controlkey["actionBar"][1]) or IsDisabledControlJustPressed(0,Controlkey["actionBar"][1]) and not Disabled then
			TriggerEvent("inventory-bar",true)
		end

		if IsControlJustReleased(0,Controlkey["actionBar"][1]) or IsDisabledControlJustReleased(0,Controlkey["actionBar"][1]) and not Disabled then
		...
		if IsPedPlantingBomb(ped) then
			if exports["ffrp-inventory"]:hasEnoughOfItem("741814745",1,false) then
...
RegisterNetEvent('ffrp-actionbar:disable')
AddEventHandler('ffrp-actionbar:disable', function(toggle)
	Disabled = toggle
end)
...
AddEventHandler('equipWeaponID', function(hash,newInformation,sqlID)
	...
	local dead = exports["ffrp-death"]:GetDeath()
	if dead then
		return
	end
...
AddEventHandler('actionbar:ammo', function(hash,amount,addition)
	...
	if ammoTable == nil then
		TriggerServerEvent("ffrp-weapons:getAmmo")
		Citizen.Wait(2000)
	end
...
function updateAmmoClient()
	...
	TriggerEvent("ffrp-weapons:SetAmmo",ammoTable)
end
...
function updateAmmoNow()
	...
	if type(ammoType) == 'number' then
		...
		TriggerServerEvent("ffrp-weapons:UpdateAmmo",newammo,ammoType,ammoTable)
	end
	return newammo
end
...
RegisterNetEvent('ffrp-item:CheckClientAmmo')
AddEventHandler('ffrp-item:CheckClientAmmo', function(weapons)
	local ped = PlayerPedId()
...
function AttachmentCheck(weaponhash)
	...
	if exports["ffrp-inventory"]:hasEnoughOfItem("silencer_l",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_AR_SUPP` )
	end

	if exports["ffrp-inventory"]:hasEnoughOfItem("silencer_l2",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_AR_SUPP_02` )
	end

	if exports["ffrp-inventory"]:hasEnoughOfItem("silencer_s",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_PI_SUPP` )
	end

	if exports["ffrp-inventory"]:hasEnoughOfItem("silencer_s2",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_PI_SUPP_02` )	
	end

	if exports["ffrp-inventory"]:hasEnoughOfItem("extended_ap",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_APPISTOL_CLIP_02` )	
	end

	if exports["ffrp-inventory"]:hasEnoughOfItem("extended_sns",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_SNSPISTOL_CLIP_02` )	
	end

	if exports["ffrp-inventory"]:hasEnoughOfItem("extended_micro",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_MICROSMG_CLIP_02` )	
	end

	if exports["ffrp-inventory"]:hasEnoughOfItem("MediumScope",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_SCOPE_MEDIUM` )	
	end

	if exports["ffrp-inventory"]:hasEnoughOfItem("SmallScope",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_SCOPE_SMALL` )	
	end


	if exports["ffrp-inventory"]:hasEnoughOfItem("TinyScope",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_SCOPE_MACRO` )	
	end

	if exports["ffrp-inventory"]:hasEnoughOfItem("extended_tec9",1,false) then
