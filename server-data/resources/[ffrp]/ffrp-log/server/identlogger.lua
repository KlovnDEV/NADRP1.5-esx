FFRP            = {}
FFRP.MySQLRady  = false
MySQL.ready(function()
    FFRP.MySQLRady = true
end)

AddEventHandler('playerConnecting', function(name, setCallback, deferrals)
    FFRP.PlayerConnecting(source, setCallback, deferrals)
end)

FFRP.PlayerConnecting = function(playerId, setCallback, deferrals)
    local mySqlState = FFRP.MySQLRady

    deferrals.defer()
    deferrals.update('[ffrp-log] Updating player information...')

    Citizen.Wait(100)

    if (not mySqlState) then
        deferrals.update('[ffrp-log] Awaiting database connection...')
    end

    while not FFRP.MySQLRady do
        Citizen.Wait(10)
    end

    if (FFRP.MySQLRady and not mySqlState) then
        deferrals.update('[ffrp-log] Updating player information...')
        Citizen.Wait(100)
    end
    ...
    if (Config.AddNewIdentifiersOnly) then
        ...
        MySQL.Async.fetchAll(query, queryParams, function(results)
            ...
            deferrals.update('[ffrp-log] Player information updated...')
