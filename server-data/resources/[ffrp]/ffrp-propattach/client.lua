...
AddEventHandler('attachPropPerm', function(attachModelSent,boneNumberSent,x,y,z,xR,yR,zR)
	...
	exports["ffrp-ped"]:GlobalObject(attachedPropPerm)
...
Citizen.CreateThread(function()
	while true do
		if attachedPropPerm ~= 0 then
			sleep = 1000
			if #(GetEntityCoords(attachedPropPerm) - GetEntityCoords(PlayerPedId())) < 15.0 then
				if IsControlJustPressed(0, 161) or IsDisabledControlJustPressed(0, 161) then 
					TriggerEvent('event:control:propAttach', 0)
				end
				sleep = 0
			else
				sleep = 1000
...
AddEventHandler('event:control:propAttach', function(useID)
	...
	if attachedPropPerm ~= 0 then
		...
		if (GetHashKey(`WEAPON_UNARMED`) ~= GetSelectedPedWeapon(PlayerPedId())) then
...
AddEventHandler('attachPropPhone', function(attachModelSent,boneNumberSent,x,y,z,xR,yR,zR)
	...
	exports["ffrp-ped"]:GlobalObject(attachedPropPhone)
...
AddEventHandler('attachProp69', function(attachModelSent,boneNumberSent,x,y,z,xR,yR,zR)
	...
	exports["ffrp-ped"]:GlobalObject(attachedProp69)
...
attachPropList = {
	...
	["phone01"] = {
		["model"] = "prop_amb_phone", ["bone"] = 57005, ["x"] = 0.14,["y"] = 0.01,["z"] = -0.02,["xR"] = 110.0,["yR"] = 120.0, ["zR"] = -15.0
	},
	...
	["galaxy"] = {
		["model"] = "ba_prop_sign_galaxy", ["bone"] = 18905, ["x"] = 0.01,["y"] = 0.01,["z"] = 0.0,["xR"] = -255.0,["yR"] = -120.0, ["zR"] = 20.0
	},
...
RegisterNetEvent('attach:galaxy')
AddEventHandler('attach:galaxy', function()
	TriggerEvent("attachItemPerm","galaxy")
end)
