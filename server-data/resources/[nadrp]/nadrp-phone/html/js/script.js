...
$(document).ready(function() {
    ...
    $.post('http://nadrp-phone/getWeather', JSON.stringify({}));
    ...
    document.onkeyup = function(data) {
        ...
        if (data.which == 27) {
            $.post('http://nadrp-phone/close', JSON.stringify({}));
        }
    }
        ...
        if (action !== undefined) {
            switch (action) {
                case "yellow-pages-delete":
                    $.post('http://nadrp-phone/deleteYP', JSON.stringify({}));
                    break;
                case "racing-create":
                ...
                case "newPostSubmit":
                    e.preventDefault();
                    $.post('http://nadrp-phone/newPostSubmit', JSON.stringify({
                    ...
                    break;
                case "group-manage":
                    $.post('http://nadrp-phone/manageGroup', JSON.stringify({ GroupID: $(this).data('action-data') }));
                    break;
                case "btnTaskGang":
                    manageGroup = $(this).data('action-data');
                    $.post('http://nadrp-phone/btnTaskGang', JSON.stringify({}));
                    break;
                ...
                case "getCallHistory":
                    if (callStates[currentCallState] === "isCallInProgress" && currentContainer !== "incoming-call")
                        ...
                        $.post('http://nadrp-phone/' + action, JSON.stringify({}));
                    break;
                case "dabcoin":
                    //openBrowser('http://nadrp.online/dabcoin/login.php');
                    break;
                default:
                    $.post('http://nadrp-phone/' + action, JSON.stringify({}));
                break;
    ...
    window.addEventListener('message', function(event) {
        ...
        switch (item.openSection) {
            ...
            case "showOutstandingPayments":
                ...
                addOutstandingPayments(item.outstandingPayments, item.type);
                openContainer('outstanding-payments');
                break;
            /*case "showOutstandingHousePayments":
                $('.outstanding-house-payments-entries').empty();
                addOutstandingHousingPayments(item.outstandingPayments);
                openContainer('outstanding-payments');
                break;*/
            case "manageKeys":
            ...
            case "racing:hud:update":
                switch (item.hudState) {
                    ...
                    case "finished":
                        ...
                        drawRaceStats();
                        $.post('http://nadrp-phone/race:completed', JSON.stringify({
        ...
        <div class="collapsible-body racing-entries-body center-align row">
            <div class="col s12">
                ...
                <div class="row no-padding">
                    <div class="col s12">
            `
        raceElement += `<button class="waves-effect waves-light btn racing-entries-entrants" data-id="${race.eventId}" aria-label="Race information" data-balloon-pos="up"><i class="fas fa-info icon"></i></button> `

        if (race.open)
            raceElement += `<button class="waves-effect waves-light btn green racing-entries-join" data-id="${race.eventId}" aria-label="Join race" data-balloon-pos="up"><i class="fas fa-flag-checkered icon"></i></button> `
            ...
            <li class="collection-item">
                <div class="row no-padding">
                    <div class="col s9" aria-label="${keys[key].sharedName}" data-balloon-pos="down">
                        <span  class="truncate" style="font-weight:bold">${keys[key].sharedName + ""}</span>
                    </div>
                    <div class="col s3 right-align">
                        <span class="phone-button manage-keys-remove" data-target-id="${keys[key].identifier}" aria-label="Remove Key" data-balloon-pos="left"><i class="fas red-text fa-user-times fa-2x"></i></span>
                    </div>
                </div>
                <div class="row no-padding">
                    <div class="col s12">
                        <span>Citizen ID: ${keys[key].identifier}</span>
                    </div>
                </div>
            </li>
        `
...
function addOutstandingPayments(payments, type) {
    for (let payment in Object.keys(payments)) {
            ...
            <ul>
                <div class="row no-padding">
                    <div class="col s12 outstanding-payment-entry">
                        ...
                        <h3>Shop: ${payments[payment].shop}</h3>
        ...
        `
            ...
            if (type === 'realestate') {
                var lastPayment = Math.ceil(7 - payments[payment].days)
                if (lastPayment == 0)
                    paymentString = "Today";
                else if (lastPayment < 0)
                    paymentString = `Payment was due ${Math.abs(lastPayment)} days ago.`
                else
                    paymentString = `${lastPayment} until payment is due.`

                    var element = `
                        <ul>
                            <div class="row no-padding">
                                <div class="col s12 outstanding-payment-entry">
                                    <h2><u>${payments[payment].house_name}</u></h2>
                                    <h3>Owner: ${payments[payment].name}</h3>
                                    <h3>Lastpayment: ${paymentString}</h3>
                                    <h3>Amount Owed: ${payments[payment].amountOwed}</h3>
                                </div>
                            </div>
                        </ul>
                    `
                        ...
                        <div class="card-content group-manage-entry-content">
                            <div class="row no-padding">
                                <div class="col s12">
                                    ...
                                    <span class="group-manage-entry-body">Rank [${employee.rank}]</span>
                                </div>
                            </div>
            ...
            if (keyType === "ownedKeys") {
                ...
                if (key.rent_due > 1)
                    ...
                    keyElement += `
                                ...
                                <li class="collection-item"><i class="fas fa-money-check-alt"></i> You owe $${amountdue}</li>
                            `
                }
                    keyElement += `
                                    </ul>
...
function openBrowser(url) {
    ...
    closePhoneShell();
    $.post('http://nadrp-phone/btnCamera', JSON.stringify({}));
    $("#browser").fadeIn(300);
}
...
function addVehicles(vehicleData, showCarPayments) {
    ...
    for (let vehicle of Object.keys(vehicleData)) {
        ...
        let carBGColor = "";
        if (vehicleData[vehicle].finance > 0)
            carIconColor = "red";
        else if (vehicleData[vehicle].finace == 0 && vehicleData[vehicle].payments > 0)
            carIconColor = "orange";
        else
            carIconColor = "green";
                    ...
                    <div class="row">
                        <div class="col s12">
                            <ul class="collection">
                                ...
                                if (vehicleData[vehicle].finance == 0) {
                                    vehicleElement+= `
                                    <li class="collection-item"><i class="fas fa-hourglass-half"></i> No remaining payments.</li>
                                    `
                                } else if (vehicleData[vehicle].finance > 0 && Math.ceil(7 - parseFloat(vehicleData[vehicle].lastPayment)) >= 0) {
                                    vehicleElement+=`
                                        <li class="collection-item"><i class="fas fa-hourglass-half"></i> ${Math.ceil(7 - parseFloat(vehicleData[vehicle].lastPayment))} days until payment is due.</li>
                                    `
                                } else if (vehicleData[vehicle].finance > 0 && Math.ceil(7 - parseFloat(vehicleData[vehicle].lastPayment)) < 0 ){
                                    vehicleElement+=`
                                        <li class="collection-item"><i class="fas fa-hourglass-half"></i> Payment was due ${Math.abs((7 - parseFloat(vehicleData[vehicle].lastPayment)))} days ago.</li>
                                    `
                                }
        if (vehicleData[vehicle].finance != 0) {
            vehicleElement += `
        ...
        if (Math.ceil(7 - parseFloat(vehicleData[vehicle].lastPayment)) <= 0 && vehicleData[vehicle].finance > 0)
...
function addMessage(item) {
    ...
    element.click(function () {
        $.post('http://nadrp-phone/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: item.msgDisplayName }));
    });
...
function addMessageOther(item) {
    ...
    element.id = item.id;
    element.click(function () {
        $.post('http://nadrp-phone/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: receiver, clientPhone: item.clientNumber }));
    });
    $(".messages-entries").prepend(element);
}
...
function updateSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            ...
            break;
        case "control":
            $.post('http://nadrp-phone/settingsUpdateToko', JSON.stringify({tag: "controlUpdate",controls: currentBinds}));
            break;
        case "browser":
            break;
        default:
...
function ResetSettings()
{
    switch (currentSettingWindow) {
        case "tokovoip":
            $.post('http://nadrp-phone/settingsResetToko', JSON.stringify());
            break;
        case "control":
            $.post('http://nadrp-phone/settingsResetControls', JSON.stringify());
            break;
        case "browser":
            break;
        default:
...
async function updateTokoSettings()
{
    ...
    await delayedLog();
    $.post('http://nadrp-phone/settingsUpdateToko', JSON.stringify({
        tag: "settings",
...
$('#racing-create-form').on('reset', function (e) {
    $.post('http://nadrp-phone/racing:map:cancel', JSON.stringify({}));
});

$('#racing-start-tracks').on('change', function (e) {
    let selectedMap = $(this).val();
    console.log(selectedMap, JSON.stringify(maps[selectedMap]))
    if(maps[selectedMap] !== undefined) {
        $.post('http://nadrp-phone/racing:map:removeBlips', JSON.stringify({}));
        $.post('http://nadrp-phone/racing:map:load', JSON.stringify({ id: selectedMap}));
...
$('#racing-start').submit(function (e) {
    ...
    $.post('http://nadrp-phone/racing:event:start', JSON.stringify({
...
$('#racing-create-form').submit(function (e) {
    e.preventDefault();
    $.post('http://nadrp-phone/racing:map:save', JSON.stringify({
...
$("#real-estate-sell-form").submit(function (e) {
    e.preventDefault();
    $.post('http://nadrp-phone/btnAttemptHouseSale', JSON.stringify({
...
$('#real-estate-transfer-form').submit(function (e) {
    e.preventDefault();
    $.post('http://nadrp-phone/btnTransferHouse', JSON.stringify({
...
$("#group-manage-pay-form").submit(function(e) {
    ...
    $.post('http://nadrp-phone/payGroup', JSON.stringify({
...
$("#group-manage-rank-form").submit(function (e) {
    e.preventDefault();
    $.post('http://nadrp-phone/promoteGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
$("#group-manage-bank-form").submit(function (e) {
    ...
    $.post('http://nadrp-phone/bankGroup', JSON.stringify({
        gangid: escapeHtml($(".group-manage-company-name").data('group-id')),
...
/*$("#group-manage-fire-form").submit(function (e) {
    e.preventDefault();
    $.post('http://nadrp-phone/fireEmployee', JSON.stringify({
...
$("#group-tasks-assign-modal-form").submit(function (e) {
    ...
    $.post('http://nadrp-phone/btnGiveTaskToPlayer', JSON.stringify({
...
$("#contacts-form").submit(function (e) {
    ...
    $.post('http://nadrp-phone/newContactSubmit', JSON.stringify({
        name: clean,
...
$("#stock-form").submit(function (event) {
    event.preventDefault();
    $.post('http://nadrp-phone/stocksTradeToPlayer', JSON.stringify({
...
$("#twat-form").submit(function (event) {
    event.preventDefault();
    $.post('http://nadrp-phone/newTwatSubmit', JSON.stringify({
...
$("#call-form").submit(function (event) {
    event.preventDefault();
    $.post('http://nadrp-phone/callContact', JSON.stringify({
        name: '',
...
$("#new-message-form").submit(function (event) {
    ...
    $.post('http://nadrp-phone/newMessageSubmit', JSON.stringify({
    ...
    switch (currentContainer) {
        case "message":
            setTimeout(function () {
                ...
                $.post('http://nadrp-phone/messageRead', JSON.stringify({ sender: sender, receiver: receiver, displayName: displayName }));
            }, 300);
            break;
        case "messages":
            setTimeout(function () {
                $.post('http://nadrp-phone/messages', JSON.stringify({}));
            }, 300);
            break;
    }
...
$('.racing-map-delete-confirm').click(function () {  
    ...
    $.post('http://nadrp-phone/racing:map:delete', JSON.stringify({ id: raceMap }));
    $('.racing-delete-confirm').fadeOut(150)
...
$('#real-estate-evict-modal-accept').click(function () {
    $.post('http://nadrp-phone/btnEvictHouse', JSON.stringify({}));
    $('#real-estate-evict-modal').modal('close');
});


$(".btnPropertyOutstanding").click(function(){
    $.post('http://nadrp-phone/btnPropertyOutstanding', JSON.stringify({}));
});
...
$('#group-manage-fire-accept').click(function () {
    ...
    $.post('http://nadrp-phone/groupFire', JSON.stringify({id: id, rank: rank}));
    $('#group-manage-fire-modal').modal('close');
...
$('.twatter-toggle-notification').click(function () {
    ...
    $.post('http://nadrp-phone/btnNotifyToggle', JSON.stringify({}));
});

$('.account-information-toggle-pager').click(function () {
    $.post('http://nadrp-phone/btnPagerToggle', JSON.stringify({}));
...
$('.racing-entries').on('click', '.racing-entries-entrants', function () {
    ...
    console.log(JSON.stringify(currentRace.racers))
    ...
    for (let id in currentRace.racers) {
        ...
        console.log(racer.fastest,racer.name, racer.total)
        let racerElement = `
            <li>
                ...
                <div class="collapsible-body">
                    <div class="row">
                        ...
                        <div class="col s9">
                            ...
                            <br>${secondsToTime(racer.fastest)}
                        </div>
                    </div>
                    <div class="row">
                        <div class="col s3 right-align">
...
$('.racing-entries').on('click', '.racing-entries-join', function () {
    ...
    $.post('http://nadrp-phone/racing:event:join', JSON.stringify({ identifier: $(this).data('id') }));
});

$('.keys-entries').on('click', '.manage-keys', function () {
    $.post('http://nadrp-phone/retrieveHouseKeys', JSON.stringify({}));
});

$('.keys-entries').on('click', '.remove-shared-key', function(e) {
    $.post('http://nadrp-phone/removeSharedKey', JSON.stringify({
...
$('.manage-keys-entries').on('click', '.manage-keys-remove', function () {
    $.post('http://nadrp-phone/removeHouseKey', JSON.stringify({
...
$('.group-tasks-entries').on('click', '.group-tasks-track', function () {
    $.post('http://nadrp-phone/trackTaskLocation', JSON.stringify({ TaskIdentifier: $(this).data('id') }));
});

$('.delivery-job-entries').on('click', '.delivery-job-accept', function (e) {
    $.post('http://nadrp-phone/selectedJob', JSON.stringify({ jobType: $(this).data('job-type'), jobId: $(this).data('job-id') }));
});
...
$('.garage-entries').on('click', '.garage-spawn', function (e) {
    e.preventDefault();
    $.post('http://nadrp-phone/vehspawn', JSON.stringify({ vehplate: $(this).data('plate') }));
    $.post('http://nadrp-phone/btnGarage', JSON.stringify({}));
});

$('.garage-entries').on('click', '.garage-track', function () {
    $.post('http://nadrp-phone/vehtrack', JSON.stringify({ vehplate: $(this).data('plate') }));
});

$('.garage-entries').on('click', '.garage-pay', function (e) {
    $.post('http://nadrp-phone/vehiclePay', JSON.stringify({ vehiclePlate: $(this).data('plate') }));
    setTimeout(function () {
        $.post('http://nadrp-phone/btnGarage', JSON.stringify({}));
    }, 1500);
});
...
$('.gps-entries, .keys-entries').on('click', '.gps-location-click', function () {
    $.post('http://nadrp-phone/loadUserGPS', JSON.stringify({ house_id: $(this).data('house-id'), house_type: $(this).data('house-type') }));
})

$('.contacts-entries, .call-history-entries').on('click', '.btn-contacts-call', function () {
    $.post('http://nadrp-phone/callContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
});
...
$('#confirm-modal-accept').click(function (event) {
    $.post('http://nadrp-phone/removeContact', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
