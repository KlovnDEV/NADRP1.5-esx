...
let objectDumps = [
	...
	{ objectID: -1251197000, Description: "Safe" },
];
...
on("onResourceStart", async (resource) => {
    if (resource == GetCurrentResourceName()){
		cid = exports["nadrp-ped"].isPed("steamid")
		if (cid = "") {
...
on('playerSpawned', (broughtData) => {
	let cid = exports["nadrp-ped"].isPed("steamid")
...
function RemoveItem(id,amount) {
	...
	emitNet("server-remove-item",cid,id,amount,pId)
}
		...
		if (GetVehicleDoorLockStatus(vehicleFound) == 2 && distanceRear < 1.5) {
			...
			if (distanceRear > 1.5) {
				...
				if(licensePlate != null){
					...
					let foodtruck = false
					if(exports["nadrp-ped"].isPed("myjob") == "taco") {
						foodtruck = true
					}
...
function drawMarkersUI() {
	for (let Row in DrawInventories) {
		DrawMarker(20,DrawInventories[Row].position.x, DrawInventories[Row].position.y, DrawInventories[Row].position.z-0.8,0,0,0,0,0,0,0.35,0.5,0.15,255,50,50,91,0,0,0,0)
	}	
}
...
on('nadrp-weapons:SetAmmo', (sentammoTable) => {
	if (sentammoTable) {
...
on('inventory-bind', (slot) => {
	if(exports["nadrp-death"].GetDeath()){return;}
	let cid = exports["nadrp-ped"].isPed("steamid")
...
function OpenGui() {
	...
	openedInv = true
	emitNet('inventory:updateCash')
	SendNuiMessage( JSON.stringify({ response: "openGui" })) 
	SetNuiFocus(true, true)
	weaponsLicence = exports["nadrp-ped"].isPed("weaponsLicence")
	cash = exports["nadrp-ped"].isPed("mycash")

	let brought = hadBrought[cid]
	let cop = false
	let job = exports["nadrp-ped"].isPed("myjob")
	if(job == "police") {
		cop = true
	}
	setTimeout(()=>{
		console.log('UPDATE CASH NOW #'+cash)
...
on('toggle-animation', (toggleAnimation) => {
	...
	if(toggleAnimation) {
		TriggerEvent("animation:load", "mini@repair")
