...
local gasStations = {
    ...
    {2810.485,-3699.464,400},
    {2800.692,-3703.801,140.001},
    {2770.122,-3699.369,140.001},
    {2830.871, -3700.348,140.001}
...
AddEventHandler("RefuelCarServerReturn",function()
    ...
    local timer = (100 - curFuel) * 350
    refillVehicle()
    local finished = exports["nadrp-taskbar"]:taskBar(timer,"Refueling")
            ...
            if GetPedInVehicleSeat(veh, -1) == player then
                ...
                if Fuel > 0 then
                    if IsVehicleEngineOn(veh) then
                        ...
                        if algofuel > 140 then
                            algofuel = algofuel * 2.3
                        else
                            algofuel = algofuel / 1.6
                        end
...
Citizen.CreateThread(function()
    while true do
        ...
        for i=1, #gasStations, 1 do
            if #(vector3(gasStations[i][1], gasStations[i][2],gasStations[i][3]) - vector3(pos.x,pos.y,pos.z)) < 22.0 then
                local veh = TargetVehicle()
                if DoesEntityExist(veh) and IsEntityAVehicle(veh) and #(GetEntityCoords(veh) - GetEntityCoords(PlayerPedId())) < 5.0 then
                    ...
                    info = string.format("Press ~b~"..Controlkey["generalUse"][2].."~s~ to refuel your vehicle | ~b~$%s", costs)
                    local crd = GetEntityCoords(veh)
                    DrawMarker(36,crd["x"],crd["y"],crd["z"]+1.5, 0, 0, 0, 0, 0, 0, 1.0, 1.0, 1.0, 15, 100, 15, 130, 1, 0, 0, 1)
