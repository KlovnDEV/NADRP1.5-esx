...
AddEventHandler("nadrp:playerBecameJob", function(jobpassed, name, notify)
	job = jobpassed.name
    if not job then
        job = "None"
    end
...
RegisterNetEvent('nadrp-weapons:SetAmmo')
AddEventHandler('nadrp-weapons:SetAmmo', function(sentammoTable)
	...
	while true do
		...
		if IsPedShooting(ped) then
			...
			if throwableWeapons[weapon] then
				if exports["nadrp-inventory"]:hasEnoughOfItem(weapon,1,false) then
		...
		if IsPedPlantingBomb(ped) then
			if exports["nadrp-inventory"]:hasEnoughOfItem("741814745",1,false) then
...
RegisterNetEvent('nadrp-actionbar:disable')
AddEventHandler('nadrp-actionbar:disable', function(toggle)
	Disabled = toggle
end)
...
AddEventHandler('equipWeaponID', function(hash,newInformation,sqlID)
	...
	local dead = exports["nadrp-death"]:GetDeath()
	if dead then
		return
	end
...
function updateAmmoClient()
	...
	TriggerEvent("nadrp-weapons:SetAmmo",ammoTable)
end
...
function updateAmmoNow()
	...
	if type(ammoType) == 'number' then
		...
		TriggerServerEvent("nadrp-weapons:UpdateAmmo",newammo,ammoType,ammoTable)
	end
	return newammo
end
...
RegisterNetEvent('nadrp-item:CheckClientAmmo')
AddEventHandler('nadrp-item:CheckClientAmmo', function(weapons)
	local ped = PlayerPedId()
...
function AttachmentCheck(weaponhash)
	...
	if exports["nadrp-inventory"]:hasEnoughOfItem("silencer_l",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_AR_SUPP` )
	end

	if exports["nadrp-inventory"]:hasEnoughOfItem("silencer_l2",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_AR_SUPP_02` )
	end

	if exports["nadrp-inventory"]:hasEnoughOfItem("silencer_s",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_PI_SUPP` )
	end

	if exports["nadrp-inventory"]:hasEnoughOfItem("silencer_s2",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_PI_SUPP_02` )	
	end

	if exports["nadrp-inventory"]:hasEnoughOfItem("extended_ap",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_APPISTOL_CLIP_02` )	
	end

	if exports["nadrp-inventory"]:hasEnoughOfItem("extended_sns",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_SNSPISTOL_CLIP_02` )	
	end

	if exports["nadrp-inventory"]:hasEnoughOfItem("extended_micro",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_MICROSMG_CLIP_02` )	
	end

	if exports["nadrp-inventory"]:hasEnoughOfItem("MediumScope",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_SCOPE_MEDIUM` )	
	end

	if exports["nadrp-inventory"]:hasEnoughOfItem("SmallScope",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_SCOPE_SMALL` )	
	end


	if exports["nadrp-inventory"]:hasEnoughOfItem("TinyScope",1,false) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, `COMPONENT_AT_SCOPE_MACRO` )	
	end

	if exports["nadrp-inventory"]:hasEnoughOfItem("extended_tec9",1,false) then
