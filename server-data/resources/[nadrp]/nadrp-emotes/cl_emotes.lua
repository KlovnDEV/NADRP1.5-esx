...
function StressTest(animName)
    if (animName == "pushups" or animName == "situps" or animName == "chinups" or
        ...
        (#(GetEntityCoords(PlayerPedId()) - vector3(1777.2374267578,2481.0405273438,45.59899520874)) <
            10.0 or
            #(GetEntityCoords(PlayerPedId()) - vector3(-1201.4105224609,-1570.5053710938,4.6082391738892)) <
            10.0) then
        local finished = exports["drp-taskbar"]:taskBar(15000, "Relieving Stress")
            TriggerEvent('drp-status:remove','stress',1*10000)
            playing_emote = false
...
AddEventHandler('notepad', function()
    ...
    if GetPedInVehicleSeat(veh, -1) ~= PlayerPedId() then
        ...
        TaskPlayAnim(PlayerPedId(), "amb@medic@standing@timeofdeath@base","base", 8.0, 0.01, -1, 49, 0, 0, 0, 0)
        Citizen.Wait(1000)
        ...
        TaskPlayAnim(PlayerPedId(), "amb@medic@standing@timeofdeath@exit","exit", 0.1, 1.0, 1.0, 48, 0, 0, 0, 0)
        Citizen.Wait(5000)
        TriggerEvent("destroyProp")
        TriggerEvent("destroyProp69")
