...
RegisterServerEvent('nadrp-log:AdminLog')
AddEventHandler('nadrp-log:AdminLog', function(name, identifier, type, log)
...
RegisterServerEvent('nadrp-log:ExploiterLog')
AddEventHandler('nadrp-log:ExploiterLog', function(name,identifier,type,log)
    if log == nil then return end
    if type == nil then type = 'error' end
    if identifier == nil then identifier = ESX.GetPlayerFromId(source)[1].identifier end
...
RegisterServerEvent('nadrp-log:BusinessLog')
AddEventHandler('nadrp-log:BusinessLog', function(name,identifier,type,log,business)
    if log == nil then return end
    if type == nil then type = 'error' end
    if identifier == nil then identifier = ESX.GetPlayerFromId(source)[1].identifier end
...
RegisterServerEvent('nadrp-log:AddHousingLog')
AddEventHandler('nadrp-log:AddHousingLog', function(name,identifier,type,log,house_name)
    if log == nil then return end
    if type == nil then type = 'error' end
    if identifier == nil then identifier = ESX.GetPlayerFromId(source)[1].identifier end
...
RegisterNetEvent('nadrp-log:SendDiscordLog')
AddEventHandler('nadrp-log:SendDiscordLog', function(data)
    local src = source
    if data ~= nil then
...
function DiscordLog(data)
    ...
    local discordInfo = {
        ...
        ["footer"] = {
            ["text"] = 'nadrp-LOG | 1.0.0'
        }
    }

    PerformHttpRequest(Config.WebHook, function(err, text, headers) end, 'POST', json.encode({ username = 'nadrp-LOG', embeds = { discordInfo } }), { ['Content-Type'] = 'application/json' })
end
    ...
    if discordId then
        ...
        if member.code == 200 then
            ...
            if data ~= nil then
                ...
                print("[nadrp-log] ERROR: Couldnt get discord data")
            end
        else
            print("[nadrp-log] ERROR: Code 200 was not reached. Error provided: " .. member.data)
        end
    end
end, true)
