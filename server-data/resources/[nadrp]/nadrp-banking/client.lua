...
RegisterCommand('atm', function(source, args, raw)
    TriggerEvent('nadrp-bank:checkATM')
end)

RegisterCommand('givecash', function(source, args)
    local toPlayer = args[1]
    local amount = args[2]
    TriggerEvent('nadrp-bank:givecash', toPlayer, amount)
    TaskPlayAnim(PlayerPedId(), 'friends@laf@ig_5', 'nephew', 5.0, 1.0, 5.0,48, 0.0, 0, 0, 0)
end)

RegisterNetEvent('nadrp-bank:checkATM')
AddEventHandler('nadrp-bank:checkATM', function()
  if IsNearATM() then
    atmOpen = true
    openGui()
    bankOpen = true
...
function openGui()
    ...
    ESX.TriggerServerCallback('getCash', function (amount)
        if amount then
            TriggerEvent("nadrp-banking:viewCash", amount)
        else
            print('3')
        end
    end)
    ...
    if IsNearATM() then
      if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
          ...
          if ( atmuse ) then 
              ...
              atmuse = false
              local finished = exports["nadrp-taskbar"]:taskBar(3000,"Retrieving Card")
              ClearPedTasks(GetPlayerPed(-1))
          else
              atmuse = true
        ...
        if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
            ...
            if ( atmuse ) then 
                ...
                atmuse = false
                local finished = exports["nadrp-taskbar"]:taskBar(1000,"Retrieving Card")
                ClearPedTasks(GetPlayerPed(-1))
            else
                atmuse = true
                TaskPlayAnim( player, "mp_common", "givetake1_a", 1.0, 1.0, -1, 49, 0, 0, 0, 0 )
  ...
  while true do 
    ...
    for i=1, #closedBanks do
      if closedBanks[i]["closed"] then
        ...
        timer = timer - 60000
        if timer <= 0 then 
          TriggerServerEvent('nadrp-banking:resetClosedBank', i)
        end
      end
    end
  end
...
RegisterNUICallback('quickCash', function(data, cb)
  TriggerEvent('nadrp-bank:withdraw', 100)
  cb('ok')
end)
...
RegisterNUICallback('transferSubmit', function(data, cb)
  ...
  TriggerEvent('nadrp-bank:transfer', tonumber(fromPlayer), tonumber(data.toPlayer), tonumber(data.amount))
  cb('ok')
end)
...
RegisterNetEvent('nadrp-banking:updateClosedBank')
AddEventHandler('nadrp-banking:updateClosedBank', function(newBanks)
  closedBanks = newBanks
end)

-- Process deposit if conditions met
RegisterNetEvent('nadrp-bank:deposit')
...
RegisterNetEvent('nadrp-bank:withdraw')
...
RegisterNetEvent('nadrp-dirtyMoney:givedm')
AddEventHandler('nadrp-dirtyMoney:givedm', function(toPlayer, amount)
...
RegisterNetEvent('nadrp-bank:transfer')
AddEventHandler('nadrp-bank:transfer', function(fromPlayer, toPlayer, amount)
...
RegisterNetEvent('nadrp-banking:updateBalance')
AddEventHandler('nadrp-banking:updateBalance', function(balance, show)
    local id = PlayerId()
...
RegisterNetEvent('nadrp-banking:updateCash')
AddEventHandler('nadrp-banking:updateCash', function(balance, show)
	SendNUIMessage({
		updateCash = true,
		cash = balance,
    show = show
...
RegisterNetEvent("nadrp-banking:viewBalance")
AddEventHandler("nadrp-banking:viewBalance", function()
  SendNUIMessage({
    viewBalance = true
  })
end)
...
RegisterNetEvent("nadrp-banking:addBalance")
AddEventHandler("nadrp-banking:addBalance", function(amount)
	SendNUIMessage({
		addBalance = true,
		amount = amount
	})
...
RegisterNetEvent("nadrp-banking:removeBalance")
AddEventHandler("nadrp-banking:removeBalance", function(amount)
	SendNUIMessage({
		removeBalance = true,
		amount = amount
	})
...
RegisterNetEvent("nadrp-banking:addCash")
AddEventHandler("nadrp-banking:addCash", function(amount)
	SendNUIMessage({
		addCash = true,
		amount = amount
	})
...
RegisterNetEvent("nadrp-banking:removeCash")
AddEventHandler("nadrp-banking:removeCash", function(amount)
	SendNUIMessage({
		removeCash = true,
		amount = amount
	})
...
AddEventHandler("es:addedMoney", function(amt, total)
    TriggerEvent("nadrp-banking:updateCash", total)
    TriggerEvent("nadrp-banking:addCash", amt)
    ESX.TriggerServerCallback('getCash', function (amount)
        if amount then 
            TriggerEvent("nadrp-banking:viewCash", amount)
        else
...
AddEventHandler("es:removedMoney", function(amt, total)
    TriggerEvent("nadrp-banking:updateCash", total)
    TriggerEvent("nadrp-banking:removeCash", amt)
    ESX.TriggerServerCallback('getCash', function (amount)
        if amount then
            TriggerEvent("nadrp-banking:viewCash", amount)
        end
...
RegisterNetEvent("nadrp-banking:viewCash")
AddEventHandler("nadrp-banking:viewCash", function(amountt)
    amount = amountt
    SendNUIMessage({
        viewCash = true,
        cash = amountt
