    ...
    while true do
		if IsControlPressed(0, 21) and job == 'police' then
			...
			if IsControlJustPressed(0,27) then
				TriggerEvent('nadrp-policejob:hotkeys', 1)
			elseif IsControlJustPressed(0,173) then
				TriggerEvent('nadrp-policejob:hotkeys', 2)
			elseif IsControlJustPressed(0,174) then
				TriggerEvent('nadrp-policejob:hotkeys', 3)
			elseif IsControlJustPressed(0,175) then
				TriggerEvent('nadrp-policejob:hotkeys', 4)
			end
		else
			Citizen.Wait(500)
		end
...
RegisterNetEvent('nadrp-policejob:hotkeys')
AddEventHandler('nadrp-policejob:hotkeys', function(key)
    local inVeh = IsPedInAnyVehicle(PlayerPedId())
    if job == 'police' and not IsHandcuffed and not inVeh then
        if not inAction then
            if key ~= nil then
                if key == 1 then
                    TriggerEvent('nadrp-policejob:softcuff')
                    inAction = true
                elseif key == 2 then
                    TriggerEvent('nadrp-policejob:uncuff')
                    inAction = true
                elseif key == 3 then
...
RegisterNetEvent('nadrp-policejob:getarrested')
AddEventHandler('nadrp-policejob:getarrested', function(isCommand, playerheading, playercoords, playerlocation)
	playerPed = PlayerPedId()
...
RegisterNetEvent('nadrp-policejob:doarrested')
AddEventHandler('nadrp-policejob:doarrested', function()
	Citizen.Wait(250)
	LoadAnimDict('mp_arrest_paired')
...
RegisterNetEvent('nadrp-policejob:douncuffing')
AddEventHandler('nadrp-policejob:douncuffing', function()
	Citizen.Wait(250)
	LoadAnimDict('mp_arresting')
...
RegisterNetEvent('nadrp-policejob:getuncuffed')
AddEventHandler('nadrp-policejob:getuncuffed', function(playerheading, playercoords, playerlocation)
	local x, y, z   = table.unpack(playercoords + playerlocation * 1.0)
	SetEntityCoords(PlayerPedId(), x, y, z)
	SetEntityHeading(PlayerPedId(), playerheading)
	Citizen.Wait(250)
	...
	TriggerEvent('nadrp-policejob:HandCuffedState', false, false)
end)

RegisterNetEvent('nadrp-policejob:loose')
AddEventHandler('nadrp-policejob:loose', function(playerheading, playercoords, playerlocation)
	local x,y,z  = table.unpack(playercoords + playerlocation * 1.0)
	SetEntityCoords(PlayerPedId(), x, y, z)
	SetEntityHeading(PlayerPedId(), playerheading)
	Citizen.Wait(250)
    ...
    TriggerEvent('nadrp-policejob:handcuff')
	ClearPedTasks(PlayerPedId())
	TriggerEvent('nadrp-policejob:HandCuffedState', true, true)
end)

RegisterNetEvent('nadrp-policejob:getarrestedhard')
AddEventHandler('nadrp-policejob:getarrestedhard', function(playerheading, playercoords, playerlocation)
	playerPed = PlayerPedId()
	SetCurrentPedWeapon(playerPed, GetHashKey('WEAPON_UNARMED'), true)
	local x, y, z   = table.unpack(playercoords + playerlocation * 1.0)
	SetEntityCoords(PlayerPedId(), x, y, z)
...
RegisterNetEvent('nadrp-policejob:hardcuff')
AddEventHandler('nadrp-policejob:hardcuff', function()
	local playerPed = PlayerPedId()
	Citizen.CreateThread(function()
		if IsShackles then
...
AddEventHandler('nadrp-policejob:drag', function(copID)
	DragStatus.IsDragged = not DragStatus.IsDragged
	DragStatus.CopId     = tonumber(copID)
end)

RegisterNetEvent('nadrp-policejob:resetDrag')
AddEventHandler('nadrp-policejob:resetDrag', function(copID)
	DragStatus.IsDragged = false
	DragStatus.CopId     = tonumber(copID)
end)
...
AddEventHandler('pd:radio', function ()
	local radio = exports["nadrp-inventory"]:hasEnoughOfItem("radio",1,false)
