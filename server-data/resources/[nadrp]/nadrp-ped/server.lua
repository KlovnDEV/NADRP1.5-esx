...
RegisterServerEvent('nadrp:setMetaData')
AddEventHandler('nadrp:setMetaData', function(data)
  local src = source
...
RegisterServerEvent('nadrp:GetMetaData')
AddEventHandler('nadrp:GetMetaData', function()
  local src = source
  local user = ESX.GetPlayerFromId(src)
  local ident = user.identifier
  MySQL.Async.fetchAll('SELECT metaData FROM users WHERE identifier=@identifier', {
    ...
    if result ~= nil then
      ...
      TriggerClientEvent('nadrp:LoadMetaData', user.source, data)
    end
  end)
end)
  ...
  MySQL.Async.fetchAll('SELECT callsign FROM characters WHERE identifier=@ident', {
    ...
    if result[1] ~= nil then
      ...
      if callsign ~= nil then
        TriggerClientEvent('nadrp-dispatch:myCallsign', player.source, tostring(callsign))
      end
    end
  end)
end)
