...
AddEventHandler( 'cell:doors', function(num)
    ...
    TriggerServerEvent("nadrp-doors:alterlockstate",tonumber(num))
end)
...
Citizen.CreateThread(function()
    ...
    plyCoords = GetEntityCoords(playerped)
    while true do
        for i = 1, #nadrp.DoorCoords do
            local distance = #(plyCoords - vector3(nadrp.DoorCoords[i]["x"], nadrp.DoorCoords[i]["y"], nadrp.DoorCoords[i]["z"]))
            if distance < distanceCheckStage1 then
                closeDoorsStage1[i] = nadrp.DoorCoords[i]
            else
    ...
    while true do
        ...
        if currentTargetDist < distvariable and targetDoor ~= nil then
            ...
            if cZ(objCoords["z"],targetDoor["z"],limit) then
                ...
                if(nadrp.offSet[targetDoor["doorType"]]) then
                    if (type(targetDoor["doorType"]) == "number") then
                        local pos = nadrp.offSet[targetDoor["doorType"]]
                            ...
                            if targetDoor["lock"] == 0 then
                                if targetDoor["keypad"] ~= nil then
                                    ...
                                    if targetDoor["skipSwing"] == true then
                                        TriggerServerEvent("nadrp-doors:alterlockstate",curClosestNum)
                                    else
                                        local active = true
                                        local swingcount = 0
                                        while active do
...
function OpenCheck(curClosestNum)
    local gangType = exports["nadrp-ped"]:isPed("gang")
    ...
    if nadrp.DoorCoords[curClosestNum]["keypad"] ~= nil then
        return true
    end

    local foundValid = false
    for k,v in pairs(nadrp.rankCheck) do
        local rank = exports["nadrp-ped"]:GroupRank(k)
...
function isKeyDoor(num)
    ...
    if nadrp.DoorCoords[num]["doorType"] == "prop_gate_prison_01" then
        return false
    end
    if nadrp.DoorCoords[num]["doorType"] == 1286392437 then
        return false
    end
    if nadrp.DoorCoords[num]["doorType"] == "v_ilev_fin_vaultdoor" then
        return false
    end
    if nadrp.DoorCoords[num]["doorType"] == -1603817716 then
        return false
    end
    if nadrp.DoorCoords[num]["doorType"] == 1501451068 then
        return false
    end
    if nadrp.DoorCoords[num]["doorType"] == -550347177 then
        return false
    end
    if nadrp.DoorCoords[num]["doorType"] == "gabz_pillbox_dlc_gate" then
        return false
    end
    if nadrp.DoorCoords[num]["doorType"] == "gabz_mrpd_bollards1" then
        return false
    end
    if nadrp.DoorCoords[num]["doorType"] == "gabz_mrpd_bollards2" then
        return false
    end
    if nadrp.DoorCoords[num]["doorType"] == "gabz_mrpd_garage_door" then
        return false
    end
    if nadrp.DoorCoords[num]["doorType"] == "prop_facgate_07b" then
        return false
    end
    if nadrp.DoorCoords[num]["doorType"] == "lost_mc_door_01" then
        return false
    end
    if nadrp.DoorCoords[num]["doorType"] == "lost_mc_gate" then
        return false
    end
    if nadrp.DoorCoords[num]["doorType"] == "v_ilev_carmod3door" then
        return false
    end
    return true
end
