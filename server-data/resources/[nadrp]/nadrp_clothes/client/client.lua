...
function Save(save)
    if save then
        data = GetCurrentPed()
        TriggerServerEvent("nadrp-clothes:insert_character_current", data)
        if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
            TriggerServerEvent("nadrp-clothes:insert_character_face", data)
            TriggerServerEvent("nadrp-clothes:set_tats", currentTats)
        end
    else
        LoadPed(oldPed)
    end
...
RegisterNetEvent("nadrp-clothes:inService")
AddEventHandler("nadrp-clothes:inService", function(service)
    isService = service
end)

RegisterNetEvent("nadrp-clothes:hasEnough")
AddEventHandler("nadrp-clothes:hasEnough", function(menu)
...
--[[RegisterCommand('spawnme', function(source, args)
    --print("spawned me")
--TriggerEvent("nadrp-clothes:register")
    OpenMenu("barbermenu")
end)]]

RegisterNetEvent("nadrp-clothes:setclothes")
AddEventHandler("nadrp-clothes:setclothes", function(data,alreadyExist)
    ...
    local function setDefault()
        Citizen.CreateThread(function()
            ...
            TriggerEvent("nadrp-clothes:defaultReset")
...
RegisterNetEvent("nadrp-clothes:defaultReset")
AddEventHandler("nadrp-clothes:defaultReset", function(sex)
    Citizen.Wait(1000)
    if sex == 'f' then
        SetSkin(`mp_f_freemode_01`, true)
    else
...
RegisterNetEvent("nadrp-clothes:settattoos")
AddEventHandler("nadrp-clothes:settattoos", function(playerTattoosList)
    currentTats = playerTattoosList
    SetTats(GetTats())
end)

RegisterNetEvent("nadrp-clothes:setpedfeatures")
AddEventHandler("nadrp-clothes:setpedfeatures", function(data)
    player = GetPlayerPed(-1)
    if data == false then
...
RegisterNetEvent('nadrp-clothes:outfits')
AddEventHandler('nadrp-clothes:outfits', function(pAction, pId, pName)
    if pAction == 1 then
        TriggerServerEvent("nadrp-clothes:set_outfit",pId, pName, GetCurrentPed())
    elseif pAction == 2 then
        TriggerServerEvent("nadrp-clothes:remove_outfit",pId)
    elseif pAction == 3 then
        TriggerEvent('item:deleteClothesDna')
        TriggerServerEvent("nadrp-clothes:get_outfit", pId)
    else
...
--new regiser
AddEventHandler('nadrp-clothes:register', function(name)
    OpenMenu("clothesmenu")
end)
...
RegisterCommand('outfitadd', function(source, args, raw)
    ...
    local name = args[2]
    local nearcloth = IsNearShop(clothingShops)
    local inMotel = exports['nadrp-motels']:nearClothingMotel()
    local inHouse = exports['nadrp_garages']:nearClothingHouse()
