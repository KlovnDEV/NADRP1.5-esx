...
AddEventHandler("AskForTrain", function()
    local finished = exports['nadrp-taskbar']:taskBar(60 * 1000, 'Requesting Train')
	...
	if metroDist < countryDist then
		if metroDist < 25.0 then 
            ...
            TriggerEvent('chat:addMessage', {
                template = '<div class="chat-message nonemergency">Thomas | {0} </div>',
                args = {str}
            });
        else
            local str = ' You are not near a train station..'
            TriggerEvent('chat:addMessage', {
                template = '<div class="chat-message nonemergency">Thomas | {0} </div>',
                args = {str}
            });
		...
		if(IsControlJustReleased(1,  38))then
			...
			if IsThisModelATrain( GetEntityModel(targetVehicle) ) and not intrain then
				...
				if (intrain or IsPedInAnyTrain(GetPlayerPed(-1))) and GetEntitySpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false)) * 3.6 then
					...
					--local speeds = GetEntitySpeed(GetVehiclePedIsUsing(GetPlayerPed(-1)))
...
function StartMetro(coords)
    ...
    local str =  ' Train will depart in 30 seconds.'
    TriggerEvent('chat:addMessage', {
        template = '<div class="chat-message nonemergency">Thomas | {0} </div>',
        args = {str}
    });
...
function StartCountry(coords)
	...
	local models = { "freight", "freightcar", "freightgrain", "freightcont1", "freightcont2", "freighttrailer", "tankercar", "metrotrain", "s_m_m_lsmetro_01" }
	for  _,model in ipairs(models) do
		tempmodel = GetHashKey(model)
		RequestModel(tempmodel)
		while not HasModelLoaded(tempmodel) do
