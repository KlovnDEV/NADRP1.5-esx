...
Citizen.CreateThread(function()
	...
	while tac.GetPlayerData().job2 == nil do
		Citizen.Wait(10)
	end
...
RegisterNetEvent('tac:setJob2')
AddEventHandler('tac:setJob2', function(job2)
...
function OpenCloakroomMenu()
	...
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		title = _U('cloakroom'),
		elements = elements
	}, function(data, menu)
		...
		if data.current.value == 'citizen_wear' then
			tac.TriggerServerCallback('tac_skin:getPlayerSkin', function(skin, jobSkin)
				TriggerEvent('skinchanger:loadSkin', skin)
			end)
		end

		if data.current.value == 'gang_wear' then
			tac.TriggerServerCallback('tac_skin:getPlayerSkin', function(skin, jobSkin, job2Skin)
				if skin.sex == 0 then
...
function OpenArmoryMenu()
	...
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'armory', {
		title = _U('armory'),
		elements = elements
	}, function(data, menu)
...
function OpenVehicleSpawnerMenu()
	...
	tac.TriggerServerCallback('tac_society:getVehiclesInGarage', function(vehicles)
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
			...
			tac.Game.SpawnVehicle(vehicleProps.model, vehSpawnPoint, vehSpawnHeading, function(vehicle)
				tac.Game.SetVehicleProperties(vehicle, vehicleProps)
			end)

			_TriggerServerEvent('tac_society:removeVehicleFromGarage', ActualGang.Name, vehicleProps)
		end, function(data, menu)
...
function OpenGangActionsMenu()
	...
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'gang_actions', {
		...
		if data.current.value == 'citizen_interaction' then
			tac.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if distance ~= -1 and distance <= 3.0 then
					...
					if data2.current.value == 'body_search' then
						...
						if isHandsup then
							...
							tac.ShowNotification('~r~Action Impossible~s~ : Le joueur cible doit lever les mains !')
						end
					end
		...
		if data.current.value == 'vehicle_interaction' then
			tac.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_interaction', {
						...
						if IsAnyVehicleNearPoint(coords, 3.0) then
							...
							if DoesEntityExist(vehicle) then
								Citizen.CreateThread(function()
									...
									tac.ShowNotification(_U('vehicle_unlocked'))
								end)
							end
						end
					end
...
function OpenBodySearchMenu(player)
	tac.TriggerServerCallback('GangsBuilder:getOtherPlayerData', function(data)
		if data.foundPlayer then
			...
			for i = 1, #data.weapons, 1 do
				table.insert(elements, {
					label = tac.GetWeaponLabel(data.weapons[i].name),
			...
			tac.UI.Menu.CloseAll()
			tac.ShowNotification('Le joueur a déconnecté vous ne pouvez pas le fouillez.')
		end
	end, player)
end

function OpenVehicleInfosMenu(vehicleData)
	tac.TriggerServerCallback('GangsBuilder:getVehicleInfos', function(infos)
		local elements = {}
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_infos', {
...
function OpenBuyWeaponsMenu()
	...
	for i = 1, #ActualGang.Weapons, 1 do
		table.insert(elements, {label = tac.GetWeaponLabel(ActualGang.Weapons[i].name), rightlabel = {'$' .. ActualGang.Weapons[i].price}, value = ActualGang.Weapons[i].name, price = ActualGang.Weapons[i].price})
	end

	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_buy_weapons', {
		title = _U('buy_weapon_menu'),
		elements = elements
	}, function(data, menu)
		tac.TriggerServerCallback('GangsBuilder:buyWeapon', function(hasEnoughMoney)
...
function OpenGetWeaponMenu()
	tac.TriggerServerCallback('GangsBuilder:getArmoryWeapons', function(weapons)
		local elements = {}

		for i = 1, #weapons, 1 do
			table.insert(elements, {label = tac.GetWeaponLabel(weapons[i].name), rightlabel = {'[' .. weapons[i].ammo .. ']'}, value = weapons[i].name, ammo = weapons[i].ammo})
		end
		...
		tac.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_get_weapon', {
			...
			tac.TriggerServerCallback('GangsBuilder:removeArmoryWeapon', function()
				OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
	...
	tac.UI.Menu.Open('default', GetCurrentResourceName(), 'armory_put_weapon', {
		...
		tac.TriggerServerCallback('GangsBuilder:addArmoryWeapon', function()
			OpenPutWeaponMenu()
		end, data.current.value, data.current.ammo)
	end, function(data, menu)
	end)
...
function OpenGetStocksMenu()
	tac.TriggerServerCallback('GangsBuilder:getStockItems', function(items)
		local elements = {}
...
function OpenPutStocksMenu()
	tac.TriggerServerCallback('GangsBuilder:getPlayerInventory', function(inventory)
		local elements = {}
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0, 38) and PlayerData.job2 ~= nil and ActualGang and (GetGameTimer() - GUI.Time) > 150 then
				...
				if CurrentAction == 'delete_vehicle' then
					...
					_TriggerServerEvent('tac_society:putVehicleInGarage', ActualGang.Name, vehicleProps)
				...
				if CurrentAction == 'menu_boss_actions' then
					...
					TriggerEvent('tac_society:openBossMenu2', ActualGang.Name, function(data, menu)
						CurrentAction = 'menu_boss_actions'
						CurrentActionMsg = _U('open_bossmenu')
						CurrentActionData = {}
					end, {wash = false})
		...
		if IsControlPressed(0, 168) and PlayerData.job2 ~= nil and ActualGang and not isDead and not tac.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'gang_actions') and (GetGameTimer() - GUI.Time) > 150 then
			OpenGangActionsMenu()
			GUI.Time = GetGameTimer()
		end
	end
