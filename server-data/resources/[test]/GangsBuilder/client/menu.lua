	...
	addGangMenu.SubMenu.OnItemSelect = function(_, item, index)
		...
		if item == confirmItem then
			if GangData.Name == nil then
				tac.ShowNotification('Aucun nom !')
				return
			end

			if GangData.Label == nil then
				tac.ShowNotification('Aucun label !')
				return
			end

			if GangData.Weapons == nil then
				tac.ShowNotification('Aucun kit d\'armes !')
				return
			end

			if GangData.Cloakroom == nil then
				tac.ShowNotification('Aucun vestiaire !')
				return
			end

			if GangData.Armory == nil then
				tac.ShowNotification('Aucune armurerie !')
				return
			end

			if GangData.VehSpawner == nil then
				tac.ShowNotification('Aucun point pour spawn véhicule !')
				return
			end

			if GangData.VehSpawnPoint == nil then
				tac.ShowNotification('Aucun emplacement de spawn véhicule !')
				return
			end

			if GangData.VehSpawnHeading == nil then
				tac.ShowNotification('Aucune rotation du véhicule !')
				return
			end

			if GangData.VehDeleter == nil then
				tac.ShowNotification('Aucun point de suppression véhicule !')
				return
			end

			if GangData.BossActions == nil then
				tac.ShowNotification('Aucun point de gestion gang !')
				return
			end

			addGangMenu.SubMenu:Visible(false)
			_TriggerServerEvent('GangsBuilder:addGang', GangData)
		end
	end
	...
	addGangMenu.SubMenu.OnListSelect = function(_, item, index)
		if item == cloakroomItem then
			if index == 1 then
				...
				tac.ShowNotification('Coordonnées ajouté.')
			else
		...
		if item == vehSpawnHeadingItem then
			if index == 1 then
				...
				tac.ShowNotification('Rotation ajouté.')
			else
...
RegisterNetEvent('GangsBuilder:OpenMenu')
AddEventHandler('GangsBuilder:OpenMenu', function()
