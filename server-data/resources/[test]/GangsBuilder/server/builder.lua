...
Citizen.CreateThread(function()
	...
	for i = 1, #GangsData, 1 do
		TriggerEvent('tac_society:registerSociety', GangsData[i].Name, GangsData[i].Label, 'society_' .. GangsData[i].Name, 'society_' .. GangsData[i].Name, 'society_' .. GangsData[i].Name, {type = 'public'})
	end
end)
...
AddEventHandler('tac:playerLoaded', function(source, xPlayer)
	local plyGang = GetGang(xPlayer.job2)
	TriggerClientEvent('GangsBuilder:SyncGang', source, plyGang)
end)

AddEventHandler('tac:setJob2', function(source, job2)
	local plyGang = GetGang(job2)
	TriggerClientEvent('GangsBuilder:SyncGang', source, plyGang)
end)

tac.AddGroupCommand('gangsbuilder', 'superadmin', function(source)
