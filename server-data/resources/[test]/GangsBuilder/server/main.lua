		...
		if targetXPlayer then
			if itemType == 'item_standard' then
				...
				if targetItem.count >= amount and targetItem.count > 0 then
					if sourceXPlayer.canCarryItem(itemName, amount) then
						...
						targetXPlayer.showNotification(_U('got_confiscated', amount, tac.GetItem(itemName).label, sourceXPlayer.name))
						sourceXPlayer.showNotification(_U('you_confiscated', amount, tac.GetItem(itemName).label, targetXPlayer.name))
					else
			...
			elseif itemType == 'item_account' then
				...
				if targetAccountMoney >= amount and amount > 0 then
					...
					targetXPlayer.showNotification(_U('got_confiscated_account', amount, tac.GetAccountLabel(itemName), sourceXPlayer.name))
					sourceXPlayer.showNotification(_U('you_confiscated_account', amount, tac.GetAccountLabel(itemName), targetXPlayer.name))
				else
					sourceXPlayer.showNotification(_U('quantity_invalid'))
				end
			elseif itemType == 'item_weapon' then
				if targetXPlayer.hasWeapon(itemName) then
					if Config.VIPWeapons[itemName] == nil then
						...
						targetXPlayer.showNotification(_U('got_confiscated_weapon', tac.GetWeaponLabel(itemName), amount, sourceXPlayer.name))
						sourceXPlayer.showNotification(_U('you_confiscated_weapon', tac.GetWeaponLabel(itemName), targetXPlayer.name, amount))
					else
...
AddEventHandler('GangsBuilder:putInVehicle', function(target)
	local xPlayerTarget = tac.GetPlayerFromId(target)
...
AddEventHandler('GangsBuilder:getStockItem', function(itemName, count)
	...
	TriggerEvent('tac_addoninventory:getSharedInventory', 'society_' .. xPlayer.job2.name, function(inventory)
...
tac.RegisterServerCallback('GangsBuilder:getStockItems', function(source, cb)
	local xPlayer = tac.GetPlayerFromId(source)
...
AddEventHandler('GangsBuilder:putStockItems', function(itemName, count)
	...
	TriggerEvent('tac_addoninventory:getSharedInventory', 'society_' .. xPlayer.job2.name, function(inventory)
		...
		if sourceItem.count >= count and count > 0 then
			...
			xPlayer.showNotification(_U('have_deposited', count, tac.GetItem(itemName).label))
		else
			xPlayer.showNotification(_U('quantity_invalid'))
		end
	end)
...
tac.RegisterServerCallback('GangsBuilder:getPlayerInventory', function(source, cb)
	local xPlayer = tac.GetPlayerFromId(source)
	cb({items = xPlayer.inventory})
end)

tac.RegisterServerCallback('GangsBuilder:getOtherPlayerData', function(source, cb, target)
...
tac.RegisterServerCallback('GangsBuilder:getVehicleInfos', function(source, cb, plate)
...
tac.RegisterServerCallback('GangsBuilder:getArmoryWeapons', function(source, cb)
	local xPlayer = tac.GetPlayerFromId(source)

	TriggerEvent('tac_datastore:getSharedDataStore', 'society_' .. xPlayer.job2.name, function(store)
		local weapons = store.get('weapons') or {}
		cb(weapons)
	end)
end)

tac.RegisterServerCallback('GangsBuilder:addArmoryWeapon', function(source, cb, weaponName, weaponAmmo)
	local xPlayer = tac.GetPlayerFromId(source)
...
tac.RegisterServerCallback('GangsBuilder:removeArmoryWeapon', function(source, cb, weaponName, weaponAmmo)
	local xPlayer = tac.GetPlayerFromId(source)
...
tac.RegisterServerCallback('GangsBuilder:buyWeapon', function(source, cb, weaponName)
	local xPlayer = tac.GetPlayerFromId(source)
	...
	TriggerEvent('tac_addonaccount:getSharedAccount', 'society_' .. xPlayer.job2.name, function(account)
...
tac.RegisterServerCallback('GangsBuilder:isBusy', function(source, cb, type, identifier)
...
tac.RegisterServerCallback('GangsBuilder:AddBusyList', function(source, cb, type, identifier)
	if BusyList[type].serverId then
		identifier = source
	end
...
tac.RegisterServerCallback('GangsBuilder:RemoveBusyList', function(source, cb, type, identifier)
	if BusyList[type].serverId then
		identifier = source
	end
