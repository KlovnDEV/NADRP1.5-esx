			...
			if CompareChannels(playerData, "radio", radioChannel) then -- Check if player is in the same radio channel as you
				if playerServerId ~= player then
					if value then
						...
						MumbleSetVolumeOverrideByServerId(player, radioVolume)
					else
						if not vehicleTargets[player] then -- Mute if player is not in client vehicle
							if playerData.call > 0 then -- Check if the client is in a call
								if not CompareChannels(voiceData[player], "call", playerData.call) then -- Check if the client is in a call with the unmuted player
...
AddEventHandler('mumble:SetRadioVolume', function(passedRadioVolume)
	radioVolume = passedRadioVolume
end)
		...
		if initialised then
			...
			if mumbleConfig.radioEnabled then
				if not mumbleConfig.controls.radio.pressed then
					...
					if (IsEntityPlayingAnim(PlayerPedId(), "random@arrests","generic_radio_chatter", 3)) and not playerRadioActive then
...
function GetCallChannel()
	local callChannel = voiceData[playerServerId]["call"] or 0
	if callChannel ~= nil then
		return callChannel
	end
end

function GetRadioChannel()
	local radioChannel = voiceData[playerServerId]["radio"] or 0
