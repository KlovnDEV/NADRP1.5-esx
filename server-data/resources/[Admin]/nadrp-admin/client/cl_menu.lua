nadrp.Admin.Menu = {}
nadrp._Admin.Menu = {}
nadrp._Admin.Menu.PlayerOptions = {}
nadrp._Admin.Menu.SearchOptions = {
    type = nil,
    data = nil
}
nadrp._Admin.Menu.Target = {}
...
function nadrp.Admin.Menu.Init(self)
...
function nadrp.Admin.Menu.SetSubMenuProperties(self, menu)
    WarMenu.SetMenuWidth(menu, 0.5)
    WarMenu.SetTitleColor(menu, 135, 206, 250, 255)
    WarMenu.SetTitleBackgroundColor(menu, 0 , 0, 0, 150)
    WarMenu.SetMenuBackgroundColor(menu, 0, 0, 0, 100)
...
function nadrp.Admin.Menu.DrawCommand(self, cmd)
    if not cmd or not nadrp.Admin:CommandExists(cmd) then return end

    Citizen.CreateThread(function()
        while true do
            Citizen.Wait(0)
            if WarMenu.IsMenuOpened("command") then nadrp.Admin:GetCommandData(cmd).drawcommand() else return end
            if WarMenu.MenuButton("Back", "acommands") then return end
        end
    end)
end

function nadrp.Admin.Menu.DrawTargets(self, cmd, cb)
    WarMenu.OpenMenu("targetmenu")

    Citizen.CreateThread(function()
        while WarMenu.IsMenuOpened("targetmenu") do
            Citizen.Wait(0)
            for k,v in spairs(nadrp._Admin.Players, function(t, a, b) return t[a].src < t[b].src end) do
                if WarMenu.MenuButton("[" .. v.src .. "] " .. v.name, "command") then self:DrawCommand(cmd) cb(v) nadrp.Admin:GetCommandData(cmd).drawcommand() return end
            end
            if WarMenu.MenuButton("Back", "command") then self:DrawCommand(cmd) nadrp.Admin:GetCommandData(cmd).drawcommand() return end
        end
    end)
end

function nadrp.Admin.Menu.DrawTextInput(self, defaultText, cb)
    Citizen.CreateThread(function()
...
function nadrp.Admin.Menu.ShowTextEntry(self, title, subMsg, cb)
    SendNUIMessage({open = true, textEntry = true, title = title, submsg = subMsg and subMsg or ""})
    textEntryCb = function(text) cb(text) end
end

function nadrp.Admin.Menu.DrawRanks(self, cmd, cb)
    WarMenu.OpenMenu("ranklist")

    Citizen.CreateThread(function()
        while WarMenu.IsMenuOpened("ranklist") do
            Citizen.Wait(0)
            for k,v in spairs(nadrp.Admin:GetRanks(), function(t, a, b) return t[a].grant < t[b].grant end) do
                if WarMenu.MenuButton(k, "command") then self:DrawCommand(cmd) cb(k) nadrp.Admin:GetCommandData(cmd).drawcommand() return end
            end
            if WarMenu.MenuButton("Back", "command") then self:DrawCommand(cmd) nadrp.Admin:GetCommandData(cmd).drawcommand() return end
        end
    end)
...
Citizen.CreateThread(function()
    ...
    local function DrawPlayers()
        for k,v in spairs(nadrp._Admin.Players, function(t, a, b) return t[a].src < t[b].src end) do
            if WarMenu.Button("[" .. v.src .. "] " .. v.name .. (v.sessioned and " - SESSIONED?" or ""), v.status, v.sessioned and {r = 255, g = 0, b = 0, a = 255} or nil) then nadrp._Admin.Menu.PlayerOptions = v viewingIp = false viewingLicense = false WarMenu.OpenMenu("aplayeropts") end
        end
    ...
    local function DrawDiscPlayers()
        for k,v in spairs(nadrp._Admin.DiscPlayers, function(t, a, b) return t[a].src < t[b].src end) do
            if WarMenu.MenuButton("[" .. v.src .. "] " .. v.name, "aplayeropts") then nadrp._Admin.Menu.PlayerOptions = v viewingIp = false end
        end
        if WarMenu.MenuButton("Back", "aplayers") then return end
    end

    local function DrawPlayerOptions()
        local options = nadrp._Admin.Menu.PlayerOptions
        ...
        if options then
            ...
            if nadrp.Admin:IsSuperAdmin() then
    ...
    local function DrawCommands()
        local userRank = nadrp.Admin:GetPlayerRank()

        if cat then
            for k,v in spairs(nadrp.Admin:GetCommands(), function(t, a, b) return t[a].title < t[b].title end) do
                if v.category == cat then
                    if nadrp.Admin:RankHasCommand(userRank, v.command) then if WarMenu.Button(v.title) then WarMenu.OpenMenu("command") nadrp.Admin.Menu:DrawCommand(v.command) end end
                end
            end
        end
    ...
    local function DrawCategories()
        for k,v in spairs(nadrp.Admin:GetCategories(), function(t, a, b) return a < b end) do
    ...
    local function DrawDev()
        local userRank = nadrp.Admin:GetPlayerRank()
        if WarMenu.Button("Dev mode: " .. (nadrp._Admin.Menu.DevMode and "Disable" or "Enable")) and userRank == "superadmin" then 
            nadrp._Admin.Menu.DevMode = not nadrp._Admin.Menu.DevMode 
            TriggerEvent("nadrp-admin:currentDevmode",nadrp._Admin.Menu.DevMode)
        end
        if WarMenu.Button("Dev debug: " .. (nadrp._Admin.Menu.DevDebug and "Disable" or "Enable")) and userRank == "superadmin" then 
            nadrp._Admin.Menu.DevDebug = not nadrp._Admin.Menu.DevDebug 
            TriggerEvent("nadrp-admin:currentDebug",nadrp._Admin.Menu.DevDebug)
            TriggerEvent("hud:enabledebug", nil,nadrp._Admin.Menu.DevDebug)
        end
        if WarMenu.MenuButton("Back", "amenu") then return end
    end
...
RegisterNetEvent("nadrp-admin:openMenu")
AddEventHandler("nadrp-admin:openMenu", function()
