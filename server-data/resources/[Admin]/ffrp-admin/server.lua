...
RegisterServerEvent('ffrp-admin:Cloak')
AddEventHandler('ffrp-admin:Cloak', function(src, toggle)
    FFRP._Admin.CloakedPlayers[src] = toggle
    TriggerClientEvent("ffrp-admin:Cloak", -1, src, toggle)
end)

AddEventHandler("tac:playerLoaded", function(user)
    TriggerClientEvent("ffrp-admin:Cloak", user, FFRP._Admin.CloakedPlayers)
end)

AddEventHandler("playerDropped", function()
    local src = source
    if FFRP._Admin.CloakedPlayers[src] then
        FFRP._Admin.CloakedPlayers[src] = false
        TriggerClientEvent("ffrp-admin:Cloak", -1, src, false)
    end
end)
...
RegisterServerEvent('ffrp-admin:RaveMode')
AddEventHandler('ffrp-admin:RaveMode', function(toggle)
    local source = source
    TriggerClientEvent('ffrp-admin:RaveMode', -1, toggle)
end)

RegisterServerEvent('ffrp-admin:AddPlayer')
AddEventHandler("ffrp-admin:AddPlayer", function()
    local licenses
    ...
    TriggerClientEvent("ffrp-admin:AddPlayer", -1, data)
    FFRP.Admin.AddAllPlayers()
end)

RegisterServerEvent('ffrp-admin:bringPlayer')
AddEventHandler('ffrp-admin:bringPlayer', function(target, coords)
    local source = source
...
function FFRP.Admin.AddAllPlayers(self)
    ...
    for i=1, #xPlayers, 1 do
        ...
        TriggerClientEvent("ffrp-admin:AddAllPlayers", source, data)
    end
end

function FFRP.Admin.AddPlayerS(self, data)
...
AddEventHandler("playerDropped", function()
    ...
    TriggerClientEvent("ffrp-admin:RemovePlayer", -1, data )
    Wait(600000)
    TriggerClientEvent("ffrp-admin:RemoveRecent", -1, data)
end)
    ...
    if targ ~= nil then
        if timer ~= nil then
            if (user.getGroup() ~= "user") and (user.getGroup() ~= "") then
                TriggerClientEvent('ffrp-admin:startCooldown', targ.source, timer)
