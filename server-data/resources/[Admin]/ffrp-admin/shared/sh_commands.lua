...
function cmd.RunCommand(caller, args)
    ...
    local steamid = exports["ffrp-ped"]:isPed("steamid")
    local log = string.format("%s [%s] Set %s [steam:%s] : Rank to : %s ", name, steamid, caller.target.name, caller.target.comid, caller.rank)

    FFRP.Admin:Log('Admin', 'Set Rank', log, PlayerPedId())
    FFRP.Admin:Bring(PlayerPedId(), caller.target.src)
end
...
function cmd.DrawCommand()
        ...
        if WarMenu.Button("Select a target", "Selected: " .. (cmd.vars.target and cmd.vars.target.name or "None")) then FFRP.Admin.Menu:DrawTargets(cmd.command, function(_target)
            cmd.vars.target = _target
        end) end

        if WarMenu.Button("Select a rank", "Selected: " .. (cmd.vars.rank ~= nil and tostring(cmd.vars.rank) or "None")) then FFRP.Admin.Menu:DrawRanks(cmd.command, function(_rank)
            cmd.vars.rank = _rank
        end) end
        ...
        if args.target and args.rank then if WarMenu.Button("Set " .. args.target.name .. "'s rank to " .. args.rank) then FFRP.Admin:GetCommandData(cmd.command).runcommand(args) cmd.vars.target = nil cmd.vars.rank = nil end end
end

FFRP.Admin:AddCommand(cmd)
...
function cmd.RunCommand(caller, args, source)
    ...
    local msg = 'Player : ' .. caller.target.steamid .. " banned for time : " .. caller.time .. " | Reason : " .. caller.reason
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a ban length", "Length: " .. (cmd.vars.time and cmd.vars.time or "No Time Given")) then
        FFRP.Admin.Menu:DrawTextInput(cmd.vars.time and cmd.vars.time or "", function(result)
            if result ~= nil then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
                cmd.vars.time = result
                print(tonumber(cmd.vars.time))
            end
        end)
    end

    if WarMenu.Button("Enter a reason", "Reason: " .. (cmd.vars.reason and cmd.vars.reason or "No Reason Given")) then
        FFRP.Admin.Menu:DrawTextInput(cmd.vars.reason and cmd.vars.reason or "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.target and cmd.vars.time then if WarMenu.Button("Ban " .. cmd.vars.target.name) then FFRP.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, reason = cmd.vars.reason, time = cmd.vars.time}) cmd.vars.target = nil cmd.vars.reason = nil cmd.vars.time = nil end end
end
...
function cmd.RunCommand( caller, args, source)
    ...
    local name = GetPlayerName(PlayerId())
    local msg = 'Player with hex : ' .. caller.steamid .. " unbanned from server"
    local steamid = exports["ffrp-ped"]:isPed("steamid")
    local log = string.format("%s [%s] un-banned : %s [steam:%s]", name, steamid, caller.target.name, caller.target.comid)
    TriggerEvent('chat:addMessage', {
        template = '<div class="chat-message emergency">SERVER | {0} </div>',
        args = {msg}
    })
    FFRP.Admin:Log('Admin', 'Unban Player', log, PlayerPedId())
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a Steam ID", "Entered: " .. (cmd.vars.steamid and cmd.vars.steamid or "None")) then 
        FFRP.Admin.Menu:ShowTextEntry("Enter a Steam ID", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.steamid then if WarMenu.Button("Unban " .. cmd.vars.steamid) then FFRP.Admin:GetCommandData(cmd.command).runcommand({steamid = cmd.vars.steamid}) cmd.vars.steamid = nil end end
end
...
function cmd.RunCommand(caller, args)
    if not caller.target then return end
    if not caller.reason then caller.reason = "No Reason Given" end
    TriggerServerEvent("EasyAdmin:kickPlayer", caller.target.src, caller.reason)
    local name = GetPlayerName(PlayerId())
    local steamid = exports["ffrp-ped"]:isPed("steamid")
    local log = string.format("%s [%s] Kicked : %s [steam:%s] For : %s", name, steamid, caller.target.name, caller.target.comid, caller.reason)
    FFRP.Admin:Log('Admin', 'Kick', log, PlayerPedId())
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.target then if WarMenu.Button("Kick " .. cmd.vars.target.name) then FFRP.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, reason = cmd.vars.reason}) end end
end
...
function cmd.RunCommand(caller, args)
    ...
    FFRP.Admin:Log('Admin', 'God Mode', caller.toggle, PlayerPedId())
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("God Mode: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle FFRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.DrawCommand()
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Noclip: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle FFRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.DrawCommand()
    if WarMenu.Button("Teleport to marker") then FFRP.Admin:GetCommandData(cmd.command).runcommand({}) end
end
...
function cmd.RunCommand(caller, args)
    ...
    TriggerServerEvent('ffrp-admin:Cloak', src, caller.toggle)
end
...
function cmd.Init()
    ...
    if IsDuplicityVersion() then 
        AddEventHandler("playerDropped", function()
            ...
            if cmd.vars.cloaked[src] then
                TriggerServerEvent('ffrp-admin:Cloak', src, false)
    ...
    RegisterNetEvent("ffrp-admin:CloakRemote")
    AddEventHandler("ffrp-admin:CloakRemote", function()
        if FFRP.Admin:GetPlayerRank() == "dev" then
            cmd.vars.toggle = not cmd.vars.toggle
            FFRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle})
        end
    end)

    RegisterNetEvent("ffrp-admin:Cloak")
    AddEventHandler("ffrp-admin:Cloak", function(player, toggle)
...
function cmd.DrawCommand()
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Cloak: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle FFRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.RunCommand(caller, args)
    if not caller.target then return end
	TriggerEvent('mythic_hospital:client:RemoveBleed', caller.target.src) 
	TriggerEvent('mythic_hospital:client:ResetLimbs', caller.target.src)
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Revive in Distance") then 
        TriggerEvent("ffrp-admin:ReviveInDistance")
    end

    if cmd.vars.target then if WarMenu.Button("Revive and Heal " .. cmd.vars.target.name) then FFRP.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, runontarget = true}) end end
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter x", "x: " .. (cmd.vars.x and cmd.vars.x or "0.0")) then
        FFRP.Admin.Menu:ShowTextEntry("Enter Item", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end

            if string.find(result, ",") then
                local resultSplit = FFRP.Admin.split(result, ',')
    ...
    if WarMenu.Button("Enter y", "y: " .. (cmd.vars.y and cmd.vars.y or "0.0")) then
        FFRP.Admin.Menu:ShowTextEntry("Enter y", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if WarMenu.Button("Enter z", "z: " .. (cmd.vars.z and cmd.vars.z or "0.0")) then
        FFRP.Admin.Menu:ShowTextEntry("Enter z", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.x and cmd.vars.y and cmd.vars.z then if WarMenu.Button("Teleport to: " .. "X:["..cmd.vars.x.."] Y:["..cmd.vars.y.."] Z:["..cmd.vars.z.."]") then FFRP.Admin:GetCommandData(cmd.command).runcommand({x = cmd.vars.x,y = cmd.vars.y,z = cmd.vars.z}) end end
end
...
function cmd.RunCommand(caller, args)
    ...
    FFRP.Admin:Log('Admin', 'Announcement', caller.message, PlayerPedId())
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a message", "Message: " .. (cmd.vars.message and cmd.vars.message or "No Message")) then
        FFRP.Admin.Menu:ShowTextEntry("Enter a Message", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.message then if WarMenu.Button("Send Message: " .. cmd.vars.message) then FFRP.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message}) end end
end
...
function cmd.RunCommand(caller, args)
    if not caller.message then return end
    TriggerEvent("ffrp-admin:runSpawnCommand", caller.message)
    FFRP.Admin:Log('Admin', 'Spawned Car', tostring(caller.message), PlayerPedId())
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter a model", "Model: " .. (cmd.vars.message and cmd.vars.message or "No model")) then
        FFRP.Admin.Menu:ShowTextEntry("Enter a model", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.message then 
        if WarMenu.Button("Spawn model: " .. cmd.vars.message) then 
            FFRP.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message}) 
        end 
    end
end
FFRP.Admin:AddCommand(cmd)
...
function cmd.RunCommand(caller, args)
    ...
    FFRP.Admin:Log('Admin', 'Spawned item', tostring(caller.message)..' x'..caller.amount, PlayerPedId())
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter Amount", "Amount: " .. (cmd.vars.amount and cmd.vars.amount or "1")) then
        FFRP.Admin.Menu:ShowTextEntry("Enter Amount", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
            end
    ...
    if cmd.vars.message then if WarMenu.Button("Spawn Item: " .. cmd.vars.message.." | "..cmd.vars.amount) then FFRP.Admin:GetCommandData(cmd.command).runcommand({message = cmd.vars.message,amount = cmd.vars.amount}) end end
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.target then if WarMenu.Button("Teleport to " .. cmd.vars.target.name .. "'s position") then FFRP.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, retn = false}) end end
    if cmd.vars.lastPos then if WarMenu.Button("Return to your last position") then FFRP.Admin:GetCommandData(cmd.command).runcommand({target = false, retn = true}) end end
end
...
function cmd.RunCommand(caller, args)
    ...
    local name = GetPlayerName(PlayerId())
    local steamid = exports["ffrp-ped"]:isPed("steamid")
    local log = string.format("%s [%s] teleported : %s [steam:%s] : to them self", name, steamid, caller.target.name, caller.target.comid)

    FFRP.Admin:Log('Admin', 'Teleported', log, PlayerPedId())
    FFRP.Admin:Bring(PlayerPedId(), caller.target.src)
end
...
function cmd.DrawCommand()
    ...
    if cmd.vars.target then if WarMenu.Button("Bring " .. cmd.vars.target.name .. " to you.") then FFRP.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, retn = false}) end end
end
...
function cmd.RunCommand(caller, args)
    if not caller.target then return end
    cmd.RunClCommand(caller)
end

function cmd.RunClCommand(args)
    if args.target == nil then return end 
    local ped = PlayerPedId()
    local targId = not args.retn and GetPlayerFromServerId(args.target.src)
...
function cmd.DrawCommand()
    ...
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Attach: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle FFRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle,target = cmd.vars.target}) end
end
...
function cmd.RunCommand(caller, args)
    ...
    local name = GetPlayerName(PlayerId())
    local steamid = exports["ffrp-ped"]:isPed("steamid")
    local log = string.format("%s [%s] set ravemode: %s", name, steamid, caller.toggle)
    FFRP.Admin:Log('Admin', 'Rave Mode', log, PlayerPedId())
end
...
function cmd.Init()
    ...
    RegisterNetEvent("ffrp-admin:RaveMode")
    AddEventHandler("ffrp-admin:RaveMode", function(toggle)
...
function cmd.RunClCommand(args)
    TriggerServerEvent("ffrp-admin:RaveMode", args.toggle)
end

function cmd.DrawCommand()
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Rave Mode: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle FFRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.RunCommand(caller, args)
    ...
    FFRP.Admin:Log('Admin', 'Player Blips', {playerBlips = caller.toggle}, PlayerPedId())
end
...
function cmd.DrawCommand()
    cmd.vars.toggle = cmd.vars.toggle ~= nil and cmd.vars.toggle or nil
    if WarMenu.Button("Player Blips: " .. (cmd.vars.toggle and "Disable" or "Enable")) then cmd.vars.toggle = not cmd.vars.toggle FFRP.Admin:GetCommandData(cmd.command).runcommand({toggle = cmd.vars.toggle}) end
end
...
function cmd.RunCommand(caller, args)
    ...
    local name = GetPlayerName(PlayerId())
    local steamid = exports["ffrp-ped"]:isPed("steamid")
    local log = string.format("%s [%s] fixed %s's [steam:%s] vehicle", name, steamid, caller.target.name, caller.target.comid)
    FFRP.Admin:Log('Admin', 'Fix Car', log, PlayerPedId())
end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Select a target", "Selected: " .. (cmd.vars.target and cmd.vars.target.name or "None")) then 
        FFRP.Admin.Menu:DrawTargets(cmd.command, function(_target)
            cmd.vars.target = _target
        end)
    end

    if cmd.vars.target then if WarMenu.Button("Fix " .. cmd.vars.target.name .. "'s vehicle") then FFRP.Admin:GetCommandData(cmd.command).runcommand({target = cmd.vars.target, runontarget = true}) end end
...
function cmd.DrawCommand()
    ...
    if WarMenu.Button("Enter an entity ID") then 
        FFRP.Admin.Menu:ShowTextEntry("Enter an entity ID", "", function(result)
            if result then
                if string.gsub(result, " ", "") == "" or result == "" then result = nil end
    ...
    if cmd.vars.ent then if WarMenu.Button("Delete Entity", "Entity: " .. (cmd.vars.ent or "none")) then
        FFRP.Admin:GetCommandData(cmd.command).runcommand({ent = cmd.vars.ent})
    end end
end
