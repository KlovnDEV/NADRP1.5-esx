...
FFRP = FFRP or {}
FFRP.Admin = FFRP.Admin or {}
FFRP.Admin.DB = {}
FFRP._Admin = FFRP._Admin or {}
FFRP._Admin.Ranks = {}
FFRP._Admin.Commands = {}
FFRP._Admin.Players = {}
FFRP._Admin.DiscPlayers = {}
FFRP._Admin.CurAdmins = {}

function FFRP.Admin.Log(self, table, type, log, user)
    if not log or not tostring(log) then return else log = tostring(log) end

    if log then
        local name = GetPlayerName(PlayerId())
        local steamid = exports['ffrp-ped']:isPed('steamid')
        exports["ffrp-log"]:AddLog(table, name, steamid, type, log)
        return
    end
...
function FFRP.Admin.ChatPrint(self, msg, target)

end

function FFRP.Admin.RankExists(self, rank)
    rank = tostring(rank)
...
function FFRP.Admin.CommandExists(self, cmd)
    cmd = tostring(cmd)
...
function FFRP.Admin.RankInheritsCommand(self, rank, cmd)
...
function FFRP.Admin.GetPlayerRank(self, user)
    if not IsDuplicityVersion() then
        return group
    end
end

function FFRP.Admin.RankHasCommand(self, rank, cmd)
...
function FFRP.Admin.GetRanks(self)
    return FFRP._Admin.Ranks
end

function FFRP.Admin.GetRankData(self, rank)
    if not rank then return false end
...
function FFRP.Admin.GetCommands(self)
    return FFRP._Admin.Commands
end

function FFRP.Admin.GetCategories(self)
    local tmp = {}
...
function FFRP.Admin.GetCommandData(self, cmd)
    if not cmd then return false end
...
function FFRP.Admin.IsAdmin(self, user)
...
function FFRP.Admin.IsSuperAdmin(self, user)
    if not IsDuplicityVersion() then
...
function FFRP.Admin.IsValidUser(self, user)
    if not user then return false end
...
function FFRP.Admin.CanTarget(self, user, target)
...
function FFRP.Admin.CanTargetRank(self, user, rank)
...
function FFRP.Admin.AddCommand(self, cmd)
    local tmp = {}
    ...
    if chicken then
        ...
        tmp.runcommand = function(args) args.command = tmp.command FFRP.Admin:RunCommand(args) end
...
function FFRP.Admin.AddRank(self, rank, data)
    local tmp = {}
...
--[[function FFRP.Admin.GetBanTimeFromString(self, time)
...
function FFRP.Admin.SetStatus(self, status, src)
    ...
    if IsDuplicityVersion() then
        ...
        if not player then else FFRP._Admin.Players[src].status = status end

        for k,v in pairs(FFRP._Admin.CurAdmins) do
            TriggerClientEvent("ffrp-admin:setStatus", k, src, status)
        end

        return
    end

    TriggerServerEvent("ffrp-admin:setStatus", status)
end

function FFRP.Admin.Bring(self, player, target)
    ...
    TriggerServerEvent('ffrp-admin:bringPlayer', target, pCoords)
end