...
function FFRP.Admin.CheckForSessions(self)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            local players = FFRP._Admin.Players

            for k,v in pairs(players) do
                local src = v.src
                local playerId = GetPlayerFromServerId(src)
                if not src then
                    FFRP._Admin.Players[src].sessioned = true
                else
                    if not NetworkIsPlayerActive(playerId) or not NetworkIsPlayerConnected(playerId) then FFRP._Admin.Players[src].sessioned = true end
                end
            end
        end
    end)
...
stop = true
function FFRP.Admin.BeginAFK(self, stop)
    if stop then
        afk.isAfk = false
        afk.msgAFK = false
        afk.stringToType = ""
    ...
    afk.event = AddEventHandler("ffrp-admin:afkStringCheck", function(text)
        if string.lower(text) == afk.stringToType then FFRP.Admin:BeginAFK(true) return end
    end)
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if curTime - beginTime >= 300000 then TriggerServerEvent("ffrp-admin:Disconnect", "AFK Kick") return end
        end
    end)
end

function FFRP.Admin.AFKCheck(self)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            local rank = FFRP.Admin:GetPlayerRank()
            local rankData = FFRP.Admin:GetRankData(rank)
...
RegisterNetEvent("ffrp-admin:afkStringCheck")

RegisterNetEvent("ffrp-admin:setStatus")
AddEventHandler("ffrp-admin:setStatus", function(src, status)
    local player = FFRP._Admin.Players[src]
    if not player then return else FFRP._Admin.Players[src].status = status end
end)

RegisterNetEvent("ffrp-admin:sendPlayerInfo")
AddEventHandler("ffrp-admin:sendPlayerInfo", function(data, discData)
    FFRP._Admin.Players = data
    FFRP._Admin.DiscPlayers = discData
end)

RegisterNetEvent("ffrp-admin:RemovePlayer")
AddEventHandler("ffrp-admin:RemovePlayer", function(data)
    FFRP._Admin.DiscPlayers[data.src] = data
    FFRP._Admin.Players[data.src] = nil
end)

RegisterNetEvent("ffrp-admin:RemoveRecent")
AddEventHandler("ffrp-admin:RemoveRecent", function(data)
    if data ~= nil then
        FFRP._Admin.DiscPlayers[data.src] = nil
    end
end)

RegisterNetEvent("ffrp-admin:AddPlayer")
AddEventHandler("ffrp-admin:AddPlayer", function(player)
    FFRP._Admin.Players[player.src] = player
end)

RegisterNetEvent('ffrp-admin:teleportUser')
AddEventHandler('ffrp-admin:teleportUser', function(x, y, z)
    SetEntityCoords(PlayerPedId(), x, y, z)
    TriggerEvent('DoLongHudText', 'You were summoned')
end)

RegisterNetEvent("ffrp-admin:AddAllPlayers")
AddEventHandler("ffrp-admin:AddAllPlayers", function(data)
    FFRP._Admin.Players[data.src] = data
end)

function FFRP.Admin.RunCommand(self, args)
    if not args or not args.command then return end
    TriggerServerEvent("ffrp-admin:runCommand", args)
end

function FFRP.Admin.RunClCommand(self, cmd, args)
    if not cmd or not self:CommandExists(cmd) then return end
    self:GetCommandData(cmd).runclcommand(args)
end

RegisterNetEvent("ffrp-admin:RunClCommand")
AddEventHandler("ffrp-admin:RunClCommand", function(cmd, args)
    FFRP.Admin:RunClCommand(cmd, args)
end)

RegisterNetEvent("ffrp-admin:updateData")
AddEventHandler("ffrp-admin:updateData", function(src, type, data)
    if not src or not type or not data then return end
    if not FFRP._Admin.Players[src] then return end
    
    FFRP._Admin.Players[src][type] = data
end)

RegisterNetEvent("ffrp-admin:noLongerAdmin")
AddEventHandler("ffrp-admin:noLongerAdmin", function()
    FFRP._Admin.Players = {}
    
    for k,v in pairs(FFRP._Admin.Menu.Menus) do
        if WarMenu.IsMenuOpened(k) then WarMenu.CloseMenu() end
    end
end)

--[[RegisterNetEvent("ffrp-admin:bringPlayer")
AddEventHandler("ffrp-admin:bringPlayer", function(targPos)
    local ped = PlayerPedId()
...
RegisterNetEvent("ffrp-admin:bring")
AddEventHandler("ffrp-admin:bring", function(target)
    ...
    TriggerServerEvent("ffrp-admin:bringPlayerServer",pos,target)
end)

RegisterNetEvent("ffrp-admin:runSpawnCommand")
AddEventHandler("ffrp-admin:runSpawnCommand", function(model, livery)
    Citizen.CreateThread(function()
...
function FFRP.Admin.teleportMarker(self)
...
RegisterNetEvent("ffrp-admin:ReviveInDistance")
AddEventHandler("ffrp-admin:ReviveInDistance", function()
    local playerList = {}
...
RegisterNetEvent('ffrp-admin:startCooldown')
AddEventHandler('ffrp-admin:startCooldown', function(timer)
    pCooldown = timer
