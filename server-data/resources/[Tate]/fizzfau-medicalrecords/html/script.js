$(document).ready(function(){
    $(".container").hide();
    $(".create-report").hide();
    var current = "#show-page"; var other =  "#create-page"; var result = null
    ...
    document.onkeyup = function (data) {
        if (data.which == 13) { //enter
            ...
            if (input != "") {
                $.post('http://fizzfau-medicalrecords/search', JSON.stringify({input: input}));
            } else {
   ...
   $(document).on("click", "#close", function (param) { 
    ...
    function close() { 
        $.post('http://fizzfau-medicalrecords/close', JSON.stringify({display: false}));
    ...
    $(document).on("click", "#patient-submit", function (e) {
        ...
        if (patient_name != "" || patient_data != "") {
            document.getElementById("patient-name").value = ""
            document.getElementById("patient-data").value = ""
            $.post('http://fizzfau-medicalrecords/create', JSON.stringify({name: patient_name, text: patient_data}));
