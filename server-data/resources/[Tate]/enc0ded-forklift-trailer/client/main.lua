...
if Config.enableClientCommands then
	RegisterCommand("spawn-pallets", function(_, args)
		...
		local offsetFromPlayer = GetOffsetFromEntityInWorldCoords(PlayerPedId(-1), 0.0, 1.0, 0.0)
		local coords = {x = offsetFromPlayer.x, y = offsetFromPlayer.y, z = offsetFromPlayer.y, h = GetEntityHeading(PlayerPedId(-1))}
		SpawnPalletsWithProducts(num, kind, coords, {x=0.0, y=2.25})
	end)
	...
	RegisterCommand("delete-pallet", function()
		...
		local productNetId = DecorGetInt(pallet, 'product_attached')
		local productEntity = NetworkGetEntityFromNetworkId(productNetId)
		DeleteEntity(pallet)
		DeleteEntity(productEntity)
	end)
